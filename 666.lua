...
-- // NUEVOS IDs MÁS REALISTAS Y PERTURBADORES
local ASSETS = {
    FuegoReal = 241907058,          -- Fuego intenso
    SatanAvatarID = "86446020650559", -- Nuevo ID de Satanás
    LetrasSangre = 88425869123525,  -- Efecto de sangre hiperrealista
    Slash = 121659392958456         -- Efecto de cortes nuevo
}

-- // Cargar la descripción de Satanás de fondo para evitar lag al invocar
local SatanDesc = nil
task.spawn(function()
    pcall(function()
        SatanDesc = Players:GetHumanoidDescriptionFromUserId(tonumber(ASSETS.SatanAvatarID))
    end)
end)
...
-- // NUEVA FUNCIÓN ÚNICA: Clona a Satanás para TODO el ritual (Tamaño Normal)
local function CrearSatanNPC(targetCFrame, parentFolder, isCentral)
    local char = LocalPlayer.Character
    char.Archivable = true 
    local npc = char:Clone()
    npc.Name = isCentral and "SatanasSupremo" or "SatanasManada"
    npc.Parent = parentFolder
    
    -- Limpiar scripts para evitar fallos
    ...
    if hum then
        ...
        task.spawn(function()
            -- Si por alguna razón no cargó antes, intentamos de nuevo
            if not SatanDesc then
                pcall(function() SatanDesc = Players:GetHumanoidDescriptionFromUserId(tonumber(ASSETS.SatanAvatarID)) end)
            end
            
            if SatanDesc then
                pcall(function() 
                    hum:ApplyDescription(SatanDesc) 
                end)
            end
        end)
    end
...
local function CrearEscenaEpica()
    ...
    -- 3. Entidad Demoníaca Central (Satanás Normal Size)
    local cframeCentralOculto = CFrame.lookAt(posOculta + Vector3.new(0, 7, 0), posOculta + (spawnPos - hrp.Position).Unit * 10)
    local _, DemonioHRP = CrearSatanNPC(cframeCentralOculto, SceneFolder, true)

    -- Partículas Nuevas en el Central
    if DemonioHRP then
        ...
        BloodLetters.Parent = DemonioHRP
    end
    ...
    -- 5. LA MANADA: Ahora son 5 Clones de Satanás ascendiendo desde el suelo
    local EspectrosHRP = {}
    ...
    for i = 1, 5 do
        ...
        -- Empiezan bajo tierra y suben al mismo tiempo
        local posOcultaEspectro = posOculta + Vector3.new(offsetX, 0, offsetZ) 
        local posFinalEspectro = spawnPos + Vector3.new(offsetX, 1.5, offsetZ) 
        
        local lookCFrameOculto = CFrame.lookAt(posOcultaEspectro, posOculta)
        local lookCFrameFinal = CFrame.lookAt(posFinalEspectro, spawnPos)
        
        local _, espectroHRP = CrearSatanNPC(lookCFrameOculto, SceneFolder, false)
        
        if espectroHRP then
            -- Aura oscura para distinguir a los clones
            local aura = Instance.new("ParticleEmitter")
            aura.Texture = "rbxassetid://" .. tostring(ASSETS.FuegoReal)
            aura.Color = ColorSequence.new(Color3.fromRGB(0, 0, 0), Color3.fromRGB(40, 0, 0))
            aura.Size = NumberSequence.new({NumberSequenceKeypoint.new(0, 4), NumberSequenceKeypoint.new(1, 0)})
            aura.Rate = 50 
            aura.Speed = NumberRange.new(2, 4)
            aura.Lifetime = NumberRange.new(1, 1.5)
            aura.ZOffset = 1 
            aura.Parent = espectroHRP
            
            table.insert(EspectrosHRP, { HRP = espectroHRP, PosFinal = lookCFrameFinal })
        end
    end

    -- // ANIMACIONES (Todo sube coordinado desde las profundidades)
    ...
    -- Invocación del Demonio y el Cubo
    ...
    if DemonioHRP then
        local cframeFinalCentral = CFrame.lookAt(posFinal + Vector3.new(0, 7, 0), posFinal + (hrp.Position - spawnPos).Unit * 10)
        TweenService:Create(DemonioHRP, tweenInfoSubir, {CFrame = cframeFinalCentral}):Play()
    end
    
    -- Invocación de los 5 clones de Satanás de la manada
    for _, datos in ipairs(EspectrosHRP) do
        TweenService:Create(datos.HRP, tweenInfoSubir, {CFrame = datos.PosFinal}):Play()
    end
    ...
    if DemonioHRP then
        local cframeBajarCentral = CFrame.lookAt(posOculta + Vector3.new(0, 7, 0), posOculta + (hrp.Position - spawnPos).Unit * 10)
        TweenService:Create(DemonioHRP, tweenInfoBajar, {CFrame = cframeBajarCentral}):Play()
    end

    for _, datos in ipairs(EspectrosHRP) do
        local lookCFrameAbajo = CFrame.lookAt(datos.HRP.Position - Vector3.new(0, 25, 0), posOculta)
        TweenService:Create(datos.HRP, tweenInfoBajar, {CFrame = lookCFrameAbajo}):Play()
    end
    
    -- Apagar flamas y auras
...
Tab:CreateLabel("Script Final: Manada de Clones de Satanás + FX Realistas")
