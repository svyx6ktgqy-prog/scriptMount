...
-- // NUEVA Función: Generar Espectros (SOLIDOS DESDE EL INICIO PARA EVITAR BUGS)
local function GenerarDemonioMinion(cframeOculto, parentFolder)
    local minion = Instance.new("Part")
    minion.Size = Vector3.new(4, 6, 4)
    minion.CFrame = cframeOculto -- Empiezan bajo tierra
    minion.Anchored = true
    minion.CanCollide = false
    minion.Color = Color3.fromRGB(15, 0, 0)
    minion.Transparency = 0 -- ¡CERO TRANSPARENCIA! Esto arregla el bug de invisibilidad
    ...
    minionMesh.Scale = Vector3.new(0.08, 0.08, 0.08) 
    minionMesh.VertexColor = Vector3.new(1.5, 0.1, 0.1) 
...
local function CrearEscenaEpica()
    ...
    -- Posiciones de animación del Jefe
    ...
    -- 3. Entidad Demoníaca Jefe (Satan Central)
    ...
    for i = 1, 5 do
        ...
        -- Posición final en la superficie (Elevados +4 para no hundirse)
        local posFinalMinion = spawnPos + Vector3.new(offsetX, 4, offsetZ) 
        -- Posición inicial bajo tierra (-25)
        local posOcultaMinion = posFinalMinion - Vector3.new(0, 25, 0)
        
        -- Hacemos que miren al centro
        local lookCFrameOculto = CFrame.lookAt(posOcultaMinion, posOculta + Vector3.new(0, 4, 0))
        local lookCFrameFinal = CFrame.lookAt(posFinalMinion, spawnPos + Vector3.new(0, 4, 0))
        
        local minion, aura = GenerarDemonioMinion(lookCFrameOculto, SceneFolder)
        
        -- Guardamos sus datos para animarlos todos juntos
        table.insert(DataEspectros, {
            Parte = minion,
            Aura = aura,
            CF_Arriba = lookCFrameFinal,
            CF_Abajo = lookCFrameOculto
        })
    end
    ...
    -- Sube el Demonio Jefe desde el cubo rojo
    ...
    -- Suben los 5 Demonios de la manada desde el suelo
    for _, datos in ipairs(DataEspectros) do
        if datos.Aura then datos.Aura.Rate = 50 end
        TweenService:Create(datos.Parte, tweenInfoSubir, {CFrame = datos.CF_Arriba}):Play()
    end

    -- Mantener la escena en pantalla
    task.wait(6)

    -- Animación de regreso al inframundo (Baja el Jefe)
    ...
    -- Bajan los 5 Demonios de la manada de regreso a la tierra
    for _, datos in ipairs(DataEspectros) do
        TweenService:Create(datos.Parte, tweenInfoBajar, {CFrame = datos.CF_Abajo}):Play()
    end
...
Tab:CreateLabel("Demonios Invisibles Fixeados 😈")
