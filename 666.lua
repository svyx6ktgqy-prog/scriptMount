...
local ASSETS = {
    ...
    Satan = 1221768461,       -- Malla del Demonio Central y de la Manada
...
-- // MÉTODO QUIRÚRGICO: Crear directamente un Mesh de Satán (sin clonar al jugador)
local function GenerarDemonioFantasma(targetCFrame, parentFolder)
    local espectro = Instance.new("Part")
    espectro.Size = Vector3.new(6, 8, 6)
    espectro.Anchored = true
    espectro.CanCollide = false
    espectro.Color = Color3.fromRGB(20, 0, 0)
    espectro.CFrame = targetCFrame
    espectro.Transparency = 1 -- Inicia invisible (Ghost Style)
    espectro.Name = "EspectroSatan"
    espectro.Parent = parentFolder
    
    local mesh = Instance.new("SpecialMesh")
    mesh.MeshType = Enum.MeshType.FileMesh
    mesh.MeshId = "rbxassetid://" .. tostring(ASSETS.Satan)
    mesh.Scale = Vector3.new(0.12, 0.12, 0.12) 
    mesh.VertexColor = Vector3.new(1.5, 0.1, 0.1) 
    mesh.Parent = espectro
    ...
    auraOscura.Parent = espectro
    
    return espectro, auraOscura
end
...
local function CrearEscenaEpica()
    ...
    -- 3. Entidad Demoníaca visible (EL SATÁN CENTRAL ARRIBA DEL CUBO)
    ...
    -- // 5. CREAR LA MANADA CIRCULAR (LOS 5 SATANES ESPECTRALES)
    ...
    for i = 1, 5 do
        ...
        -- Los espectros ya se posicionan en su lugar final mirando al centro
        local posEspectro = spawnPos + Vector3.new(offsetX, 5, offsetZ) -- Elevados para que coincidan con la altura del original
        local lookCFrame = CFrame.lookAt(posEspectro, spawnPos + Vector3.new(0, 5, 0))
        
        local modeloNPC, aura = GenerarDemonioFantasma(lookCFrame, SceneFolder)
        table.insert(TodasLasPartesEspectros, modeloNPC)
        if aura then table.insert(EmisoresAuraEspectros, aura) end
    end

    -- // ANIMACIONES (Demonio Central sube, Manada se desvanece)
    ...
    -- Invocación del Demonio Central (Él sí sube desde el inframundo)
    ...
    -- Aparición fantasmal de la manada de Satanes (Fade In)
    for _, aura in ipairs(EmisoresAuraEspectros) do
        aura.Rate = 50 
    end

    for _, espectro in ipairs(TodasLasPartesEspectros) do
        -- Al ser una sola parte con un SpecialMesh, solo tweeneamos la transparencia de la parte
        TweenService:Create(espectro, tweenInfoFadeIn, {Transparency = 0.1}):Play()
    end

    -- Mantener la escena en pantalla
    task.wait(6)

    -- Animación de regreso al inframundo (Demonio Central baja)
    ...
    -- Desaparecen los Satanes circulares (Fade Out)
    for _, espectro in ipairs(TodasLasPartesEspectros) do
        TweenService:Create(espectro, tweenInfoFadeIn, {Transparency = 1}):Play()
    end
...
Tab:CreateLabel("Ritual Quirúrgico: Múltiples Demonios Perfectos")
