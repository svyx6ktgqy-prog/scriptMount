...
local ASSETS = {
    ...
    SkinManada = 92539602189320 -- NUEVO: ID para los clones de la manada
}
...
-- // NUEVA Función: Clonar tu personaje, aplicar la skin y prepararlo como Fantasma
local function GenerarPersonaje(targetCFrame, parentFolder)
    ...
    if hum then
        ...
        -- [REPARACIÓN QUIRÚRGICA]: Desactivamos ApplyDescription porque decapita el clon.
        -- Dejamos que tu avatar mantenga su cabeza intacta.
        -- if ManadaDesc then
    ...
    -- [NUEVO]: Aplicar el Skin de Satán cuidadosamente POR ENCIMA del clon
    if hrp then
        local SatanOverlay = Instance.new("Part")
        SatanOverlay.Name = "SatanSkinOverlay"
        SatanOverlay.Size = Vector3.new(6, 8, 6)
        SatanOverlay.CFrame = targetCFrame -- Se posiciona exactamente sobre tu clon
        SatanOverlay.Anchored = true
        SatanOverlay.CanCollide = false
        SatanOverlay.Color = Color3.fromRGB(20, 0, 0)
        SatanOverlay.Transparency = 1 -- Inicia invisible para que las animaciones de desvanecimiento lo controlen
...
local function CrearEscenaEpica()
    ...
    -- 3. Entidad Demoníaca visible (INTACTO)
    ...
    for i = 1, 5 do
        ...
        -- Los espectros ya se posicionan en su lugar final (sin bajar ni subir)
    ...
    -- // ANIMACIONES (Demonio sube, Espectros se desvanecen)
    ...
    -- Invocación del Demonio (Él sí sube desde el inframundo)
    ...
    -- Aparición fantasmal de la manada (Fade In Ghost Style)
    ...
    for _, npc in ipairs(TodasLasPartesEspectros) do
        for _, obj in ipairs(npc:GetDescendants()) do
            -- Anima la ropa, partes y caras hacia una transparencia fantasmal (0.1)
            if (obj:IsA("BasePart") and obj.Name ~= "HumanoidRootPart") or obj:IsA("Decal") then
    ...
    -- Animación de regreso al inframundo (Demonio baja) y desvanecimiento de la manada (Fade Out)
    ...
    -- Desaparecen estilo ghost (Transparencia a 1)
    for _, npc in ipairs(TodasLasPartesEspectros) do
        for _, obj in ipairs(npc:GetDescendants()) do
            if (obj:IsA("BasePart") and obj.Name ~= "HumanoidRootPart") or obj:IsA("Decal") then
...
Tab:CreateLabel("Script Final: Ritual Intacto + Manada Fantasma NPC")
