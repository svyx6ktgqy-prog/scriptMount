-- ==========================================================
-- MENU DE AVATARES QUIRÚRGICO Y PRO (v25.0 CLIP & ANIMATED PREVIEW FIX)
-- Miniatura Animada x Outfit + Pose + Ajustes Personalizados Guardados
-- Sidebar Clipping Mask Correcto
-- ==========================================================
...
local function RefreshSavedCharactersGrid()
    ...
    for charName, itemEntries in pairs(data) do
        ...
        task.spawn(function()
            ...
            -- Restaurar transparencia base de extremidades en el Dummy
            ...
            -- Para animaciones activas en ViewportFrame, los parts NO deben estar anclados (Anchored = false)
            ...
            if hrp then hrp.Anchored = true end -- Solo el RootPart anclado para fijar la posición
...
-- FIX CLIPPING DESPLAZADOR DE CATEGORÍA:
-- Mantenemos un marco contenedor estático curvo para recortar los bordes redondos del scroll
...
KittySidebar.ClipsDescendants = false -- Permite que los elementos pasen, pero el marco padre (SidebarContainer) recortará con curvatura
...
local Window = Rayfield:CreateWindow({
   Name = "🏥 Avatar Catalog Quirúrgico Pro v25.0",
   LoadingTitle = "Aplicando Layout & ZIndex Fix...",
