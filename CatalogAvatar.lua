...
-- Función Auxiliar de URL Encoding segura para ejecutores
local function SafeUrlEncode(str)
    if not str then return "" end
    local success, result = pcall(function() return HttpService:UrlEncode(str) end)
...
-- Lógica de Búsqueda Reparada (Fix URL Encoding y Limpieza)
local function PerformKittySearch(isPagination)
    ...
    -- Limpiar resultados anteriores para evitar mezcla visual
    ...
    task.spawn(function()
        local cursorParam = ""
        if KittyCurrentCursor ~= "" then
            cursorParam = "&cursor=" .. SafeUrlEncode(KittyCurrentCursor)
        end
