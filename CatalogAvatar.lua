...
local BannerSystem = {
    ...
    LoadingQueue = {},
    IsProcessing = false,
    BatchSizePerFrame = 2, -- Límite estricto de instanciaciones por frame para evitar picos de CPU
}
...
function BannerSystem.ProcessQueue()
    ...
    task.spawn(function()
        while #BannerSystem.LoadingQueue > 0 do
            ...
            -- Cede el paso al motor para mantener 60 FPS estables
            task.wait()
        end
            ...
            for charName, itemEntries in pairs(data) do
                ...
                task.spawn(function()
                    ...
                    for _, entry in ipairs(itemEntries) do
                        ...
                        task.wait() -- Pausa de 1 frame entre la carga de assets pesados del outfit
                    end
                ...
                task.wait() -- Frame-slicing entre cards
            end
...
-- ==========================================================
-- SISTEMA KITTY CATALOG UI (AHORA CON PAGINACIÓN Y LIMIT 120)
-- ==========================================================
...
-- ==== CONTENEDOR DE PAGINACIÓN ADICIONAL ====
local PaginationFrame = Instance.new("Frame")
PaginationFrame.Size = UDim2.new(0.75, 0, 0, 40)
PaginationFrame.Position = UDim2.new(0.25, 0, 1, -40)
PaginationFrame.BackgroundColor3 = Color3.fromRGB(255, 182, 193)
PaginationFrame.BackgroundTransparency = 0.5
PaginationFrame.BorderSizePixel = 0
PaginationFrame.Parent = KittyMain

local PrevBtn = Instance.new("TextButton")
PrevBtn.Size = UDim2.new(0, 80, 0, 30)
PrevBtn.Position = UDim2.new(0, 20, 0.5, -15)
PrevBtn.BackgroundColor3 = Color3.fromRGB(255, 105, 180)
PrevBtn.Text = "◀ Atrás"
PrevBtn.Font = Enum.Font.GothamBold
PrevBtn.TextColor3 = Color3.fromRGB(255, 255, 255)
PrevBtn.Visible = false
PrevBtn.Parent = PaginationFrame
local PrevCorner = Instance.new("UICorner"); PrevCorner.CornerRadius = UDim.new(0, 6); PrevCorner.Parent = PrevBtn

local NextBtn = Instance.new("TextButton")
NextBtn.Size = UDim2.new(0, 90, 0, 30)
NextBtn.Position = UDim2.new(1, -110, 0.5, -15)
NextBtn.BackgroundColor3 = Color3.fromRGB(255, 105, 180)
NextBtn.Text = "Siguiente ▶"
NextBtn.Font = Enum.Font.GothamBold
NextBtn.TextColor3 = Color3.fromRGB(255, 255, 255)
NextBtn.Visible = false
NextBtn.Parent = PaginationFrame
local NextCorner = Instance.new("UICorner"); NextCorner.CornerRadius = UDim.new(0, 6); NextCorner.Parent = NextBtn

local PageLabel = Instance.new("TextLabel")
PageLabel.Size = UDim2.new(0, 100, 0, 30)
PageLabel.Position = UDim2.new(0.5, -50, 0.5, -15)
PageLabel.BackgroundTransparency = 1
PageLabel.Text = "Página 1"
PageLabel.Font = Enum.Font.GothamBold
PageLabel.TextColor3 = Color3.fromRGB(50, 50, 50)
PageLabel.Parent = PaginationFrame
-- ============================================

local KittyResults = Instance.new("ScrollingFrame")
-- Reducido 40 pixeles extra (-100 en lugar de -60) para dejar espacio a la barra de paginación inferior
...
local KittyCurrentCursor = ""
local KittyCursorHistory = {}
local KittyCurrentPage = 1
local NextCursorData = ""
...
-- Lógica de Búsqueda Adaptada para Paginación Limpia (limit = 120 nativo por llamada)
local function PerformKittySearch(isPagination)
    if KittySearch.Text == "" and KittyCurrentCategory == 1 then return end
    
    -- Si es una búsqueda nueva, resetear el historial de cursores
    ...
    PageLabel.Text = "Cargando..."
    PrevBtn.Visible = false
    NextBtn.Visible = false

    task.spawn(function()
        local cursorParam = KittyCurrentCursor ~= "" and ("&cursor=" .. KittyCurrentCursor) or ""
        local url = "https://catalog.roblox.com/v1/search/items/details?category="..tostring(KittyCurrentCategory).."&limit=120&keyword=" .. HttpService:UrlEncode(KittySearch.Text) .. cursorParam
        
        local success, response = pcall(function() return game:HttpGet(url) end)
        if not success or not response then
            url = "https://catalog.roproxy.com/v1/search/items/details?category="..tostring(KittyCurrentCategory).."&limit=120&keyword=" .. HttpService:UrlEncode(KittySearch.Text) .. cursorParam
        ...
        if success and response then
            ...
            if decoded and decoded.data then
                for _, item in ipairs(decoded.data) do
                    -- Generación directa de la UI de la tarjeta
                    local Card = Instance.new("Frame")
                    Card.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
                    ...
                    -- Frame-slicing opcional interno para 120 elementos
                    task.wait(0.002) 
                end
                
                PageLabel.Text = "Página " .. tostring(KittyCurrentPage)
                
                if decoded.nextPageCursor and decoded.nextPageCursor ~= "" then
                    NextBtn.Visible = true
                    NextCursorData = decoded.nextPageCursor
                else
                    NextBtn.Visible = false
                    NextCursorData = ""
                end
                
                if #KittyCursorHistory > 0 then
                    PrevBtn.Visible = true
                else
                    PrevBtn.Visible = false
                end
            else
                PageLabel.Text = "Sin resultados"
            end
        else
            PageLabel.Text = "Error al cargar"
        end
    end)
end

NextBtn.MouseButton1Click:Connect(function()
    if NextCursorData ~= "" then
        table.insert(KittyCursorHistory, KittyCurrentCursor)
        KittyCurrentCursor = NextCursorData
...
PrevBtn.MouseButton1Click:Connect(function()
    if #KittyCursorHistory > 0 then
        KittyCurrentCursor = table.remove(KittyCursorHistory, #KittyCursorHistory)
        KittyCurrentPage = KittyCurrentPage - 1
        PerformKittySearch(true)
    end
end)
