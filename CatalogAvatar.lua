...
-- Método Home aislado y modificado para cargar items ocultos/raros sin parámetros de paginación
local function PerformHomeSearch()
    for _, child in ipairs(KittyResults:GetChildren()) do
        if child:IsA("Frame") then child:Destroy() end
    end
    
    PagiLabel.Text = "Items Ocultos..."
    PagiPrevBtn.Visible = false
    PagiNextBtn.Visible = false
    
    -- Reiniciamos las variables de control por seguridad
    KittyCurrentPage = 1
    KittyPageHistory = {""}
    KittyNextCursor = nil

    -- URL sin parámetros limit, cursor o page para acceder a la sección oculta
    local url = "https://catalog.roblox.com/v1/search/items/details?category="..tostring(KittyCurrentCategory)
    if KittySearch.Text ~= "" then
        url = url .. "&keyword=" .. HttpService:UrlEncode(KittySearch.Text)
    end
