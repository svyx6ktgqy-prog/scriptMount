-- ==========================================================
-- MENU DE AVATARES QUIRÚRGICO Y PRO (v23.7 PERSISTENCIA & SCROLL FIX)
-- Miniatura Estática x Outfit + Pose Guardada + Guardado de Ajustes
-- ==========================================================
...
-- Aplica ajustes de transformación guardados previamente a un accesorio activo (con protección C1/Scale original)
local function ApplySavedTransformations(item, adjData)
    if not item or not item:IsA("Accessory") or not adjData then return end
    local handle = item:FindFirstChild("Handle")
    if not handle then return end
    local weld = handle:FindFirstChild("AccessoryWeld")
    if not weld then return end

    local origC1 = weld:GetAttribute("OriginalC1") or weld.C1
    if not weld:GetAttribute("OriginalC1") then
        weld:SetAttribute("OriginalC1", origC1)
    end

    local mesh = handle:FindFirstChildWhichIsA("DataModelMesh")
    local origScale = mesh and (mesh:GetAttribute("OriginalScale") or mesh.Scale)
    if mesh and not mesh:GetAttribute("OriginalScale") then
        mesh:SetAttribute("OriginalScale", origScale)
    end

    if not ItemAdjustments[item] then
        ItemAdjustments[item] = {
            OriginalWeldC1 = origC1,
            OrigMeshScale = origScale,
        }
    end

    local data = ItemAdjustments[item]
    data.OffsetX = adjData.OffsetX or 0
    data.OffsetY = adjData.OffsetY or 0
    data.OffsetZ = adjData.OffsetZ or 0
    data.RotX = adjData.RotX or 0
    data.RotY = adjData.RotY or 0
    data.RotZ = adjData.RotZ or 0
    data.Scale = adjData.Scale or 1

    weld.C1 = origC1 
        * CFrame.new(-data.OffsetX, -data.OffsetY, -data.OffsetZ) 
        * CFrame.Angles(math.rad(data.RotX), math.rad(data.RotY), math.rad(data.RotZ))

    if mesh and origScale then
        mesh.Scale = origScale * data.Scale
    end
end
                ...
                if handle then
                    ...
                    if accAttach then
                        ...
                        if targetAttach then
                            ...
                            weld:SetAttribute("OriginalC1", accAttach.CFrame)
                            weld.Parent = handle
                        end
                    else
                        ...
                        if bodyTarget then
                            ...
                            weld.C0 = CFrame.new()
                            weld.C1 = CFrame.new()
                            weld:SetAttribute("OriginalC1", CFrame.new())
                            weld.Parent = handle
                        end
                    end
...
EqScroll.ElasticBehavior = Enum.ElasticBehavior.Always
EqScroll.ClipsDescendants = true
EqScroll.Parent = EqPanel

local EqPadding = Instance.new("UIPadding")
EqPadding.PaddingTop = UDim.new(0, 10)
EqPadding.PaddingBottom = UDim.new(0, 15)
EqPadding.PaddingLeft = UDim.new(0, 10)
EqPadding.PaddingRight = UDim.new(0, 10)
EqPadding.Parent = EqScroll
...
CharMenu.Size = UDim2.new(0, 460, 0, 420)
...
CharMenuConstraint.MaxSize = Vector2.new(500, 450)
CharMenuConstraint.MinSize = Vector2.new(320, 350)
...
CharTitle.Size = UDim2.new(1, -50, 0, 45)
CharTitle.Position = UDim2.new(0, 15, 0, 0)
...
CharCloseBtn.Size = UDim2.new(0, 28, 0, 28)
CharCloseBtn.Position = UDim2.new(1, -40, 0, 10)
...
NameInput.Size = UDim2.new(1, -155, 0, 36)
NameInput.Position = UDim2.new(0, 15, 0, 50)
...
local NamePadding = Instance.new("UIPadding"); NamePadding.PaddingLeft = UDim.new(0, 10); NamePadding.Parent = NameInput
local NameCorner = Instance.new("UICorner"); NameCorner.CornerRadius = UDim.new(0, 8); NameCorner.Parent = NameInput

local SaveCharBtn = Instance.new("TextButton")
SaveCharBtn.Size = UDim2.new(0, 110, 0, 36)
SaveCharBtn.Position = UDim2.new(1, -125, 0, 50)
...
SaveCharBtn.TextSize = 11
SaveCharBtn.Parent = CharMenu
local SaveCorner = Instance.new("UICorner"); SaveCorner.CornerRadius = UDim.new(0, 8); SaveCorner.Parent = SaveCharBtn

local CharScroll = Instance.new("ScrollingFrame")
CharScroll.Size = UDim2.new(1, -30, 1, -115)
CharScroll.Position = UDim2.new(0, 15, 0, 100)
...
local function RefreshSavedCharactersGrid()
    ...
    for charName, charData in pairs(data) do
        local itemIds = type(charData) == "table" and (charData.Items or charData) or {}
        local savedAdjustments = type(charData) == "table" and charData.Adjustments or {}
        ...
        task.spawn(function()
            ...
            local function ProcessDummyItem(item, itemIdStr)
                if not item then return end

                if item:IsA("Accessory") then
                    local cloneItem = item:Clone()
                    cloneItem:SetAttribute("AssetId", tonumber(itemIdStr))
                    local handle = cloneItem:FindFirstChild("Handle")
                    if handle then
                        handle.Transparency = 0
                        handle.Anchored = false
                        ...
                        -- Aplicar transformaciones guardadas directamente a la miniatura
                        if savedAdjustments[itemIdStr] then
                            local adj = savedAdjustments[itemIdStr]
                            local weld = handle:FindFirstChild("AccessoryWeld")
                            if weld then
                                local origC1 = weld:GetAttribute("OriginalC1") or weld.C1
                                weld.C1 = origC1 
                            ...
                            if mesh then
                                local origScale = mesh:GetAttribute("OriginalScale") or mesh.Scale
                                mesh.Scale = origScale * (adj.Scale or 1)
                            end
                        end
                    end
                ...
                elseif item:IsA("Model") or item:IsA("Folder") then
                    for _, sub in ipairs(item:GetChildren()) do
                        ProcessDummyItem(sub, itemIdStr)
                    end
                else
                    item:Clone().Parent = dummy
                end
            ...
            for _, id in ipairs(itemIds) do
                ...
                local success, objects = pcall(function() return game:HttpGet("rbxassetid://" .. idStr) end)
                local realObjects = nil
                pcall(function() realObjects = game:GetObjects("rbxassetid://" .. idStr) end)
                
                local processedAsObject = false
                if realObjects and #realObjects > 0 then
                    for _, obj in ipairs(realObjects) do
                        processedAsObject = true
                        ProcessDummyItem(obj, idStr)
                    end
                end
                ...
                if char then
                    for _, v in ipairs(char:GetChildren()) do
                        if v:IsA("Accessory") then
                            local eqId = v:GetAttribute("AssetId") or v.SourceAssetId
                            if eqId and savedAdjustments[tostring(eqId)] then
                                ApplySavedTransformations(v, savedAdjustments[tostring(eqId)])
                            end
                        end
                    end
                end
...
SaveCharBtn.MouseButton1Click:Connect(function()
    ...
    local adjustmentsToSave = {}
    local char = LocalPlayer.Character
    if char then
        for _, v in ipairs(char:GetChildren()) do
            if v:IsA("Accessory") and ItemAdjustments[v] then
                local eqId = v:GetAttribute("AssetId") or v.SourceAssetId
                if eqId then
                    local adj = ItemAdjustments[v]
                    adjustmentsToSave[tostring(eqId)] = {
                        OffsetX = adj.OffsetX,
                        OffsetY = adj.OffsetY,
                        OffsetZ = adj.OffsetZ,
                        RotX = adj.RotX,
...
local function ApplyTransformations()
    ...
    if handle then
        local weld = handle:FindFirstChild("AccessoryWeld")
        local origC1 = weld and (weld:GetAttribute("OriginalC1") or data.OriginalWeldC1)
        if weld and origC1 then
            weld.C1 = origC1 
                * CFrame.new(-EditOffsetX, -EditOffsetY, -EditOffsetZ) 
                * CFrame.Angles(math.rad(EditRotX), math.rad(EditRotY), math.rad(EditRotZ))
        end
        local mesh = handle:FindFirstChildWhichIsA("DataModelMesh")
        local origScale = mesh and (mesh:GetAttribute("OriginalScale") or data.OrigMeshScale)
        if mesh and origScale then
            mesh.Scale = origScale * EditScale
        end
    end
end
...
KittySidebar.ElasticBehavior = Enum.ElasticBehavior.Always
...
KittyResults.ElasticBehavior = Enum.ElasticBehavior.Always
...
local Window = Rayfield:CreateWindow({
   Name = "🏥 Avatar Catalog Quirúrgico Pro v23.7",
   LoadingTitle = "Aplicando Persistencia & UI Fix...",
