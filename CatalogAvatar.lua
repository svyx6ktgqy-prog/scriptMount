-- ==========================================================
-- MENU DE AVATARES QUIRÚRGICO Y PRO (UNIFICADO v10.0 OMNI-COMPATIBLE)
-- Compatibilidad 1000% (Accesorios, Layered Clothing 3D, Gears, Maquillaje)
-- Categorías Extraídas Directamente de las Imágenes
-- ==========================================================
...
-- ==========================================================
-- SISTEMA DE EQUIPAMIENTO OMNI-COMPATIBLE (EL CEREBRO 3D/2D)
-- ==========================================================
local function UniversalEquip(assetId)
    task.spawn(function()
        ...
        local success, err = pcall(function()
            -- Obtenemos el objeto crudo. Puede ser Accessory, Model, Folder, Decal o Tool.
            local objects = game:GetObjects("rbxassetid://" .. tostring(assetId))
            
            -- Función interna para procesar cualquier item sin importar si está escondido en un Folder/Model
            local function ProcessItem(item)
                if not item then return end

                if item:IsA("Accessory") then
                    -- ESTO ATRAPA: Gorros, Alas, Cinturas, Zapatos 3D, Suéteres 3D, Vestidos, Cejas, Pestañas 3D.
                    -- AddAccessory respeta el WrapLayer y los Attachments automáticamente.
                    Hum:AddAccessory(item:Clone())

                elseif item:IsA("Shirt") or item:IsA("Pants") or item:IsA("ShirtGraphic") then
                    -- ESTO ATRAPA: Ropa 2D Clásica (Classic Shirts, T-Shirts, Classic Pants)
                ...
                elseif item:IsA("Decal") then
                    -- ESTO ATRAPA: Maquillaje 2D y Caras Clásicas
                ...
                elseif item:IsA("Tool") or item:IsA("HopperBin") then
                    -- ESTO ATRAPA: Gears / Equipamiento (Espadas, Pociones, etc.)
                ...
                elseif item:IsA("Model") or item:IsA("Folder") then
                    -- ESTO ATRAPA: Bundles o ropa 3D mal empaquetada por Roblox.
                    -- Desempaqueta y busca los accesorios dentro.
            ...
            -- Iniciamos el procesamiento para todos los objetos devueltos por el ID
        ...
        if not success then
            warn("Fallo al aplicar el item (Tu ejecutor podría bloquear GetObjects): " .. tostring(err))
        end
    end)
end
...
-- ==========================================================
-- DICCIONARIO DE CATEGORÍAS 1000% COMPATIBLE (SEGÚN IMÁGENES)
-- Estructura: {Nombre Botón, ID Categoría, ID Subcategoría}
-- ==========================================================
...
for i, catData in ipairs(CategoriesDetailed) do
...
local function PerformKittySearch()
    ...
    local url = "https://catalog.roblox.com/v1/search/items/details?category="..tostring(KittyCurrentCategory)..subParam.."&limit=30&keyword=" .. HttpService:UrlEncode(KittySearch.Text)
    
    local success, response = pcall(function() return game:HttpGet(url) end)
    
    if not success or not response then
        url = "https://catalog.roproxy.com/v1/search/items/details?category="..tostring(KittyCurrentCategory)..subParam.."&limit=30&keyword=" .. HttpService:UrlEncode(KittySearch.Text)
...
-- ==========================================================
-- INTERFAZ RAYFIELD CLÁSICA
-- ==========================================================
local AssetTypeNames = {
    [2] = "T-Shirt", [5] = "Script", [8] = "Sombrero", [9] = "Place", [10] = "Modelo", 
    [11] = "Camisa", [12] = "Pantalón", [13] = "Decal", [17] = "Cabeza", [18] = "Cara", [19] = "Gear", 
    [24] = "Animación", [27] = "Torso", [38] = "Plugin", [41] = "Pelo", 
...
local Window = Rayfield:CreateWindow({
   Name = "🏥 Avatar Catalog Quirúrgico Pro v10.0",
   LoadingTitle = "Motor OMNI-COMPATIBLE 3D...",
   LoadingSubtitle = "Maquillaje, Layered Clothes, Gears & Accesorios",
...
Panel:CreateToggle({
   Name = "🎀 Activar Menú Kitty (Visual UI)",
   CurrentValue = false,
   ...
   Callback = function(Value)
       ...
       if Value then
           KittyMain.Visible = true
           FloatingBtn.Visible = false
           Rayfield:Notify({Title = "🎀 Menú Kitty", Content = "¡El Catálogo Avanzado está abierto!", Duration = 3})
       end
   end,
})

Panel:CreateSection("🔍 Búsqueda Directa (Clásica)")
...
Panel:CreateInput({
   ...
   PlaceholderText = "Ej: Dominus, 3D Jacket, Eyelashes...",
...
Panel:CreateSection("Búsqueda por ID Directa")
...
Panel:CreateButton({
   ...
   Callback = function()
       ...
       Rayfield:Notify({Title = "Transformación 3D/2D", Content = "Enviando: " .. CurrentData.Name, Duration = 3})
   end,
})
