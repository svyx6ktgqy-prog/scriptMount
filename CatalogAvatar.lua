...
-- Incluye: Integración de Compras Servidor/Cliente & Límite Dinámico
-- ==========================================================
...
-- ==========================================================
-- SISTEMA DE COMPRAS (SERVIDOR / CLIENTE REMOTE)
-- ==========================================================
pcall(function()
    -- Intento de creación de RemoteEvent en ReplicatedStorage
    -- (Nota: Si se ejecuta puramente como LocalScript en un executor, puede que esto 
    -- solo se refleje localmente, pero el código está estructurado para soportarlo 
    -- en un entorno de juego real o híbrido).
    if not ReplicatedStorage:FindFirstChild("PromptPurchaseRemote") then
...
MarketplaceService.PromptPurchaseFinished:Connect(function(player, assetId, isPurchased)
    if isPurchased then
        print("Compró el asset exitosamente:", assetId)
    end
end)
...
task.spawn(function()
    ...
    local assetsToPreload = {
        ...
        "rbxassetid://132859920937628" -- Home Button Icon
    }
    pcall(function()
...
-- ==========================================================
-- BOTÓN HOME (BÚSQUEDA AISLADA) Y ADAPTACIÓN DEL MENÚ SUPERIOR
-- ==========================================================
local HomeBtn = Instance.new("ImageButton")
HomeBtn.Size = UDim2.new(0, 40, 0, 40)
HomeBtn.Position = UDim2.new(0, 10, 0.5, -20)
HomeBtn.Image = "rbxassetid://132859920937628"
HomeBtn.BackgroundTransparency = 1
HomeBtn.Parent = KittyTop

local SearchContainer = Instance.new("Frame")
SearchContainer.Size = UDim2.new(0.55, 0, 0, 40)
SearchContainer.Position = UDim2.new(0, 60, 0.5, -20)
...
KittySearchBtn.Position = UDim2.new(0.55, 70, 0.5, -20)
...
-- ESPACIO ACHICADO PARA LA PAGINACIÓN (-100 en vez de -60)
...
-- ==========================================================
-- CONTENEDOR DE PAGINACIÓN QUIRÚRGICA
-- ==========================================================
...
-- ==========================================================
-- SISTEMA DE CONFIGURACIÓN DE LÍMITE MAX. ELEMENTOS
-- ==========================================================
local MaxItemsFrame = Instance.new("Frame")
MaxItemsFrame.Size = UDim2.new(1, 0, 0, 95)
MaxItemsFrame.BackgroundColor3 = Color3.fromRGB(255, 228, 225)
MaxItemsFrame.BackgroundTransparency = 0.5
MaxItemsFrame.Parent = KittySidebar
local MaxItemsFrameCorner = Instance.new("UICorner"); MaxItemsFrameCorner.CornerRadius = UDim.new(0, 6); MaxItemsFrameCorner.Parent = MaxItemsFrame

local MaxLabel = Instance.new("TextLabel")
MaxLabel.Size = UDim2.new(1, -45, 0, 25)
MaxLabel.Position = UDim2.new(0, 5, 0, 5)
MaxLabel.BackgroundTransparency = 1
MaxLabel.Text = "Elementos: 30"
MaxLabel.Font = Enum.Font.GothamBold
MaxLabel.TextColor3 = Color3.fromRGB(80, 80, 80)
MaxLabel.TextSize = 12
MaxLabel.TextXAlignment = Enum.TextXAlignment.Left
MaxLabel.Parent = MaxItemsFrame

local MaxInput = Instance.new("TextBox")
MaxInput.Size = UDim2.new(0, 35, 0, 20)
MaxInput.Position = UDim2.new(1, -40, 0, 7)
MaxInput.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
MaxInput.Text = "30"
MaxInput.Font = Enum.Font.GothamBold
MaxInput.TextColor3 = Color3.fromRGB(50, 50, 50)
MaxInput.TextSize = 11
MaxInput.Parent = MaxItemsFrame
local MaxInputCorner = Instance.new("UICorner"); MaxInputCorner.CornerRadius = UDim.new(0, 4); MaxInputCorner.Parent = MaxInput

local MaxBg = Instance.new("Frame")
MaxBg.Size = UDim2.new(1, -20, 0, 8)
MaxBg.Position = UDim2.new(0, 10, 0, 35)
MaxBg.BackgroundColor3 = Color3.fromRGB(200, 180, 180)
MaxBg.Parent = MaxItemsFrame
local MaxBgCorner = Instance.new("UICorner"); MaxBgCorner.Parent = MaxBg

local MaxFill = Instance.new("Frame")
MaxFill.Size = UDim2.new(30/120, 0, 1, 0)
MaxFill.BackgroundColor3 = Color3.fromRGB(255, 20, 147)
MaxFill.Parent = MaxBg
local MaxFillCorner = Instance.new("UICorner"); MaxFillCorner.Parent = MaxFill

local MaxKnob = Instance.new("TextButton")
MaxKnob.Size = UDim2.new(0, 16, 0, 16)
MaxKnob.Position = UDim2.new(1, -8, 0.5, -8)
MaxKnob.BackgroundColor3 = Color3.fromRGB(255, 105, 180)
MaxKnob.Text = ""
MaxKnob.Parent = MaxFill
local MaxKnobCorner = Instance.new("UICorner"); MaxKnobCorner.CornerRadius = UDim.new(1, 0); MaxKnobCorner.Parent = MaxKnob

local MaxApplyBtn = Instance.new("TextButton")
MaxApplyBtn.Size = UDim2.new(1, -20, 0, 25)
MaxApplyBtn.Position = UDim2.new(0, 10, 0, 60)
MaxApplyBtn.BackgroundColor3 = Color3.fromRGB(255, 105, 180)
MaxApplyBtn.Text = "Apply Cambios"
MaxApplyBtn.Font = Enum.Font.GothamBold
MaxApplyBtn.TextColor3 = Color3.fromRGB(255, 255, 255)
MaxApplyBtn.TextSize = 11
MaxApplyBtn.Parent = MaxItemsFrame
local MaxApplyCorner = Instance.new("UICorner"); MaxApplyCorner.CornerRadius = UDim.new(0, 6); MaxApplyCorner.Parent = MaxApplyBtn

local isDraggingMax = false
MaxKnob.InputBegan:Connect(function(input)
    if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then isDraggingMax = true end
end)
UserInputService.InputEnded:Connect(function(input)
    if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then isDraggingMax = false end
end)

local function UpdateMaxLimit(val)
    val = math.clamp(math.floor(val), 0, 120)
    KittySearchLimit = val
    MaxLabel.Text = "Elementos: " .. val
    MaxInput.Text = tostring(val)
    MaxFill.Size = UDim2.new(val/120, 0, 1, 0)
end

UserInputService.InputChanged:Connect(function(input)
    if isDraggingMax and (input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch) then
        local relativeX = math.clamp(input.Position.X - MaxBg.AbsolutePosition.X, 0, MaxBg.AbsoluteSize.X)
        local percentage = relativeX / MaxBg.AbsoluteSize.X
        UpdateMaxLimit(percentage * 120)
    end
end)

MaxInput.FocusLost:Connect(function()
    local val = tonumber(MaxInput.Text) or 30
    UpdateMaxLimit(val)
end)

-- Protección para evitar Error HTTP 400 (Roblox solo acepta límites de búsqueda específicos)
local function GetValidApiLimit(val)
    if val <= 0 then return 10 end
    local limits = {10, 28, 30, 50, 60, 100, 120}
    local closest = limits[1]
    local minDiff = math.abs(val - limits[1])
    for i = 2, #limits do
        local diff = math.abs(val - limits[i])
...
-- ==========================================================
-- VARIABLES DE ESTADO Y FUNCIÓN DE BÚSQUEDA PAGINADA
-- ==========================================================
...
-- Proceso de la búsqueda con sistema de Long-Press en las tarjetas
local function ProcessSearchResults(decodedData)
        ...
        ClickBtn.InputBegan:Connect(function(input)
            if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
                ...
                task.spawn(function()
                    while isPressing do
                        if tick() - holdStart >= 0.5 then
                            isPressing = false -- Ejecuta Long-Press
                            local assetId = tonumber(item.id)
                            if assetId and assetId > 0 then
                                -- Intenta ejecutar compra en el servidor
                                local PromptPurchaseRemote = ReplicatedStorage:FindFirstChild("PromptPurchaseRemote")
                                if PromptPurchaseRemote then
                                    PromptPurchaseRemote:FireServer(assetId)
                                else
                                    -- Fallback cliente (Seguridad)
        ...
        ClickBtn.InputEnded:Connect(function(input)
            if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
                if isPressing then
                    isPressing = false
                    local timeHeld = tick() - holdStart
                    if timeHeld < 0.5 then -- Fue un Click Corto
...
local function PerformHomeSearch()
    ...
    local apiLimit = tostring(GetValidApiLimit(KittySearchLimit))
    local url = "https://catalog.roblox.com/v1/search/items/details?category="..tostring(KittyCurrentCategory).."&limit="..apiLimit.."&keyword=" .. HttpService:UrlEncode(KittySearch.Text)
    
    local success, response = pcall(function() return game:HttpGet(url) end)
    
    if not success or not response then
        url = "https://catalog.roproxy.com/v1/search/items/details?category="..tostring(KittyCurrentCategory).."&limit="..apiLimit.."&keyword=" .. HttpService:UrlEncode(KittySearch.Text)
    ...
    if success and response then
        ...
        if decoded and decoded.data then
            ProcessSearchResults(decoded.data)
        end
    end
end
...
-- ==========================================================
-- BÚSQUEDA ESTÁNDAR (PAGINADA)
-- ==========================================================
local function PerformKittySearch(isPagination)
    if KittySearch.Text == "" then return end
...
MaxApplyBtn.MouseButton1Click:Connect(function() PerformKittySearch(false) end)
...
PagiNextBtn.MouseButton1Click:Connect(function()
    if KittyNextCursor and KittyNextCursor ~= "" and KittyCurrentPage < KittyMaxPages then
        ...
        if not KittyPageHistory[KittyCurrentPage] then
            KittyPageHistory[KittyCurrentPage] = KittyNextCursor
        end
