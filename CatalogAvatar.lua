...
-- VISUALIZADOR PREVIEW - ACTUALIZADO A COMPRA LONG-PRESS / EQUIPAR TAP NORMAL
    ...
    if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
        ...
        task.spawn(function()
            while isPreviewPressing do
                if tick() - previewHoldStart >= 0.5 then
...
local function PerformHomeSearch()
    ...
    local limit = math.clamp(KittySearchLimit, 0, 120)
...
-- Contenedor Límite Elementos (Slider e Input)
...
MaxItemsFill.Size = UDim2.new(30/120, 0, 1, 0)
...
MaxItemsInputBox.FocusLost:Connect(function()
    local val = tonumber(MaxItemsInputBox.Text) or 30
    val = math.clamp(val, 0, 120)
    MaxItemsInputBox.Text = tostring(val)
    MaxItemsLabel.Text = "Límite: " .. tostring(val)
    KittySearchLimit = val
    MaxItemsFill.Size = UDim2.new(val/120, 0, 1, 0)
end)
