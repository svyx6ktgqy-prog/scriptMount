...
-- Fix UI + Scroll Categories + Native Purchase + HumanoidDescription Equip
-- ==========================================================
...
local function UniversalEquip(assetId)
    task.spawn(function()
        ...
        -- Método 1: HumanoidDescription (Nativo, casi nunca falla en visualización local)
        local successDesc = pcall(function()
            ...
            if s_info and info then
                ...
                if typeId == 2 then desc.GraphicTShirt = assetId
                elseif typeId == 11 then desc.Shirt = assetId
                elseif typeId == 12 then desc.Pants = assetId
                elseif typeId == 18 then desc.Face = assetId
                elseif typeId == 17 then desc.Head = assetId
                elseif typeId == 8 then desc.HatAccessory = desc.HatAccessory == "" and tostring(assetId) or desc.HatAccessory .. "," .. tostring(assetId)
                elseif typeId == 41 then desc.HairAccessory = desc.HairAccessory == "" and tostring(assetId) or desc.HairAccessory .. "," .. tostring(assetId)
                elseif typeId == 42 then desc.FaceAccessory = desc.FaceAccessory == "" and tostring(assetId) or desc.FaceAccessory .. "," .. tostring(assetId)
                elseif typeId == 43 then desc.NeckAccessory = desc.NeckAccessory == "" and tostring(assetId) or desc.NeckAccessory .. "," .. tostring(assetId)
                elseif typeId == 44 then desc.ShoulderAccessory = desc.ShoulderAccessory == "" and tostring(assetId) or desc.ShoulderAccessory .. "," .. tostring(assetId)
                elseif typeId == 45 then desc.FrontAccessory = desc.FrontAccessory == "" and tostring(assetId) or desc.FrontAccessory .. "," .. tostring(assetId)
                elseif typeId == 46 then desc.BackAccessory = desc.BackAccessory == "" and tostring(assetId) or desc.BackAccessory .. "," .. tostring(assetId)
                elseif typeId == 47 then desc.WaistAccessory = desc.WaistAccessory == "" and tostring(assetId) or desc.WaistAccessory .. "," .. tostring(assetId)
                end
                
                Hum:ApplyDescription(desc)
            else
                error("Fallo al obtener información del producto")
            end
        end)

        -- Método 2: Fallback Clásico (GetObjects) por si es un modelo no soportado por HumanoidDescription
...
-- FIX COMPRA: Llamado directo para abrir la ventana nativa de Roblox sin bloqueos
...
-- FIX UI: Botón Cerrar reposicionado más a la izquierda para evitar cortes
...
-- FIX UI: Botón Search más corto para dejar respirar a la "X"
...
-- FIX SCROLL: CanvasDinámico para que nunca se corten las categorías abajo
...
local CategoriesEnglish = {
    ...
    {"Emotes", 12}, {"Bundles", 4} -- Añadido extra por si acaso
}
