...
-- ==========================================================
-- SISTEMA DE PERSONAJES GUARDADOS (CHARACTERS.json) [ACTUALIZADO UI & PREVIEW]
-- ==========================================================
local CharMenu = Instance.new("Frame")
CharMenu.Size = UDim2.new(0.9, 0, 0.8, 0) 
...
NameInput.Size = UDim2.new(1, -145, 0, 36) 
...
CharScroll.Size = UDim2.new(1, -30, 1, -100) 
...
local function RefreshSavedCharactersGrid()
    ...
    for charName, itemIds in pairs(data) do
        ...
        task.spawn(function()
            ...
            -- FUNCIÓN AUXILIAR PARA EQUIPAR COMO UNIVERSAL EQUIP (SOLUCIONA ITEMS INVISIBLES)
            local function ProcessDummyItem(item)
                ...
                elseif item:IsA("MeshPart") then
                    local currentPart = dummy:FindFirstChild(item.Name)
                    if currentPart and currentPart:IsA("MeshPart") then
                        currentPart.MeshId = item.MeshId
                        local surface = item:FindFirstChildWhichIsA("SurfaceAppearance")
                        if surface then
            ...
            -- 4. WorldModel vital para que la Ropa 3D, Físicas y Welds se rendericen en el Viewport
            local VWorld = Instance.new("WorldModel")
            VWorld.Parent = Viewport

            -- Fijar partes estáticas (Congelar en la pose)
