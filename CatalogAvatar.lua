-- ==========================================================
-- MENU DE AVATARES QUIRÚRGICO Y PRO (UNIFICADO v9.5 + KITTY MENU)
-- NUEVOS FIXES: Cálculo manual de Scroll + Safe Equip + Clean Purchase
-- ==========================================================
...
-- ==========================================================
-- SISTEMA DE EQUIPAMIENTO HÍBRIDO (ANTI-BLOQUEO DE EJECUTORES)
-- ==========================================================
local function UniversalEquip(assetId)
    task.spawn(function()
        ...
        if not cleanAssetId then return end

        -- Método 1: HumanoidDescription (Refinado para evitar colapsos asíncronos)
        local successDesc, err = pcall(function()
            local s_info, info = pcall(function() return MarketplaceService:GetProductInfo(cleanAssetId) end)
            ...
            if s_info and info then
                ...
                elseif typeId == 8 then desc.HatAccessory = desc.HatAccessory == "" and tostring(cleanAssetId) or desc.HatAccessory .. "," .. tostring(cleanAssetId)
                elseif typeId == 41 then desc.HairAccessory = desc.HairAccessory == "" and tostring(cleanAssetId) or desc.HairAccessory .. "," .. tostring(cleanAssetId)
                elseif typeId == 42 then desc.FaceAccessory = desc.FaceAccessory == "" and tostring(cleanAssetId) or desc.FaceAccessory .. "," .. tostring(cleanAssetId)
                elseif typeId == 43 then desc.NeckAccessory = desc.NeckAccessory == "" and tostring(cleanAssetId) or desc.NeckAccessory .. "," .. tostring(cleanAssetId)
                elseif typeId == 44 then desc.ShoulderAccessory = desc.ShoulderAccessory == "" and tostring(cleanAssetId) or desc.ShoulderAccessory .. "," .. tostring(cleanAssetId)
                elseif typeId == 45 then desc.FrontAccessory = desc.FrontAccessory == "" and tostring(cleanAssetId) or desc.FrontAccessory .. "," .. tostring(cleanAssetId)
                elseif typeId == 46 then desc.BackAccessory = desc.BackAccessory == "" and tostring(cleanAssetId) or desc.BackAccessory .. "," .. tostring(cleanAssetId)
                elseif typeId == 47 then desc.WaistAccessory = desc.WaistAccessory == "" and tostring(cleanAssetId) or desc.WaistAccessory .. "," .. tostring(cleanAssetId)
                end
        ...
        -- Método 2: Fallback Clásico (GetObjects)
        if not successDesc then
            pcall(function()
                local objects = game:GetObjects("rbxassetid://" .. tostring(cleanAssetId))
                for _, obj in ipairs(objects) do
                    ...
                    elseif obj:IsA("Decal") then
                        ...
                        if head then
                            ...
                            if face then face.Texture = obj.Texture else obj:Clone().Parent = head end
                        end
                    end
                end
            end)
...
-- ==========================================================
-- SISTEMA DE VISUALIZADOR HUD (COMPRA INTELIGENTE)
-- ==========================================================
...
-- FIX COMPRA NUEVO: Sanitizamos el ID y usamos pcall para evitar interrupciones de seguridad
...
-- FIX NUEVO: Actualización matemática del Scroll para la barra lateral
...
-- FIX NUEVO: Actualización matemática del Scroll para los resultados
...
local Window = Rayfield:CreateWindow({
   Name = "🏥 Avatar Catalog Quirúrgico Pro v9.5",
   LoadingTitle = "Motor Unificado v9.5...",
   LoadingSubtitle = "Fix Visuals + Native Purchase + Hybrid Equip",
...
Panel:CreateButton({
   ...
   Callback = function()
       ...
       Rayfield:Notify({Title = "Transformación Aplicada", Content = "Enviando: " .. CurrentData.Name, Duration = 3})
   end,
})
