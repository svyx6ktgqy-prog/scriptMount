-- ==========================================================
-- MENU DE AVATARES QUIRÚRGICO Y PRO (v19 DEFINITIVA)
-- Fix Ropa 3D + Tools Reales + Personajes Completos
-- ==========================================================
    ...
    if char then
        for _, v in ipairs(char:GetDescendants()) do
            if v:IsA("BasePart") and v.Parent:IsA("Accessory") then
                -- No modificamos la transparencia si es Ropa 3D para evitar parpadeos
                if not v:FindFirstChildWhichIsA("WrapLayer") then
...
-- ==========================================================
-- 2. CEREBRO V19: ROPA 3D PERFECTA + TOOLS + BUNDLES
-- ==========================================================
    ...
    local success, err = pcall(function()
        ...
        local function ProcessItem(item)
            ...
            if item:IsA("Accessory") then
                ...
                if handle then
                    -- VERIFICACIÓN VITAL PARA ROPA 3D (Layered Clothing)
                    local wrap = handle:FindFirstChildWhichIsA("WrapLayer")
                    if wrap then
                        -- Si es ropa 3D (Chaquetas, Pantalones, Suéteres), NO tocamos NADA de físicas.
                    ...
                    -- Si NO es ropa 3D, es un accesorio clásico (Alas, Pelos, etc.). Aplicamos físicas.
                    pcall(function()
            ...
            -- SOPORTE PARA PERSONAJES COMPLETOS (Actualización de Extremidades R15)
            ...
            -- SOPORTE TOTAL PARA TOOLS Y GEARS (En mano y Menú Inferior)
            elseif item:IsA("Tool") or item:IsA("HopperBin") or item:IsA("Gear") then
                ...
                if Backpack then 
                    ...
                    -- Forzamos equiparlo en la mano inmediatamente
...
-- ==========================================================
-- 3. SISTEMA DE VISUALIZADOR HUD Y MENÚ DE EQUIPADOS
-- ==========================================================
...
RefreshEquippedItems = function()
    ...
    for _, v in ipairs(char:GetChildren()) do
        if v:IsA("Accessory") or v:IsA("Shirt") or v:IsA("Pants") or v:IsA("ShirtGraphic") or v:IsA("Tool") then
            table.insert(items, v)
        end
    end
    
    -- Agregar Tools del Backpack al panel visual
    local backpack = LocalPlayer:FindFirstChildOfClass("Backpack")
    if backpack then
...
local Window = Rayfield:CreateWindow({
   Name = "🏥 Avatar Catalog Quirúrgico Pro v19",
   LoadingTitle = "Actualizando Soporte Ropa 3D & Tools...",
