    ...
    if success and response then
        ...
        if decoded and decoded.data then
            ...
            for _, item in ipairs(decoded.data) do
                ...
                -- REPARACIÓN DEL CÓDIGO CORTADO ORIGINAL Y CIERRE DEL LOOP:
                local CardPrice = Instance.new("TextLabel")
                CardPrice.Size = UDim2.new(1, -25, 0, 20)
                CardPrice.Position = UDim2.new(0, 25, 0, 155)
                CardPrice.BackgroundTransparency = 1
                CardPrice.Text = (item.price and item.price > 0) and (tostring(item.price) .. " R$") or "Gratis"
                CardPrice.Font = Enum.Font.GothamBold
                CardPrice.TextSize = 13
                CardPrice.TextColor3 = Color3.fromRGB(50, 50, 50)
                CardPrice.TextXAlignment = Enum.TextXAlignment.Left
...
-- ==========================================================
-- 🟢 NUEVAS OPCIONES PARA EL MENÚ RAYFIELD SOLICITADAS 🟢
-- ¡IMPORTANTE!: Reemplaza la variable 'Tab' por el nombre 
-- exacto de la pestaña que utilices en tu librería de Rayfield.
-- ==========================================================

Tab:CreateSection("⚡ Optimización y Rendimiento")

-- 1. Botón limpiador avanzado de RAM (Delta iPhone Safe)
Tab:CreateButton({
    Name = "🧹 Limpiar RAM y Caché (Optimizar Juego)",
    Callback = function()
        pcall(function()
            -- Recolector de basura en memoria sin generar errores callback
            collectgarbage("collect")
            
            -- Optimización visual ligera en el workspace
            ...
            -- Apagar efectos de sombras globales
            local lighting = game:GetService("Lighting")
            lighting.GlobalShadows = false
            
            if NotifyUser then
                NotifyUser("Rendimiento", "RAM limpiada y optimización aplicada para Delta.")
            end
        end)
    end,
})

Tab:CreateSection("👁️ UI y Visualización")

-- 2. Botón ocultar-mostrar visualizador de items
Tab:CreateToggle({
    Name = "Ocultar/Mostrar Visualizador UI",
    CurrentValue = true,
    Flag = "VisualizerToggle",
    Callback = function(Value)
        pcall(function()
            local visualizer = CoreGui:FindFirstChild("QuirurgicoVisualizer")
            if visualizer then
...
Tab:CreateSection("📱 Orientación de Pantalla")

-- 3. Botones para Orientación (Vertical y Horizontal)
Tab:CreateButton({
...
Tab:CreateSection("🔍 Búsqueda Directa")

-- 4. Buscar por ID también
Tab:CreateInput({
    Name = "Equipar Directamente por ID",
    PlaceholderText = "Escribe el ID aquí...",
    RemoveTextAfterFocusLost = false,
    Callback = function(Text)
        local inputId = tonumber(Text)
        if inputId and inputId > 0 then
            pcall(function()
                -- Llama a la función UniversalEquip ya existente en el código
                UniversalEquip(inputId, false)
                if NotifyUser then
                    NotifyUser("Item Equipado", "Se equipó correctamente el ID: " .. tostring(inputId))
                end
            end)
        else
            if NotifyUser then
