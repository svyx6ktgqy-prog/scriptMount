...
-- Animaciones + Fix Ropa 3D + Auto-Ocultar Piernas + Fix Collar al Pecho
-- ==========================================================
...
local function UniversalEquip(assetId, isReequipping)
    ...
    local success, err = pcall(function()
        ...
        local function ProcessItem(item)
            ...
            -- =========================================================
            -- SOPORTE ACCESORIOS, COLLARES (PECHO + UPSCALE) Y ROPA 3D
            -- =========================================================
                ...
                if handle then
                    ...
                    if accAttach then
                        ...
                        if targetAttach then
                            ...
                            -- AJUSTE QUIRÚRGICO DE COLLARES (BAJAR AL PECHO Y UPSCALE)
                            local offsetCFrame = CFrame.new()
                            local attNameLower = string.lower(accAttach.Name)
                            local itemNameLower = string.lower(item.Name)

                            if string.find(attNameLower, "neck") or string.find(itemNameLower, "collar") or string.find(itemNameLower, "neck") or string.find(itemNameLower, "chain") or string.find(itemNameLower, "cadena") or string.find(itemNameLower, "pendant") then
                                offsetCFrame = CFrame.new(0, -0.18, -0.02)
                                
                                local mesh = handle:FindFirstChildWhichIsA("SpecialMesh")
                                if mesh then
                                    mesh.Scale = mesh.Scale * Vector3.new(1.12, 1.12, 1.12)
                                elseif handle:IsA("MeshPart") then
                                    pcall(function()
                                        handle.Size = handle.Size * 1.12
                                    end)
                                end
                            end
                        ...
                        if string.find(itemNameLower, "collar") or string.find(itemNameLower, "neck") or string.find(itemNameLower, "chain") or string.find(itemNameLower, "cadena") then
                            ...
                            fallbackOffset = CFrame.new(0, 0.25, -0.05)
                        else
