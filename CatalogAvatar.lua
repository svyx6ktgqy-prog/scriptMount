-- ==========================================================
-- MENU DE AVATARES QUIRÚRGICO Y PRO (v18 DEFINITIVA)
-- Live Update + Fix Ropa 3D + Switches Fusionados
-- ==========================================================
...
local KeepEquippedOnDeath = false -- Ahora se controla junto al menú Kitty
local SavedEquippedIDs = {}
local EqPanel, RefreshEquippedItems -- Declaraciones adelantadas para el Live Update
...
-- ==========================================================
-- 3. CEREBRO V18: ROPA 3D PROTEGIDA & LIVE UPDATE
-- ==========================================================
local function UniversalEquip(assetId, isReequipping)
    ...
    local success, err = pcall(function()
        ...
        local function ProcessItem(item)
            ...
            if item:IsA("Accessory") then
                ...
                if handle then
                    -- Aislamos el cambio de físicas en su propio pcall para que no rompa la Ropa 3D (Jackets/Pants)
                    pcall(function()
                    ...
                    if wrap then
                        -- Es Layered Clothing puro, solo lo insertamos y dejamos que Roblox haga el resto
                    ...
                    -- MOTOR DE SOLDADURA MATEMÁTICA (Alas, Pelo 3D, Espadas Clásicas)
    ...
    if success then
        -- LIVE UPDATE: Si el panel de ítems está abierto, se refresca inmediatamente
        ...
        warn("Fallback activado para ID: " .. tostring(numericId) .. " | Error interno: " .. tostring(err))
        local Backpack = LocalPlayer:FindFirstChildOfClass("Backpack")
        if Backpack then
            local fallbackTool = CreateExtremeBinoculars()
            fallbackTool.Parent = Backpack
            NotifyUser("⚠️ Contingencia Activada", "Se inyectó el equipo de reemplazo.")
        end
    end
end
...
RefreshEquippedItems = function()
    ...
    for _, item in ipairs(items) do
        ...
        btn.MouseButton1Click:Connect(function()
            if eqId then SavedEquippedIDs[eqId] = nil end
            item:Destroy()
            frame:Destroy()
        end)
    end
...
local Window = Rayfield:CreateWindow({
   Name = "🏥 Avatar Catalog Quirúrgico Pro v18",
   LoadingTitle = "Live Updates y Fix de Físicas 3D...",
...
-- 1. SWITCHES FUSIONADOS (Kitty Menu + Persistencia al Morir)
Panel:CreateToggle({
   ...
   Callback = function(Value)
       ...
       if Value then
           ...
           Rayfield:Notify({Title = "Sistema Unificado", Content = "Menú y persistencia DESACTIVADOS.", Duration = 3})
       end
   end,
})
