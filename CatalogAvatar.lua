-- ==========================================================
-- MENU DE AVATARES QUIRÚRGICO Y PRO (UNIFICADO v17 DEFINITIVO)
-- Anti-Duplicados + Fix Ropa 3D + Persistencia al Morir
-- ==========================================================
...
local KeepEquippedOnDeath = true
...
-- Comprueba si el ítem ya está en uso en el personaje
...
-- ==========================================================
-- 3. CEREBRO V17: FIX ROPA 3D, DUPLICADOS Y SOLDADURA
-- ==========================================================
local function UniversalEquip(assetId, isReequipping)
        ...
        local function ProcessItem(item)
            ...
            if item:IsA("Accessory") then
                ...
                if handle then
                    ...
                    -- A. FIX ROBUSTO DE ROPA 3D (Layered Clothing: Jackets, Pants 3D, Shoes)
    ...
    if not success then
        warn("Fallback activado para ID: " .. tostring(numericId))
        local Backpack = LocalPlayer:FindFirstChildOfClass("Backpack")
        if Backpack then
            local fallbackTool = CreateExtremeBinoculars()
            fallbackTool.Parent = Backpack
            NotifyUser("⚠️ Contingencia Activada", "Se inyectó una herramienta de respaldo.")
        end
    end
end
...
local Window = Rayfield:CreateWindow({
   Name = "🏥 Avatar Catalog Quirúrgico Pro v17",
   LoadingTitle = "Motor C0/C1, Anti-Duplicados y Fix 3D...",
...
Panel:CreateToggle({
   ...
   Callback = function(Value)
       ...
       if Value then
           ...
           FloatingBtn.Visible = false
           Rayfield:Notify({Title = "🎀 Menú Kitty", Content = "¡Activado!", Duration = 3})
       end
   end,
})

Panel:CreateToggle({
   Name = "🛡️ Mantener Ropa al Morir (Auto-Reequipar)",
   CurrentValue = true,
   Flag = "KeepOnDeathToggle",
   Callback = function(Value)
       KeepEquippedOnDeath = Value
       Rayfield:Notify({
           Title = "Persistencia al Morir", 
           Content = Value and "Tus ítems se mantendrán tras reaparecer." or "Las prendas se perderán al morir.", 
           Duration = 3
       })
   end,
})
