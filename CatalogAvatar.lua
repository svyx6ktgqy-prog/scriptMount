    ...
    if success and response then
        ...
        if decoded and decoded.data then
            ...
            for _, item in ipairs(decoded.data) do
                ...
                CardPrice.Position = UDim2.new(0, 25, 0, 155)
                CardPrice.BackgroundTransparency = 1
                CardPrice.Text = (item.price == 0 or item.price == nil) and "Gratis" or tostring(item.price)
                CardPrice.Font = Enum.Font.GothamBold
                CardPrice.TextSize = 13
                CardPrice.TextColor3 = Color3.fromRGB(50, 50, 50)
                CardPrice.TextXAlignment = Enum.TextXAlignment.Left
...
-- ==========================================================
-- 🚀 MENU RAYFIELD (NUEVAS OPCIONES INTEGRADAS)
-- ==========================================================
local Rayfield = loadstring(game:HttpGet('https://sirius.menu/rayfield'))()

local Window = Rayfield:CreateWindow({
    Name = "Menú Quirúrgico & Pro",
    LoadingTitle = "Cargando Módulos...",
    LoadingSubtitle = "Optimizando",
    ConfigurationSaving = { Enabled = false },
    Discord = { Enabled = false, Invite = "", RememberJoins = true },
    KeySystem = false
})

local MainTab = Window:CreateTab("Herramientas / Ajustes", 4483362458)

-- 1. Botón limpiador avanzado de RAM y Cache (Optimizado para Delta iPhone)
MainTab:CreateButton({
    Name = "🧹 Limpiador Avanzado RAM/Caché (Delta iOS)",
    Callback = function()
        pcall(function()
            -- Limpieza profunda de memoria nativa LUA
            collectgarbage("collect")
            
            -- Limpieza de caché de imágenes de tu sistema de banners
            BannerSystem.ClearCache()
            
            -- Purgado de ContentProvider para evitar cuellos de botella
            game:GetService("ContentProvider"):PreloadAsync({})
            
            -- Limpieza de hilos muertos y assets gráficos huérfanos
            for _, v in pairs(game:GetService("CoreGui"):GetDescendants()) do
                if v:IsA("ScreenGui") and not v.Enabled and v.Name ~= "QuirurgicoVisualizer" and v.Name ~= "KittyCatalogGui" then
                    v:Destroy()
                end
            end
        end)
        if NotifyUser then
            NotifyUser("Optimizador", "Memoria y Caché limpiados correctamente. Sin errores de CallBack.")
        end
    end,
})

-- 2. Botón ocultar-mostrar visualizador de ítems
MainTab:CreateToggle({
    Name = "👁️ Mostrar/Ocultar Visualizador de Ítems",
    CurrentValue = true,
    Flag = "VisualizerToggle",
    Callback = function(Value)
        if CoreGui:FindFirstChild("QuirurgicoVisualizer") then
            CoreGui.QuirurgicoVisualizer.Enabled = Value
        end
    end,
})

-- 3. Botones para Orientación de Pantalla
MainTab:CreateSection("📱 Orientación de Pantalla")

MainTab:CreateButton({
    Name = "Modo Horizontal (Landscape)",
    Callback = function()
        pcall(function()
            LocalPlayer.PlayerGui.ScreenOrientation = Enum.ScreenOrientation.LandscapeSensor
        end)
        if NotifyUser then
            NotifyUser("Pantalla", "Orientación ajustada a Horizontal.")
        end
    end,
})

MainTab:CreateButton({
    Name = "Modo Vertical (Portrait)",
    Callback = function()
        pcall(function()
            LocalPlayer.PlayerGui.ScreenOrientation = Enum.ScreenOrientation.Portrait
        end)
        if NotifyUser then
            NotifyUser("Pantalla", "Orientación ajustada a Vertical.")
        end
    end,
})

-- 4. Búsqueda por ID Rápida
MainTab:CreateSection("🔍 Búsqueda Rápida de Ítem")

local inputId = ""
MainTab:CreateInput({
    Name = "ID del Ítem a Equipar",
...
MainTab:CreateButton({
    Name = "✅ Equipar Ítem por ID",
    Callback = function()
        local idNum = tonumber(inputId)
        if idNum and idNum > 0 then
            CurrentData.Id = tostring(idNum)
            UniversalEquip(idNum, false)
            if NotifyUser then
                NotifyUser("Éxito", "Equipando Ítem ID: " .. tostring(idNum))
            end
        else
            if NotifyUser then
                NotifyUser("Error", "Por favor ingresa un ID numérico válido.")
            end
        end
    end,
})
