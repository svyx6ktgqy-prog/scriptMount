-- ==========================================================
-- MENU DE AVATARES QUIRÚRGICO Y PRO (UNIFICADO v9.6 + EXTREME DEBUG)
-- NUEVOS FIXES: Custom Debug Logger + Safe Equip + Clean Purchase
-- ==========================================================
...
-- ==========================================================
-- SISTEMA DE DIAGNÓSTICO ABSOLUTO (ANTI-SILENCIO)
-- ==========================================================
local function DebugAlert(title, text)
    -- 1. Intento por Consola
    print("[DIAGNÓSTICO] " .. title .. " | " .. tostring(text))
    pcall(function() warn("[DIAGNÓSTICO WARN] " .. title .. " | " .. tostring(text)) end)

    -- 2. Intento por Notificación Nativa de Roblox
    pcall(function()
    ...
    -- 3. Intento Forzado (Creación de GUI propia inbloqueable)
    pcall(function()
        local logGui = CoreGui:FindFirstChild("QuirurgicoUltraLog")
        if not logGui then
            logGui = Instance.new("ScreenGui")
            logGui.Name = "QuirurgicoUltraLog"
        ...
        msg.Size = UDim2.new(0, 320, 0, 50)
        msg.BackgroundColor3 = Color3.fromRGB(15, 15, 15)
        ...
        Debris:AddItem(msg, 10) -- Se borra después de 10 segundos
    end)
end

-- ==========================================================
-- SISTEMA DE EQUIPAMIENTO HÍBRIDO SUPER-REFORZADO
-- ==========================================================
local function UniversalEquip(assetId)
    local cleanAssetId = tonumber(tostring(assetId):match("%d+"))
    if not cleanAssetId then 
        DebugAlert("ERROR CRÍTICO", "ID inválida extraída del botón.")
        return 
    end

    DebugAlert("INICIO", "Procesando ID: " .. cleanAssetId)

    task.spawn(function()
        local Char = LocalPlayer.Character
        if not Char then 
            DebugAlert("ERROR CHAR", "No se encontró Character.")
            return 
        end
        
        local Hum = Char:FindFirstChildOfClass("Humanoid")
        if not Hum then 
            DebugAlert("ERROR HUM", "No se encontró Humanoid.")
            return 
        end

        DebugAlert("FASE 1", "Personaje válido. Iniciando Método 1 (Description).")

        -- ==========================================
        -- MÉTODO 1: HumanoidDescription
        -- ==========================================
        local successDesc, errDesc = pcall(function()
            DebugAlert("FASE 1A", "Llamando a GetProductInfo...")
            local info = MarketplaceService:GetProductInfo(cleanAssetId)
            DebugAlert("FASE 1B", "GetProductInfo respondió.")
            
            local desc = Hum:GetAppliedDescription()
            local typeId = info.AssetTypeId
            
            DebugAlert("FASE 1C", "AssetTypeId: " .. tostring(typeId))
            
            if typeId == 2 then desc.GraphicTShirt = cleanAssetId
            elseif typeId == 11 then desc.Shirt = cleanAssetId
            elseif typeId == 12 then desc.Pants = cleanAssetId
            elseif typeId == 18 then desc.Face = cleanAssetId
            elseif typeId == 17 then desc.Head = cleanAssetId
            elseif typeId == 8 or (typeId >= 41 and typeId <= 47) then
                local currentStr = desc.HatAccessory
                desc.HatAccessory = currentStr == "" and tostring(cleanAssetId) or currentStr .. "," .. tostring(cleanAssetId)
            end
            
            DebugAlert("FASE 1D", "Aplicando Description...")
            Hum:ApplyDescription(desc)
        end)

        if successDesc then
            DebugAlert("ÉXITO TOTAL", "Método 1 completado.")
            return
        else
            DebugAlert("FALLO MÉTODO 1", string.sub(tostring(errDesc), 1, 80))
        end

        DebugAlert("FASE 2", "Iniciando Método 2 (GetObjects).")

        -- ==========================================
        -- MÉTODO 2: GetObjects Clásico
        -- ==========================================
        local successObj, errObj = pcall(function()
            local objects = game:GetObjects("rbxassetid://" .. cleanAssetId)
            DebugAlert("FASE 2A", "GetObjects respondió. Objetos: " .. tostring(#objects))
        ...
        if successObj then
            DebugAlert("ÉXITO TOTAL", "Método 2 completado.")
            return
        else
            DebugAlert("FALLO MÉTODO 2", string.sub(tostring(errObj), 1, 80))
        end

        DebugAlert("FASE 3", "Iniciando Método 3 (InsertService).")

        -- ==========================================
        -- MÉTODO 3: InsertService (Fallback)
        -- ==========================================
        local successInsert, errInsert = pcall(function()
            local InsertService = game:GetService("InsertService")
            local model = InsertService:LoadLocalAsset("rbxassetid://" .. cleanAssetId)
            if not model then
                model = InsertService:LoadAsset(cleanAssetId)
            end
            
            if model then
                DebugAlert("FASE 3A", "Modelo insertado correctamente.")
        ...
        if successInsert then
            DebugAlert("ÉXITO TOTAL", "Método 3 completado.")
        else
            DebugAlert("FRACASO ABSOLUTO", "Ningún método funcionó: " .. string.sub(tostring(errInsert), 1, 60))
        end
    end)
end

-- ==========================================================
-- SISTEMA DE VISUALIZADOR HUD
-- ==========================================================
...
ImagePreview.MouseButton1Click:Connect(function()
    local idStr = tostring(CurrentData.Id):match("%d+")
...
-- ==========================================================
-- SISTEMA KITTY CATALOG UI (MENÚ Y BOTÓN FLOTANTE)
-- ==========================================================
...
SidebarLayout:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(function()
    KittySidebar.CanvasSize = UDim2.new(0, 0, 0, SidebarLayout.AbsoluteContentSize.Y + 20)
end)
...
ResultsLayout:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(function()
    KittyResults.CanvasSize = UDim2.new(0, 0, 0, ResultsLayout.AbsoluteContentSize.Y + 30)
end)
...
-- ==========================================================
-- DICCIONARIO Y LÓGICA DEL SCRIPT ORIGINAL
-- ==========================================================
...
local Window = Rayfield:CreateWindow({
   Name = "🏥 Avatar Catalog Quirúrgico Pro v9.6",
   LoadingTitle = "Motor Unificado v9.6...",
   LoadingSubtitle = "Extreme Debug Logger Activo",
...
Panel:CreateToggle({
   ...
   Callback = function(Value)
       ...
       if Value then
           KittyMain.Visible = true
           FloatingBtn.Visible = false
           Rayfield:Notify({Title = "🎀 Menú Kitty", Content = "¡Activado! Puedes cerrarlo para usar el botón flotante.", Duration = 3})
       end
   end,
})
