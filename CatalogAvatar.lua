...
local function UniversalEquip(assetId)
    task.spawn(function()
        ...
        -- 1. Obtener información del Asset para saber QUÉ estamos equipando
        local successInfo, info = pcall(function() 
            return MarketplaceService:GetProductInfo(assetId) 
        end)
        
        if not successInfo or not info then return end
        local typeId = info.AssetTypeId

        -- ==========================================
        -- MÉTODO 1: Inyección Directa Local (100% Fiable para ropa y caras)
        -- ==========================================
        if typeId == 11 then -- Camisa
        ...
        -- ==========================================
        -- MÉTODO 2: Motor GetObjects para Accesorios (Depende del Ejecutor)
        -- ==========================================
        local successObj, objects = pcall(function()
            return game:GetObjects("rbxassetid://" .. tostring(assetId))
        end)

        if successObj and objects then
            for _, obj in ipairs(objects) do
                if obj:IsA("Accessory") then
                    Hum:AddAccessory(obj)
                elseif obj:IsA("Decal") and typeId == 18 then
                    local head = Char:FindFirstChild("Head")
                    if head then
                        local face = head:FindFirstChildOfClass("Decal")
                        if face then face.Texture = obj.Texture else obj.Parent = head end
                    end
                end
            end
        else
            warn("Tu ejecutor no soporta GetObjects o el modelo 3D está restringido.")
        end
    end)
end

-- ==========================================================
-- SISTEMA DE VISUALIZADOR HUD (COMPRA INTELIGENTE EN PLAYERGUI)
-- ==========================================================
...
VisualizerGui.ResetOnSpawn = false -- Evita que desaparezca al morir
VisualizerGui.Parent = PlayerGui -- Alojado en PlayerGui para permitir PromptPurchase
