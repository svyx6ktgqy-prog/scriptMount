...
-- ==========================================================
-- NUEVO: MENÚ DE ALERTA DE OBTENCIÓN (CUSTOM + NATIVO DEFAULT)
-- ==========================================================
...
BtnAlertYes.MouseButton1Click:Connect(function()
    ...
    if assetId and assetId > 0 then
        -- Abre el menú por defecto de Roblox (MarketplaceService) de manera segura
        pcall(function()
...
-- ==========================================================
-- VISUALIZADOR PREVIEW CORREGIDO (Menú Alert Implementado)
-- ==========================================================
...
-- Método Home aislado y sin paginación (Carga ítems raros/ocultos)
local function PerformHomeSearch()
    ...
    -- Ocultar UI de paginación para indicar que es una búsqueda especial
    if PagiLabel then PagiLabel.Text = "Búsqueda Especial (Sin Cursor)" end
    if PagiPrevBtn then PagiPrevBtn.Visible = false end
    if PagiNextBtn then PagiNextBtn.Visible = false end

    -- Búsqueda cruda, sin 'cursor', como se solicitó para ver items ocultos/raros
...
-- ==========================================================
-- CONTENEDOR DE LÍMITE (NUEVO SPINNER INTEGRADO CORREGIDO)
-- ==========================================================
...
-- Solución al Error de Red: Si cambiamos el límite, reseteamos la búsqueda
-- para que el Cursor de Paginación de la API de Roblox no se rompa por discordancia.
local function UpdateLimit(newVal)
    newVal = math.clamp(newVal, KITTY_MIN_LIMIT, KITTY_MAX_LIMIT)
    if KittySearchLimit == newVal then return end -- Evita recargas innecesarias
...
local SpinnerDropdown = Panel:CreateDropdown({
   ...
   Callback = function(Option)
       ...
       if SearchResultsCache[selectedText] then
           ...
           CurrentData.Id = tostring(item.Id); CurrentData.Name = item.Name;
           -- Nota: Asumiendo que el script de Rayfield se manejaría a sí mismo a partir de este punto
       end
   end,
})
