-- ==========================================================
-- MENU DE AVATARES QUIRÚRGICO Y PRO (v21 DEFINITIVA + FIX ROPA 3D)
-- Animaciones en Panel + Reset Total a Avatar Predeterminado
-- ==========================================================
...
local LegsHidden = false -- [NUEVO] Variable para ocultar piernas
...
-- ==========================================================
-- [NUEVO] GESTIÓN DE TRANSPARENCIA DE PIERNAS (EVITAR CLIPPING)
-- ==========================================================
local LegPartNames = {
...
local function ToggleLegsVisibility(hide)
    LegsHidden = hide
...
local function ResetToDefaultAvatar()
    ...
    SavedEquippedIDs = {}
    ToggleLegsVisibility(false) -- [NUEVO] Restaurar piernas visibles
    
    -- Detener y limpiar animaciones en curso
    ...
    -- Limpiar Tools agregadas a la mochila
    ...
    -- Limpiar elementos añadidos al personaje
    ...
    -- Restaurar ropa y cuerpo original mediante la API de Roblox
    pcall(function()
...
-- ==========================================================
-- 2. BUCLE ANTI-INVISIBILIDAD Y RENDER (AHORA CON FIX DE PIERNAS)
-- ==========================================================
RunService.RenderStepped:Connect(function()
    ...
    if char then
        ...
        -- [NUEVO] Mantener las piernas invisibles si la opción está activa
        if LegsHidden then
            for _, partName in ipairs(LegPartNames) do
                local part = char:FindFirstChild(partName)
                if part and part:IsA("BasePart") then
...
-- ==========================================================
-- 3. CEREBRO V21: FIX ROPA 3D + ANIMACIONES EN MENÚ + TOOLS
-- ==========================================================
local function UniversalEquip(assetId, isReequipping)
    ...
    local success, err = pcall(function()
        ...
        -- Fallback si GetObjects no devuelve instancias directamente (Animaciones u Emotes)
        if #objects == 0 then
            ...
            -- Crear contenedor para el Menú de Equipados
        ...
        local function ProcessItem(item)
            ...
            -- SOPORTE ACCESORIOS Y ROPA 3D (Layered Clothing)
            if item:IsA("Accessory") then
                ...
                if handle then
                    ...
                    -- [NUEVO] DETECCIÓN DE ROPA 3D PARA NO ROMPER LA DEFORMACIÓN
                    local wrapLayer = handle:FindFirstChildWhichIsA("WrapLayer", true)
                    
                    if not wrapLayer then
                        -- Es un accesorio rígido tradicional: aplicamos el Weld manual
            ...
            -- SOPORTE ANIMACIONES Y EMOTES EN TIEMPO REAL
...
LocalPlayer.CharacterAdded:Connect(function(newChar)
    ...
    if LegsHidden then ToggleLegsVisibility(true) end -- [NUEVO] Oculta piernas al renacer
...
-- [NUEVA SECCIÓN] Control de Transparencia para pantalones
Panel:CreateSection("🦵 Ajuste de Malla / Anti-Clipping")

Panel:CreateToggle({
   Name = "🚫 Ocultar Piernas Base (Fix Pantalón 3D)",
   CurrentValue = false,
   Callback = function(Value)
       ToggleLegsVisibility(Value)
       Rayfield:Notify({
           Title = "Piernas " .. (Value and "Ocultas" or "Visibles"),
           Content = Value and "Se ocultaron las piernas base." or "Piernas restauradas.",
           Duration = 2.5
       })
   end,
})
