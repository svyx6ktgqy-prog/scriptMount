-- ==========================================================
-- MENU DE AVATARES QUIRÚRGICO Y PRO (v25.4 ANTI-FREEZE & ULTRA-ASYNC)
-- Gestión Eficiente de Memoria + Frame-Slicing + Carga Asíncrona de I/O
-- + Filtro de Precios Avanzado & Copiado de ID a Portapapeles (iOS Delta)
-- + Edit Parts Menu Integrado & Persistencia de Partes Ocultas
-- ==========================================================
...
-- ==========================================================
-- SISTEMA DE BODY PARTS VISIBILITY
-- ==========================================================
local HiddenBodyParts = {
    Head = false, Torso = false, LeftArm = false, RightArm = false, LeftLeg = false, RightLeg = false
}
local UpdateCheckboxesUI 

local function GetHidesLegs(char)
    if not char then return false end
    for _, v in ipairs(char:GetDescendants()) do
        if v:GetAttribute("HidesLegs") then return true end
    end
    return false
end

local function ApplyBodyPartVisibility(character, hiddenPartsTable)
    if not character then return end
    local hidesLegs = GetHidesLegs(character)
    ...
    for _, part in ipairs(character:GetChildren()) do
        if part:IsA("BasePart") and part.Name ~= "HumanoidRootPart" then
            ...
            for logicalName, isSet in pairs(hiddenPartsTable or {}) do
                if isSet then
                    for _, realName in ipairs(partMaps[logicalName]) do
                        if part.Name == realName then
                            isHidden = true
                            break
                        end
                    end
            ...
            if hidesLegs and not isHidden then
            ...
            if part:IsA("MeshPart") or part.Name == "Head" then
                ...
                if face then
                    face.Transparency = isHidden and 1 or 0
                end
            end
        end
    end
...
local function ResetToDefaultAvatar()
    SavedEquippedIDs = {}
    ItemAdjustments = {}
    HiddenBodyParts = { Head = false, Torso = false, LeftArm = false, RightArm = false, LeftLeg = false, RightLeg = false }
    if UpdateCheckboxesUI then UpdateCheckboxesUI() end
    ...
    ApplyBodyPartVisibility(Char, HiddenBodyParts)
...
LocalPlayer.CharacterAdded:Connect(function(newChar)
    ...
    task.spawn(function()
        ...
        task.wait(0.5)
        ApplyBodyPartVisibility(newChar, HiddenBodyParts)
    end)
end)
...
BtnAlertNo.MouseButton1Click:Connect(function() PurchaseAlertMenu.Visible = false end)
BtnAlertYes.MouseButton1Click:Connect(function()
    PurchaseAlertMenu.Visible = false
    local assetId = tonumber(CurrentData.Id)
    if assetId and assetId > 0 then
        pcall(function() MarketplaceService:PromptPurchase(LocalPlayer, assetId) end)
    end
end)
-- ==========================================================
...
-- ==========================================================
-- MENÚ DE PERSONAJES (SAVED OUTFITS Y PARTS)
-- ==========================================================
...
local function RefreshSavedCharactersGrid()
    ...
    LoadSavedCharactersDataAsync(function(data)
        task.spawn(function()
            for charName, savedData in pairs(data) do
                -- Compatibilidad con formato antiguo vs nuevo
                local itemEntries = type(savedData) == "table" and savedData.items or savedData
                local savedHiddenParts = type(savedData) == "table" and savedData.hiddenParts or {}
                ...
                task.spawn(function()
                            ...
                            if handle then
                                ...
                                if accAttach then
                                    ...
                                    if targetAttach then
                                        ...
                                        local weld = Instance.new("Weld")
                                        weld.Name = "AccessoryWeld"; weld.Part0 = targetAttach.Parent
                                        weld.Part1 = handle; weld.C0 = targetAttach.CFrame; weld.C1 = accAttach.CFrame
                                        weld.Parent = handle
                                    end
                                else
                                    ...
                                    if bodyTarget then
                                        for _, j in ipairs(handle:GetJoints()) do j:Destroy() end
                                        local weld = Instance.new("Weld")
                                        weld.Name = "AccessoryWeld"; weld.Part0 = bodyTarget
                                        weld.Part1 = handle; weld.Parent = handle
                                    end
                                end
                                ...
                                if isPant then cloneItem:SetAttribute("HidesLegs", true) end
                            end
                    ...
                    -- Aplica la visualización de partes ocultas al dummy
                    ApplyBodyPartVisibility(dummy, savedHiddenParts)
                ...
                LoadBtn.MouseButton1Click:Connect(function()
                    ...
                    task.spawn(function()
                        ...
                        -- Cargar y Aplicar configuración de partes ocultas
                        HiddenBodyParts = {
                            Head = savedHiddenParts.Head or false,
                            Torso = savedHiddenParts.Torso or false,
                            LeftArm = savedHiddenParts.LeftArm or false,
                            RightArm = savedHiddenParts.RightArm or false,
                            LeftLeg = savedHiddenParts.LeftLeg or false,
                            RightLeg = savedHiddenParts.RightLeg or false
                        }
                        if UpdateCheckboxesUI then UpdateCheckboxesUI() end
                        ApplyBodyPartVisibility(LocalPlayer.Character, HiddenBodyParts)
...
SaveCharBtn.MouseButton1Click:Connect(function()
    ...
    if #itemsToSave == 0 and not (HiddenBodyParts.Head or HiddenBodyParts.Torso or HiddenBodyParts.LeftArm or HiddenBodyParts.RightArm or HiddenBodyParts.LeftLeg or HiddenBodyParts.RightLeg) then 
        NotifyUser("Atención", "No tienes ítems equipados ni partes ocultas para guardar.")
        return 
    end

    LoadSavedCharactersDataAsync(function(data)
        -- Guardamos Items Y configuración de Body Parts
        local charData = {
            items = itemsToSave,
            hiddenParts = {
                Head = HiddenBodyParts.Head,
                Torso = HiddenBodyParts.Torso,
                LeftArm = HiddenBodyParts.LeftArm,
                RightArm = HiddenBodyParts.RightArm,
                LeftLeg = HiddenBodyParts.LeftLeg,
                RightLeg = HiddenBodyParts.RightLeg
            }
        }
        data[name] = charData
        ...
        NameInput.Text = ""
        RefreshSavedCharactersGrid()
        NotifyUser("Guardado Exitoso", "Personaje '" .. name .. "' guardado con sus ajustes y partes.")
    end)
end)

-- ==========================================================
-- EDIT PANEL (AJUSTE DE ITEMS + BODY PARTS MENU)
-- ==========================================================
local EditPanel = Instance.new("Frame")
EditPanel.Size = UDim2.new(0, 340, 0, 320)
...
-- SUB-MENÚ DE EDICIÓN DE PARTES DEL CUERPO
local BodyPartsPanel = Instance.new("Frame")
BodyPartsPanel.Size = UDim2.new(0, 200, 0, 280)
BodyPartsPanel.Position = UDim2.new(0.5, 180, 0.5, 0)
BodyPartsPanel.AnchorPoint = Vector2.new(0, 0.5)
BodyPartsPanel.BackgroundColor3 = Color3.fromRGB(30, 30, 30)
BodyPartsPanel.Visible = false
BodyPartsPanel.ZIndex = 20
BodyPartsPanel.Parent = VisualizerGui
local BPCorner = Instance.new("UICorner"); BPCorner.CornerRadius = UDim.new(0, 10); BPCorner.Parent = BodyPartsPanel
local BPStroke = Instance.new("UIStroke"); BPStroke.Color = Color3.fromRGB(50, 205, 50); BPStroke.Thickness = 2; BPStroke.Parent = BodyPartsPanel

local BPTitle = Instance.new("TextLabel")
BPTitle.Size = UDim2.new(1, 0, 0, 35)
BPTitle.BackgroundTransparency = 1
BPTitle.Text = "🩻 Edit Body Parts"
BPTitle.Font = Enum.Font.GothamBold
BPTitle.TextColor3 = Color3.fromRGB(255, 255, 255)
BPTitle.TextSize = 13
BPTitle.Parent = BodyPartsPanel

local BPListLayout = Instance.new("UIListLayout")
BPListLayout.Padding = UDim.new(0, 5)
BPListLayout.HorizontalAlignment = Enum.HorizontalAlignment.Center
BPListLayout.SortOrder = Enum.SortOrder.LayoutOrder

local BPListFrame = Instance.new("Frame")
BPListFrame.Size = UDim2.new(1, 0, 1, -40)
BPListFrame.Position = UDim2.new(0, 0, 0, 40)
BPListFrame.BackgroundTransparency = 1
BPListFrame.Parent = BodyPartsPanel
BPListLayout.Parent = BPListFrame

local UIRefreshCallbacks = {}
local function CreatePartCheckbox(labelText, propertyKey)
    local Frame = Instance.new("Frame")
    Frame.Size = UDim2.new(1, -20, 0, 32)
    Frame.BackgroundColor3 = Color3.fromRGB(45, 45, 45)
    Frame.Parent = BPListFrame
    ...
    Btn.BackgroundColor3 = HiddenBodyParts[propertyKey] and Color3.fromRGB(50, 205, 50) or Color3.fromRGB(70, 70, 70)
    Btn.Text = HiddenBodyParts[propertyKey] and "✓" or ""
    ...
    table.insert(UIRefreshCallbacks, UpdateVis)
    
    Btn.MouseButton1Click:Connect(function()
        HiddenBodyParts[propertyKey] = not HiddenBodyParts[propertyKey]
        UpdateVis()
        ApplyBodyPartVisibility(LocalPlayer.Character, HiddenBodyParts)
    end)
end

CreatePartCheckbox("Remove Head", "Head")
CreatePartCheckbox("Remove Torso", "Torso")
CreatePartCheckbox("Remove Left Arm", "LeftArm")
CreatePartCheckbox("Remove Right Arm", "RightArm")
CreatePartCheckbox("Remove Left Leg", "LeftLeg")
CreatePartCheckbox("Remove Right Leg", "RightLeg")

UpdateCheckboxesUI = function()
    for _, cb in ipairs(UIRefreshCallbacks) do cb() end
end

-- Botón principal para abrir Sub-menú Body Parts
local OpenBPBtn = Instance.new("TextButton")
OpenBPBtn.Size = UDim2.new(0, 180, 0, 35)
OpenBPBtn.Position = UDim2.new(0.5, -90, 0, 275)
OpenBPBtn.BackgroundColor3 = Color3.fromRGB(50, 150, 100)
OpenBPBtn.Text = "🩻 Edit Body Parts"
OpenBPBtn.Font = Enum.Font.GothamBold
OpenBPBtn.TextColor3 = Color3.fromRGB(255, 255, 255)
OpenBPBtn.TextSize = 12
OpenBPBtn.ZIndex = 16
OpenBPBtn.Parent = EditPanel
local OpenBPCorner = Instance.new("UICorner"); OpenBPCorner.CornerRadius = UDim.new(0, 6); OpenBPCorner.Parent = OpenBPBtn

OpenBPBtn.MouseButton1Click:Connect(function()
    BodyPartsPanel.Visible = not BodyPartsPanel.Visible
end)
...
local function OpenEditMenuFor(item)
    ...
    EqPanel.Visible = false; EditPanel.Visible = true; BodyPartsPanel.Visible = false
end
            ...
            while isPreviewPressing do
                if tick() - previewHoldStart >= 0.55 then
                    ...
                    if assetId and assetId > 0 then
                        ...
                        if NotifyUser then NotifyUser("Error", "No hay un ítem seleccionado") end
                    end
                    break
                end
                task.wait(0.03)
...
ImagePreview.InputEnded:Connect(function(input)
    if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then isPreviewPressing = false end
end)
...
local Window = Rayfield:CreateWindow({
   Name = "🏥 Avatar Catalog Quirúrgico Pro v25.4 Ultra-Async",
