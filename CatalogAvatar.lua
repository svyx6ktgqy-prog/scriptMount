...
local ItemAdjustments = {} -- Guardar transformaciones persistentes por ítem
...
-- ==========================================================
-- SISTEMA DE EDICIÓN DE ÍTEMS EN TIEMPO REAL (PERSISTENTE Y TRASLÚCIDO)
-- ==========================================================
...
EditPanel.BackgroundTransparency = 0.4 -- Fondo traslucido para ver al personaje
...
EditCloseBtn.MouseButton1Click:Connect(function()
    EditPanel.Visible = false
    EqPanel.Visible = true -- Reabre el menú de elementos al cerrar
end)
...
local function ApplyTransformations()
    ...
    if handle then
        ...
        if weld and data.OriginalWeldC1 then
            weld.C1 = data.OriginalWeldC1 * CFrame.new(-EditOffsetX, -EditOffsetY, 0)
        end
...
BtnLeft.BackgroundTransparency = 0.2
BtnLeft.Text = "⏮️"
BtnLeft.TextSize = 20
...
BtnRight.BackgroundTransparency = 0.2
BtnRight.Text = "⏭️"
...
local function OpenEditMenuFor(item)
    ...
    -- Registra o recupera los datos iniciales/guardados del ítem
    ...
    -- Carga el estado actual guardado
    local data = ItemAdjustments[item]
    EditOffsetX = data.OffsetX
    EditOffsetY = data.OffsetY
    EditScale = data.Scale
    
    -- Refresca la posición de los sliders según el estado guardado
...
RefreshEquippedItems = function()
    ...
    for _, item in ipairs(items) do
        ...
        -- DETECTOR DE CLIC vs MANTENER PRESIONADO
            ...
            if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
                if isPressing then
                    ...
                    if timeHeld < 0.5 then
                        ...
                        ItemAdjustments[item] = nil -- Limpia la memoria del ítem desequipado
                        item:Destroy()
                        frame:Destroy()
                    end
                end
