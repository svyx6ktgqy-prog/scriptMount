-- ==========================================================
-- MENU DE AVATARES QUIRÚRGICO Y PRO (UNIFICADO v9.4 + KITTY MENU)
-- Fix Compras + Fix UI Search + Equipamiento Universal Recursivo
-- ==========================================================
...
-- ==========================================================
-- SISTEMA DE EQUIPAMIENTO UNIVERSAL RECURSIVO (EL FIX MAESTRO)
-- ==========================================================
local function UniversalEquip(assetId)
    ...
    local success, err = pcall(function()
        ...
        -- Función recursiva para escarbar en Models/Folders y encontrar los accesorios reales
        local function ApplyObject(obj)
            if obj:IsA("Accessory") then
                if Hum then Hum:AddAccessory(obj:Clone()) end
            ...
            elseif obj:IsA("Decal") then
                ...
                if head then
                    ...
                    if currentFace then
                        currentFace.Texture = obj.Texture
                    else
                        obj:Clone().Parent = head
                    end
                end
            elseif obj:IsA("CharacterMesh") or obj:IsA("BodyColors") then
                for _, v in pairs(Char:GetChildren()) do
                    if v.ClassName == obj.ClassName then v:Destroy() end
                end
                obj:Clone().Parent = Char
            elseif obj:IsA("Folder") or obj:IsA("Model") then
                -- Si viene empaquetado, revisamos su contenido
...
-- FIX COMPRA: Validación con API y uso de task.spawn
        ...
        task.spawn(function()
            pcall(function()
                ...
                if s_info and info then
                    if info.AssetTypeId == 37 or CurrentData.ItemType == "Bundle" then
...
SearchContainer.Size = UDim2.new(0.65, 0, 0, 40)
...
-- FIX UI: Botón Search redimensionado para evitar superposición con la X
local KittySearchBtn = Instance.new("TextButton")
KittySearchBtn.Size = UDim2.new(0.22, 0, 0, 40)
KittySearchBtn.Position = UDim2.new(0.68, 0, 0.5, -20)
        ...
        if decoded and decoded.data then
            for _, item in ipairs(decoded.data) do
                ...
                ClickBtn.MouseButton1Click:Connect(function()
                    ...
                    UniversalEquip(item.id) -- Usamos el nuevo sistema robusto
                end)
            end
        end
    end
...
local Window = Rayfield:CreateWindow({
   Name = "🏥 Avatar Catalog Quirúrgico Pro v9.4",
   LoadingTitle = "Motor Unificado v9.4...",
   LoadingSubtitle = "Universal Equip + Fix Compras",
...
Panel:CreateButton({
   ...
   Callback = function()
       ...
       -- Llamada a la nueva función universal
       local success = UniversalEquip(assetId)
