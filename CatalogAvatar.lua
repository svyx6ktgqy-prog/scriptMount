-- ==========================================================
-- MOTOR QUIRÚRGICO V14.0 ANCLAJE ABSOLUTO (KITTY UI + SUPER ULTRA)
-- 100% Equipamiento, Fix Soldadura Matemática 3D, Alas/Pelo, Fallbacks
-- ==========================================================
...
-- ==========================================================
-- 3. CEREBRO V14: SOLDADURA MATEMÁTICA 3D FORZADA
-- ==========================================================
local function UniversalEquip(assetId)
        ...
        local function ProcessItem(item)
            ...
            if item:IsA("Accessory") then
                ...
                if handle then
                    ...
                    -- A. Fix de Layered Clothing (Ropa 3D Gótica/Moderna)
                    ...
                    -- B. MOTOR DE SOLDADURA MATEMÁTICA (Alas, Pelo 3D, Espadas)
                    local accAttach = handle:FindFirstChildWhichIsA("Attachment")
                    if accAttach then
                        local targetAttach = nil
                        -- Buscar el anclaje exacto en el cuerpo del jugador (Ej: BackAttachment)
                        for _, v in ipairs(Char:GetDescendants()) do
                        ...
                        if targetAttach then
                            -- Destruir uniones viejas para evitar conflictos físicos
                            for _, v in ipairs(handle:GetJoints()) do v:Destroy() end
                            
                            -- Crear soldadura absoluta C0 / C1
                            ...
                            return -- Exito absoluto de anclaje, salir de la función
                        end
                    end
                end
                
                -- C. Fallback: Si no tiene anclaje físico, forzar por motor nativo
    ...
    if not success then
        warn("Error de ID. Inyectando método de contingencia extrema.")
        local Backpack = LocalPlayer:FindFirstChildOfClass("Backpack")
        if Backpack then
            local fallbackTool = CreateExtremeBinoculars()
            fallbackTool.Parent = Backpack
            NotifyUser("⚠️ Contingencia Activada", "Se ha inyectado el equipo de reemplazo de precisión.")
        end
    end
end

-- ==========================================================
-- 4. SISTEMA DE VISUALIZADOR HUD (COMPRA INTELIGENTE)
-- ==========================================================
    ...
    if assetId and assetId > 0 then
        pcall(function()
            if CurrentData.ItemType == "Bundle" then
                MarketplaceService:PromptBundlePurchase(LocalPlayer, assetId)
            else
...
SearchContainer.Size = UDim2.new(0.7, -40, 0, 40)
...
KittySearchBtn.Size = UDim2.new(0.3, -5, 0, 40)
KittySearchBtn.Position = UDim2.new(0.7, -20, 0.5, -20)
...
-- ==========================================================
-- 7. INTERFAZ RAYFIELD V14.0 OMNI-ANCHOR
-- ==========================================================
...
local Window = Rayfield:CreateWindow({
   Name = "🏥 Avatar Catalog Quirúrgico V14.0",
   LoadingTitle = "Cargando Motor Omni-Anchor 3D...",
   LoadingSubtitle = "Soldadura Matemática C0/C1 Activa",
...
Panel:CreateToggle({
   Name = "🎀 Activar Menú Kitty Visual",
   CurrentValue = false,
   ...
   Callback = function(Value)
       ...
       if Value then
           KittyMain.Visible = true
           FloatingBtn.Visible = false
           Rayfield:Notify({Title = "🎀 Menú Kitty", Content = "¡Activado! Ciérralo para ver el botón flotante.", Duration = 3})
       end
   end,
})
   ...
   Callback = function(Text)
       ...
       if success and response then
           ...
           if decoded and decoded.data then
               ...
               if #newOptions > 0 then SpinnerDropdown:Refresh(newOptions, true) end
           end
       end
   end,
})
...
Panel:CreateButton({
   Name = "⚡ Equipar con Anclaje Absoluto 3D",
   Callback = function()
       local assetId = tonumber(CurrentData.Id)
       if not assetId or assetId == 0 then return end
       UniversalEquip(assetId)
       Rayfield:Notify({Title = "Anclaje 3D", Content = "Se forzó la soldadura de: " .. CurrentData.Name, Duration = 3})
   end,
})
...
Panel:CreateSection("⚙️ Rendimiento y Contingencias")

Panel:CreateButton({
   Name = "🛠️ Forzar Fallback (Binoculares Quirúrgicos)",
   Callback = function()
       local Backpack = LocalPlayer:FindFirstChildOfClass("Backpack")
       if Backpack then
           local bino = CreateExtremeBinoculars()
           bino.Parent = Backpack
           Rayfield:Notify({Title = "Inyección Exitosa", Content = "Herramienta custom insertada.", Duration = 3})
       end
   end,
})
