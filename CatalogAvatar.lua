-- ==========================================================
-- MENU DE AVATARES QUIRÚRGICO Y PRO (FIX DEFINITIVO EQUIPADOS)
-- Soporte Collares/Pulseras + Anti-Desaparición + Menú Equipados
-- ==========================================================
...
-- ==========================================================
-- 3. CEREBRO: FIX ADHERENCIA COLLARES/PULSERAS Y EQUIPADOS
-- ==========================================================
local function UniversalEquip(assetId, isReequipping)
    ...
    local success, err = pcall(function()
        ...
        local function ProcessItem(item)
            ...
            -- =========================================================
            -- ACCESORIOS, COLLARES Y PULSERAS
            -- =========================================================
            if item:IsA("Accessory") then
                local cloneItem = item:Clone()
                cloneItem:SetAttribute("AssetId", numericId)
                local handle = cloneItem:FindFirstChild("Handle") or cloneItem:FindFirstChildWhichIsA("BasePart")
                
                if handle then
                    -- Desactivar físicas para evitar caídas
                    for _, part in ipairs(cloneItem:GetDescendants()) do
                        if part:IsA("BasePart") then
                    ...
                    -- Detección de ropa 3D y ocultación de extremidades
                    local wrapLayer = handle:FindFirstChildWhichIsA("WrapLayer")
                    if wrapLayer then
                        pcall(function() wrapLayer.AutoSkin = Enum.AutoSkin.Enabled end)
                        task.defer(function()
                    ...
                    -- DETECCIÓN DE ANCLAJE PARA COLLARES Y PULSERAS
                    local att = handle:FindFirstChildWhichIsA("Attachment")
                    local hasMatchingAttachment = false
                    ...
                    -- Si el motor nativo de Roblox lo soporta, usar AddAccessory
                    if hasMatchingAttachment then
                        Hum:AddAccessory(cloneItem)
                    else
                        -- ENSAMBLAJE MANUAL DIRECTO (Collares, Pulseras y accesorios sin Attachment nativo)
                        cloneItem.Parent = Char
                        
                        local targetPart = Char:FindFirstChild("UpperTorso") or Char:FindFirstChild("Torso") or Char:FindFirstChild("Head")
                        local nameLower = string.lower(cloneItem.Name .. " " .. handle.Name)
                        
                        -- Mapeo específico de piezas
                        if string.find(nameLower, "neck") or string.find(nameLower, "chain") or string.find(nameLower, "collar") or string.find(nameLower, "gargantilla") then
                            targetPart = Char:FindFirstChild("UpperTorso") or Char:FindFirstChild("Torso") or Char:FindFirstChild("Head")
                        elseif string.find(nameLower, "wrist") or string.find(nameLower, "bracelet") or string.find(nameLower, "pulsera") or string.find(nameLower, "arm") then
                            targetPart = Char:FindFirstChild("RightHand") or Char:FindFirstChild("RightLowerArm") or Char:FindFirstChild("Right Arm")
                        elseif string.find(nameLower, "leftwrist") or string.find(nameLower, "leftbracelet") then
                            targetPart = Char:FindFirstChild("LeftHand") or Char:FindFirstChild("LeftLowerArm") or Char:FindFirstChild("Left Arm")
                        end

                        if targetPart then
                            if att then
                                local targetAtt = targetPart:FindFirstChild(att.Name) or targetPart:FindFirstChildWhichIsA("Attachment")
                                if targetAtt then
    ...
    if success then
        ...
        NotifyUser("❌ Error", "No compatible o ID errónea.")
    end
end
...
RefreshEquippedItems = function()
    ...
    for _, item in ipairs(items) do
        ...
        if (not eqId or eqId == 0) and item:IsA("Accessory") then 
            eqId = item.SourceAssetId 
        end
        
        if eqId and tonumber(eqId) and tonumber(eqId) > 0 then
...
local Window = Rayfield:CreateWindow({
   Name = "ALB8RAAQ",
   LoadingTitle = "Actualizando Soporte Reset Default & Animaciones...",
