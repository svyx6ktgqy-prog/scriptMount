-- ==========================================================
-- MENU DE AVATARES QUIRÚRGICO Y PRO (v22 ACTUALIZADA)
-- Animaciones + Fix Ropa 3D + Auto-Ocultar Piernas + ESCALADO/OFFSET
-- ==========================================================
...
-- ==========================================================
-- 3. CEREBRO V21: FIX ROPA 3D PERFECTA + ANIMACIONES + TOOLS
-- ==========================================================
...
-- ==========================================================
-- [ NUEVO ] SISTEMA DE EDICIÓN DE ÍTEMS EN TIEMPO REAL
-- ==========================================================
local EditPanel = Instance.new("Frame")
EditPanel.Size = UDim2.new(0, 320, 0, 240)
EditPanel.Position = UDim2.new(0.5, 0, 0.5, 0)
EditPanel.AnchorPoint = Vector2.new(0.5, 0.5)
EditPanel.BackgroundColor3 = Color3.fromRGB(35, 35, 35)
...
EditCloseBtn.MouseButton1Click:Connect(function()
    EditPanel.Visible = false
    EqPanel.Visible = true -- Reabre el menú principal al cerrar
end)

local ActiveEditItem = nil
local OriginalWeldC1 = nil
local OrigMeshScale = nil
local EditOffsetX = 0
local EditOffsetY = 0
local EditScale = 1

local function ApplyTransformations()
    if not ActiveEditItem then return end
    local handle = ActiveEditItem:FindFirstChild("Handle")
    if handle then
        local weld = handle:FindFirstChild("AccessoryWeld")
        if weld and OriginalWeldC1 then
            -- Aplica los desplazamientos usando el sistema de coordenadas locales
            weld.C1 = OriginalWeldC1 * CFrame.new(-EditOffsetX, -EditOffsetY, 0)
        end
        local mesh = handle:FindFirstChildWhichIsA("DataModelMesh")
        if mesh and OrigMeshScale then
            mesh.Scale = OrigMeshScale * EditScale
        end
    end
end
...
BtnLeft.Size = UDim2.new(0, 50, 0, 40)
BtnLeft.Position = UDim2.new(0.2, -25, 0.25, 0)
BtnLeft.BackgroundColor3 = Color3.fromRGB(50, 50, 50)
BtnLeft.Text = "⏮️"
BtnLeft.TextSize = 20
BtnLeft.Parent = EditPanel
local BtnLeftCorner = Instance.new("UICorner"); BtnLeftCorner.CornerRadius = UDim.new(0, 6); BtnLeftCorner.Parent = BtnLeft
BtnLeft.MouseButton1Click:Connect(function() EditOffsetX -= 0.15; ApplyTransformations() end)

local BtnRight = Instance.new("TextButton")
BtnRight.Size = UDim2.new(0, 50, 0, 40)
BtnRight.Position = UDim2.new(0.8, -25, 0.25, 0)
BtnRight.BackgroundColor3 = Color3.fromRGB(50, 50, 50)
BtnRight.Text = "⏭️"
BtnRight.TextSize = 20
BtnRight.Parent = EditPanel
local BtnRightCorner = Instance.new("UICorner"); BtnRightCorner.CornerRadius = UDim.new(0, 6); BtnRightCorner.Parent = BtnRight
BtnRight.MouseButton1Click:Connect(function() EditOffsetX += 0.15; ApplyTransformations() end)

-- Sistema de Sliders Personalizados
local function CreateSlider(name, yPos, minVal, maxVal, defaultVal, callback)
    ...
    return function(resetVal)
        local pct = (resetVal - minVal)/(maxVal - minVal)
        Fill.Size = UDim2.new(pct, 0, 1, 0)
    end
end

local ResetScaleSlider = CreateSlider("Escala (Tamaño)", 125, 0.1, 3, 1, function(val)
    EditScale = val
    ApplyTransformations()
end)

local ResetYSlider = CreateSlider("Posición (Arriba / Abajo)", 185, -3, 3, 0, function(val)
    EditOffsetY = val
    ApplyTransformations()
end)
...
local function OpenEditMenuFor(item)
    ...
    local mesh = handle:FindFirstChildWhichIsA("DataModelMesh")
    if mesh then OrigMeshScale = mesh.Scale else OrigMeshScale = nil end
...
-- ==========================================================
-- FUNCIÓN DE REFRESCO Y BOTONES CON DETECCIÓN DE MANTENER
-- ==========================================================
RefreshEquippedItems = function()
    ...
    for _, item in ipairs(items) do
        ...
        -- [ NUEVO ] DETECTOR DE CLIC vs MANTENER PRESIONADO
        ...
        btn.InputBegan:Connect(function(input)
            if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
                ...
                -- Hilo separado para detectar el hold temporal de 0.5s
                task.spawn(function()
                    while isPressing do
                        if tick() - holdStart >= 0.5 then
                            isPressing = false -- Cancela el "desequipar"
        ...
        btn.InputEnded:Connect(function(input)
            if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
                if isPressing then
                    ...
                    -- Si se soltó antes de 0.5 segundos, hace la función normal (Desequipar)
