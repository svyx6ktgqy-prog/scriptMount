-- ==========================================================
-- MENU DE AVATARES QUIRÚRGICO Y PRO (v23 ACTUALIZADA)
-- Anti-Congelamiento + CHARACTERS.json + Gestos de Botón Flotante
-- ==========================================================
...
-- ==========================================================
-- 3. CEREBRO ASÍNCRONO ANTI-CONGELAMIENTO (FIX LAG & ANIMACIONES)
-- ==========================================================
local function UniversalEquip(assetId, isReequipping)
    -- Se ejecuta en un hilo separado para no congelar la UI ni el juego
    task.spawn(function()
        local numericId = tonumber(assetId)
        ...
        -- Carga de objetos no bloqueante
        local objects = nil
        ...
        if not getSuccess or not objects or #objects == 0 then
            -- Tratamiento de animaciones con pausa yield para evitar congelar Roblox
            task.spawn(function()
        ...
        for _, mainItem in ipairs(objects) do 
            ...
            task.wait(0.02) -- Ceder tiempo de procesador entre objetos para evitar micro-congelamientos
        end
...
CharMenu.Size = UDim2.new(0, 380, 0, 350)
...
CharTitle.Text = "📁 Personajes Guardados (CHARACTERS.json)"
...
NameInput.Size = UDim2.new(0, 230, 0, 32)
...
SaveCharBtn.Position = UDim2.new(1, -125, 0, 45)
...
CharScroll.Size = UDim2.new(1, -30, 1, -95)
CharScroll.Position = UDim2.new(0, 15, 0, 85)
...
CharGrid.CellSize = UDim2.new(0, 105, 0, 130)
...
local function RefreshSavedCharactersGrid()
    ...
    for charName, itemIds in pairs(data) do
        ...
        PreviewImg.Size = UDim2.new(1, -10, 0, 70)
        PreviewImg.Position = UDim2.new(0, 5, 0, 5)
        PreviewImg.BackgroundColor3 = Color3.fromRGB(20, 20, 20)
        PreviewImg.Image = "rbxthumb://type=AvatarHeadShot&id=" .. tostring(LocalPlayer.UserId) .. "&w=150&h=150"
        PreviewImg.Parent = Card
        local ImgCorner = Instance.new("UICorner"); ImgCorner.CornerRadius = UDim.new(0, 6); ImgCorner.Parent = PreviewImg

        local LabelName = Instance.new("TextLabel")
        LabelName.Size = UDim2.new(1, -10, 0, 18)
        LabelName.Position = UDim2.new(0, 5, 0, 78)
...
SaveCharBtn.MouseButton1Click:Connect(function()
    ...
    NameInput.Text = ""
    RefreshSavedCharactersGrid()
    NotifyUser("Guardado Exitoso", "Personaje '" .. name .. "' guardado en CHARACTERS.json")
end)
...
-- ==========================================================
-- 5. SISTEMA KITTY CATALOG UI + INTERACCIÓN AVANZADA BOTÓN
-- ==========================================================
...
-- LÓGICA DE GESTOS BOTÓN FLOTANTE (ARRASTRAR vs CLICK vs MANTENER)
...
UserInputService.InputEnded:Connect(function(input)
    if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
        ...
        -- Si fue un click corto simple (sin soltar tras arrastrar y sin haber hecho long press)
...
local Window = Rayfield:CreateWindow({
   Name = "🏥 Avatar Catalog Quirúrgico Pro v23",
   LoadingTitle = "Actualizando Soporte Characters JSON & Anti-Freeze...",
