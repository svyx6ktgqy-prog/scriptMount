-- ==========================================================
-- MENU DE AVATARES QUIRÚRGICO Y PRO (v25.3 ANTI-FREEZE & ULTRA-ASYNC)
...
local PromptPurchaseRemote = ReplicatedStorage:WaitForChild("PromptPurchaseRemote", 5)
...
local function ResetToDefaultAvatar()
    ...
    for _, part in ipairs(Char:GetChildren()) do
        if part:IsA("BasePart") then
            ...
            if string.find(pName, "leg") or string.find(pName, "foot") then part.Transparency = 0 end
        end
    end
...
SaveCharBtn.MouseButton1Click:Connect(function()
    ...
    if #itemsToSave == 0 then NotifyUser("Atención", "No tienes ítems equipados para guardar."); return end
...
-- ==========================================================
-- VISUALIZADOR PREVIEW CORREGIDO (Long-Press / Normal Tap)
-- ==========================================================
	...
	if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
		...
		task.spawn(function()
			while isPreviewPressing do
				-- Si se mantiene por 0.55 segundos, se activa la compra
...
-- ==========================================================
-- KITTY TOP BAR Y SUS ELEMENTOS (SLIDER Y HOME APLICADOS)
-- ==========================================================
...
local KittySearchLimit = 30 -- Límite global por defecto
...
local KittySearch = Instance.new("TextBox") -- Referencia temprana

-- Método Home aislado solicitado
local function PerformHomeSearch()
    ...
    local limit = math.clamp(KittySearchLimit, KITTY_MIN_LIMIT, KITTY_MAX_LIMIT)

    local url = "https://catalog.roblox.com/v1/search/items/details?category="..tostring(KittyCurrentCategory).."&limit="..tostring(limit).."&keyword=" .. HttpService:UrlEncode(KittySearch.Text)
    local success, response = pcall(function() return game:HttpGet(url) end)
    
    if not success or not response then
        url = "https://catalog.roproxy.com/v1/search/items/details?category="..tostring(KittyCurrentCategory).."&limit="..tostring(limit).."&keyword=" .. HttpService:UrlEncode(KittySearch.Text)
...
-- ==========================================================
-- CONTENEDOR DE LÍMITE CON RESTRICCIÓN ESTRICTA (10 a 30)
-- ==========================================================
...
local MaxItemsSliderBg = Instance.new("Frame")
MaxItemsSliderBg.Size = UDim2.new(0.65, 0, 0, 8)
MaxItemsSliderBg.Position = UDim2.new(0, 0, 0, 22)
MaxItemsSliderBg.BackgroundColor3 = Color3.fromRGB(255, 192, 203)
MaxItemsSliderBg.Parent = MaxItemsFrame
local SliderBgCorner = Instance.new("UICorner"); SliderBgCorner.CornerRadius = UDim.new(1, 0); SliderBgCorner.Parent = MaxItemsSliderBg

local MaxItemsFill = Instance.new("Frame")
MaxItemsFill.Size = UDim2.new(1, 0, 1, 0) -- 100% (30 de 30)
MaxItemsFill.BackgroundColor3 = Color3.fromRGB(255, 20, 147)
MaxItemsFill.Parent = MaxItemsSliderBg
local FillCorner = Instance.new("UICorner"); FillCorner.CornerRadius = UDim.new(1, 0); FillCorner.Parent = MaxItemsFill

local MaxItemsKnob = Instance.new("TextButton")
MaxItemsKnob.Size = UDim2.new(0, 16, 0, 16)
MaxItemsKnob.Position = UDim2.new(1, -8, 0.5, -8)
MaxItemsKnob.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
MaxItemsKnob.Text = ""
MaxItemsKnob.Parent = MaxItemsFill
local KnobCorner = Instance.new("UICorner"); KnobCorner.CornerRadius = UDim.new(1, 0); KnobCorner.Parent = MaxItemsKnob

local MaxItemsInputBox = Instance.new("TextBox")
MaxItemsInputBox.Size = UDim2.new(0.28, 0, 0, 22)
MaxItemsInputBox.Position = UDim2.new(0.72, 0, 0, 15)
MaxItemsInputBox.BackgroundColor3 = Color3.fromRGB(255, 240, 245)
...
MaxItemsInputBox.TextSize = 11
MaxItemsInputBox.Parent = MaxItemsFrame
local InputBoxCorner = Instance.new("UICorner"); InputBoxCorner.CornerRadius = UDim.new(0, 4); InputBoxCorner.Parent = MaxItemsInputBox

local draggingMax = false
MaxItemsKnob.InputBegan:Connect(function(input)
    if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then draggingMax = true end
end)
UserInputService.InputEnded:Connect(function(input)
    if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then draggingMax = false end
end)
UserInputService.InputChanged:Connect(function(input)
    if draggingMax and (input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch) then
        local relativeX = math.clamp(input.Position.X - MaxItemsSliderBg.AbsolutePosition.X, 0, MaxItemsSliderBg.AbsoluteSize.X)
        local pct = relativeX / MaxItemsSliderBg.AbsoluteSize.X
        MaxItemsFill.Size = UDim2.new(pct, 0, 1, 0)
        
        local val = math.floor(KITTY_MIN_LIMIT + (pct * (KITTY_MAX_LIMIT - KITTY_MIN_LIMIT)))
        MaxItemsInputBox.Text = tostring(val)
        MaxItemsLabel.Text = "Límite: " .. tostring(val)
        KittySearchLimit = val
    end
end)

-- Aquí nos aseguramos que si el usuario escribe un número lo restrinja entre 10 y 30.
MaxItemsInputBox.FocusLost:Connect(function()
    local val = tonumber(MaxItemsInputBox.Text)
    if val then
        val = math.clamp(val, KITTY_MIN_LIMIT, KITTY_MAX_LIMIT)
    else
        val = KITTY_MIN_LIMIT
    end
    ...
    MaxItemsLabel.Text = "Límite: " .. tostring(val)
    KittySearchLimit = val
    MaxItemsFill.Size = UDim2.new((val - KITTY_MIN_LIMIT) / (KITTY_MAX_LIMIT - KITTY_MIN_LIMIT), 0, 1, 0)
end)

-- Barra de Busqueda Estándar
local SearchContainer = Instance.new("Frame")
SearchContainer.Size = UDim2.new(0.36, 0, 0, 40)
