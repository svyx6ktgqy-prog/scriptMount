-- ==========================================================
-- MENU DE AVATARES QUIRÚRGICO Y PRO (v21.1 DEFINITIVA)
-- Animaciones + Fix Ropa 3D + Auto-Ocultar Piernas + Adherencia Perfecta
-- ==========================================================
...
-- ==========================================================
-- 3. CEREBRO V21.1: FIX ROPA 3D + ADHERENCIA PERFECTA Y ESCALADO
-- ==========================================================
local function UniversalEquip(assetId, isReequipping)
    ...
    local success, err = pcall(function()
        ...
        local function ProcessItem(item)
            ...
            -- =========================================================
            -- ACCESORIOS Y ROPA 3D: ADHERENCIA Y ALINEACIÓN DE PRECISIÓN
            -- =========================================================
            if item:IsA("Accessory") then
                ...
                if handle then
                    ...
                    -- Preservar la escala original del Mesh o del Part sin distorsiones
                    local mesh = handle:FindFirstChildWhichIsA("SpecialMesh") or handle:FindFirstChildWhichIsA("BlockMesh")
                    if mesh then
                        mesh.Scale = mesh.Scale -- Mantiene adherencia limpia
                    end
                    ...
                    if accAttach then
                        ...
                        if targetAttach then
                            ...
                            -- Encaje directo de CFrame para adherencia milimétrica
                            handle.CFrame = targetAttach.WorldCFrame * accAttach.CFrame:Inverse()
                        ...
                        if bodyTarget then
                            for _, v in ipairs(handle:GetJoints()) do v:Destroy() end
                            handle.CFrame = bodyTarget.CFrame
                    ...
                    if wrapLayer then
                        -- Adherencia y adaptabilidad perfeccionada para WrapLayer (Ropa 3D)
                        pcall(function()
                            wrapLayer.AutoSkin = Enum.AutoSkin.Enabled
                        end)
                        ...
                        -- Detectar si es pantalón y ocultar piernas base
                        local isPant = false
                        pcall(function()
...
RefreshEquippedItems = function()
    ...
    for _, item in ipairs(items) do
        ...
        btn.MouseButton1Click:Connect(function()
            ...
            -- Restaurar piernas si el item las ocultó
...
local Window = Rayfield:CreateWindow({
   Name = "🏥 Avatar Catalog Quirúrgico Pro v21.1",
