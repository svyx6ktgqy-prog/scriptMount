-- ==========================================================
-- MENU DE AVATARES QUIRÚRGICO Y PRO (v24.0 FIX MINIATURA & CORNER CLIPPING)
...
local function RefreshSavedCharactersGrid()
    ...
    for charName, itemEntries in pairs(data) do
        ...
        task.spawn(function()
            ...
            local function ProcessDummyItem(item)
                ...
                if item:IsA("Accessory") then
                    ...
                    if handle then
                        ...
                        -- FIX 1: Ocultar piernas en el dummy si equipa pantalones 3D o ropa que oculte extremidades
                        local isPant = false
                        pcall(function()
                            if item.AccessoryType == Enum.AccessoryType.Pants or item.AccessoryType == Enum.AccessoryType.Shorts then
                                isPant = true
            ...
            -- FIX 1: Forzar sincronización de marco para consolidar geometrías/transparencia antes de la foto
...
-- FIX 2: Contenedor con curvatura para recortar los botones al deslizarse (Clipping Mask)
...
local Window = Rayfield:CreateWindow({
   Name = "🏥 Avatar Catalog Quirúrgico Pro v24.0",
