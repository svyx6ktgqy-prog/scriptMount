-- ==========================================================
-- MENU DE AVATARES QUIRÚRGICO Y PRO (v25.2 FRAME-SLICING & PRELOAD FIX)
-- Renderizado Asíncrono + Carga Escalonada + Sistema Anti-Freeze
-- ==========================================================
...
local BannerSystem = {
    ...
    LoadingQueue = {},
    IsProcessing = false,
    BatchSizePerFrame = 3, -- Cantidad de elementos visuales a procesar por frame
}

-- 1. Precarga Asíncrona sin bloqueos
function BannerSystem.PreloadBanners(bannerList)
    task.spawn(function()
        for i, bannerData in ipairs(bannerList) do
            ...
            -- Ceder el control al motor cada N elementos para mantener 60 FPS
...
-- 2. Procesamiento Diferido (Frame Slicing)
    ...
    task.spawn(function()
        while #BannerSystem.LoadingQueue > 0 do
            ...
            while #BannerSystem.LoadingQueue > 0 and batchCount < BannerSystem.BatchSizePerFrame do
                ...
                -- Render/Creación del elemento visual
            ...
            -- Pausa de 1 frame para sostener la fluidez del hilo principal
            task.wait()
        end
...
function BannerSystem.RenderBanner(bannerData)
    ...
    -- Carga asíncrona aislada del asset de imagen
    task.spawn(function()
        if bannerData.Id then
...
-- ==========================================================
-- PRECARGA E INICIALIZACIÓN GENERAL EN SEGUNDO PLANO
-- ==========================================================
...
task.spawn(function()
    ...
    if isfile and isfile(CHARS_FILE) then
        pcall(function()
            HttpService:JSONDecode(readfile(CHARS_FILE))
        end)
    end
...
-- ==========================================================
-- 1. RESTAURACIÓN A AVATAR DEFAULT
-- ==========================================================
...
-- ==========================================================
-- SISTEMA DE PERSONAJES GUARDADOS
-- ==========================================================
...
-- ==========================================================
-- 5. SISTEMA KITTY CATALOG UI (INTEGRADO CON BANNER SYSTEM)
-- ==========================================================
...
-- Búsqueda Integrada con Procesamiento Diferido Frame Slicing
local function PerformKittySearch()
    ...
    if success and response then
        ...
        if decoded and decoded.data then
            ...
            -- Procesa los ítems del catálogo mediante Frame-Slicing
...
local Window = Rayfield:CreateWindow({
   Name = "🏥 Avatar Catalog Quirúrgico Pro v25.2 Anti-Freeze",
