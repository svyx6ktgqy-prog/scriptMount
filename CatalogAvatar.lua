-- ==========================================================
-- MENU DE AVATARES QUIRÚRGICO Y PRO (UNIFICADO v16 DEFINITIVO)
-- Fix UI + Fix Ropa 3D + Menú de Equipados
-- ==========================================================
...
-- ==========================================================
-- 1. SOLUCIÓN EXTREMA: GENERADOR QUIRÚRGICO
-- ==========================================================
...
-- ==========================================================
-- 3. CEREBRO V16: FIX ROPA 3D (Layered) Y SOLDADURA 
-- ==========================================================
local function UniversalEquip(assetId)
    ...
    local success, err = pcall(function()
        ...
        local function ProcessItem(item)
            ...
            if item:IsA("Accessory") then
                ...
                cloneItem:SetAttribute("AssetId", assetId) -- Guarda el ID para el menú de Equipados
                ...
                if handle then
                    ...
                    -- A. Fix de Layered Clothing (Chaquetas, Ropa 3D, Zapatos)
                    local wrap = handle:FindFirstChildWhichIsA("WrapLayer")
                    if wrap then
                        wrap.Enabled = true
                        wrap.AutoJoints = true
                        wrap.ShrinkFactor = 0
                        -- ⚠️ EVITAMOS soldar si es Ropa 3D porque rompe la malla. Se equipa natural:
            ...
            -- C. Fix para Ropa Clásica 2D (Classic Shirts, Classic Pants, T-Shirts)
            elseif item:IsA("Shirt") or item:IsA("Pants") or item:IsA("ShirtGraphic") then
                ...
                local cloneItem = item:Clone()
                cloneItem:SetAttribute("AssetId", assetId)
                cloneItem.Parent = Char

            -- D. Fix para Cabezas MeshPart (Heads / Faces)
    ...
    if not success then
        ...
        if Backpack then
            ...
            NotifyUser("⚠️ Contingencia Activada", "Se ha inyectado el equipo de reemplazo.")
        end
    end
end
...
-- MENÚ DE ITEMS EQUIPADOS (Panel Centrado)
...
-- LOGO EYES BUTTON (Abajo del visualizador centrado)
...
-- ==========================================================
-- 5. SISTEMA KITTY CATALOG UI (MENÚ Y BOTÓN FLOTANTE)
-- ==========================================================
    ...
    if success and response then
        ...
        if decoded and decoded.data then
            for _, item in ipairs(decoded.data) do
                ...
                -- SOLO ACTUALIZA VISUALIZADOR. (NO EQUIPA DIRECTO DESDE AQUI)
...
-- ==========================================================
-- LÓGICA DE DICCIONARIO Y RAYFIELD (Sin cambios)
-- ==========================================================
...
local Window = Rayfield:CreateWindow({
   Name = "🏥 Avatar Catalog Quirúrgico Pro v16",
   LoadingTitle = "Motor C0/C1 y Ropa 3D Activo...",
   LoadingSubtitle = "Unificación Definitiva Completada",
...
Panel:CreateButton({
   Name = "⚡ Equipar/Probar (Motor Quirúrgico C0/C1)",
   Callback = function()
