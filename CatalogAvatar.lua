-- ==========================================================
-- MENU DE AVATARES QUIRÚRGICO Y PRO (FIX CAÍDA DE OBJETOS)
-- Animaciones + Fix Ropa 3D + Auto-Ocultar Piernas + Soldadura de Respaldo
-- ==========================================================
...
-- ==========================================================
-- 3. CEREBRO: FIX ADHERENCIA Y SOLDADURA ANTI-CAÍDAS
-- ==========================================================
local function UniversalEquip(assetId, isReequipping)
    ...
    local success, err = pcall(function()
        ...
        local function ProcessItem(item)
            ...
            -- =========================================================
            -- ACCESORIOS Y ROPA 3D: ADHERENCIA GARANTIZADA SIN CAÍDAS
            -- =========================================================
            if item:IsA("Accessory") then
                ...
                if handle then
                    -- Desactivar colisiones y masa en TODAS las piezas para que la física no los tire
                    ...
                    if wrapLayer then
                        ...
                        -- Ocultar piernas si es pantalón 3D
                        local isPant = false
                        pcall(function()
                    ...
                    -- 1. Intentar equipar mediante el motor de Roblox
                    Hum:AddAccessory(cloneItem)

                    -- 2. VERIFICACIÓN Y SOLDADURA DE RESPALDO (Evita que el item caiga al suelo)
                    task.defer(function()
                        if handle and handle.Parent then
                            ...
                            if not isWelded then
                                ...
                                if att then
                                    local partName = string.gsub(att.Name, "Attachment", "")
                                    if Char:FindFirstChild(partName) then
                                        targetPart = Char[partName]
                                    end
                                else
                                    local nameLower = string.lower(cloneItem.Name)
                                    if string.find(nameLower, "neck") or string.find(nameLower, "chain") or string.find(nameLower, "collar") then
                                        targetPart = Char:FindFirstChild("UpperTorso") or Char:FindFirstChild("Torso") or Char:FindFirstChild("Head")
                                    elseif string.find(nameLower, "wrist") or string.find(nameLower, "bracelet") or string.find(nameLower, "pulsera") then
                                        targetPart = Char:FindFirstChild("RightHand") or Char:FindFirstChild("Right Arm") or Char:FindFirstChild("RightLowerArm")
                                    end
                                end
