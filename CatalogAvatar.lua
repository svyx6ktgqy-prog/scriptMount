-- ==========================================================
-- MENU DE AVATARES QUIRÚRGICO Y PRO (v21 DEFINITIVA)
-- Animaciones + Fix Ropa 3D + Auto-Ocultar Piernas + Fix Collares/Pulseras
-- ==========================================================
...
local function ResetToDefaultAvatar()
    ...
    -- Restaurar la visibilidad de las piernas y pies
...
local function UniversalEquip(assetId, isReequipping)
    ...
    local success, err = pcall(function()
        ...
        local function ProcessItem(item)
            ...
            -- =========================================================
            -- SOPORTE ACCESORIOS, COLLARES, PULSERAS Y ROPA 3D FIX
            -- =========================================================
                ...
                if handle then
                    ...
                    if accAttach then
                        ...
                        if targetAttach then
                            ...
                            -- AJUSTE QUIRÚRGICO DE ESPACIOS PARA COLLARES Y PULSERAS
                            local offsetCFrame = CFrame.new()
                            local attNameLower = string.lower(accAttach.Name)
                            local itemNameLower = string.lower(item.Name)

                            -- Collares/cadenas no adheridos al cuello
                            if string.find(attNameLower, "neck") or string.find(itemNameLower, "collar") or string.find(itemNameLower, "neck") or string.find(itemNameLower, "chain") or string.find(itemNameLower, "cadena") then
                                offsetCFrame = CFrame.new(0, -0.06, -0.04)
                            -- Pulseras y brazaletes
                            elseif string.find(attNameLower, "wrist") or string.find(attNameLower, "arm") or string.find(itemNameLower, "bracelet") or string.find(itemNameLower, "pulsera") or string.find(itemNameLower, "brazalete") then
                                offsetCFrame = CFrame.new(0, -0.02, 0)
                            end

                            weld.C0 = targetAttach.CFrame * offsetCFrame
                            weld.C1 = accAttach.CFrame
                            weld.Parent = handle
                        end
                    else
                        -- Fallback mejorado si el accesorio no trae Attachment estándar
                        local itemNameLower = string.lower(item.Name)
                        local bodyTarget = nil
                        local fallbackOffset = CFrame.new()

                        if string.find(itemNameLower, "collar") or string.find(itemNameLower, "neck") or string.find(itemNameLower, "chain") or string.find(itemNameLower, "cadena") then
                            bodyTarget = Char:FindFirstChild("UpperTorso") or Char:FindFirstChild("Torso")
                            fallbackOffset = CFrame.new(0, 0.45, -0.05)
                        elseif string.find(itemNameLower, "bracelet") or string.find(itemNameLower, "pulsera") then
                            bodyTarget = Char:FindFirstChild("RightLowerArm") or Char:FindFirstChild("LeftLowerArm") or Char:FindFirstChild("RightHand")
                        else
                            bodyTarget = Char:FindFirstChild("UpperTorso") or Char:FindFirstChild("LowerTorso") or Char:FindFirstChild("Torso") or Char.PrimaryPart
                        end
...
local Window = Rayfield:CreateWindow({
   Name = "🏥 Avatar Catalog Quirúrgico Pro v21",
