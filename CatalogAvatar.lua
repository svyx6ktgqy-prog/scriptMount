...
-- ==========================================================
-- VISUALIZADOR PREVIEW CORREGIDO (Long-Press Global Purchase)
-- ==========================================================
	...
	if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
		...
		task.spawn(function()
			while isPreviewPressing do
				-- Si se mantiene por 0.55 segundos, se activa la compra de forma global
				if tick() - previewHoldStart >= 0.55 then
					...
					if assetId and assetId > 0 then
						-- Llamada nativa y directa de MarketplaceService (Funciona en todos lados)
						pcall(function()
							MarketplaceService:PromptPurchase(LocalPlayer, assetId)
						end)
						if NotifyUser then
							NotifyUser("Obtención", "Abriendo menú de compra global para: " .. tostring(CurrentData.Name))
						end
					else
						if NotifyUser then
...
-- ==========================================================
-- CONTENEDOR DE LÍMITE (NUEVO SPINNER INTEGRADO 10 o 30 RÁPIDO)
-- ==========================================================
