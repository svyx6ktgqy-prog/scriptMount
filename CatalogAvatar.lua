-- ==========================================================
-- MENU DE AVATARES QUIRÚRGICO Y PRO (v23.3 FIX ESTÁTICO & SIZE)
-- Miniatura Estática x Outfit + Menú Delimitado
-- ==========================================================
...
-- CONTENEDOR DE SCROLL RIGIDO (NO EXPANDIBLE)
...
local function RefreshSavedCharactersGrid()
    ...
    for charName, itemIds in pairs(data) do
        ...
        -- CONSTRUCCIÓN DE MINIATURA 100% ESTÁTICA Y EXCLUSIVA PARA CADA PERSONAJE
        task.spawn(function()
            ...
            -- 1. Limpiar indumentaria previa del jugador para dejar el cuerpo base limpio
            for _, v in ipairs(dummy:GetChildren()) do
                if v:IsA("Accessory") or v:IsA("Shirt") or v:IsA("Pants") or v:IsA("ShirtGraphic") or v:IsA("Tool") or v:IsA("Script") or v:IsA("LocalScript") or v:IsA("Animator") then
                    v:Destroy()
                end
            end

            -- 2. Equipar ÚNICAMENTE los elementos guardados del outfit específico
...
local Window = Rayfield:CreateWindow({
   Name = "🏥 Avatar Catalog Quirúrgico Pro v23.3",
   LoadingTitle = "Actualizando Miniatura Estática y Size Fix...",
