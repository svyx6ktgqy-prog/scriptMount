-- =====================================================================
-- 💎 ALB8RAAQ SUPER HUB (Merged 7-in-1 Script)
...
local ReplicatedStorage = game:GetService("ReplicatedStorage") -- Added for Bombs & Outfits
...
-- =====================================================================
-- TAB 9: BOMB ARSENAL (Integrated from bombs.lua)
-- =====================================================================
local BombTab = Window:CreateTab("Bomb Arsenal 💣", 4483362458)

local InfiniteBombActive = false
local SelectedBombName = "Thunder Bomb" 
...
BombTab:CreateParagraph({
    Title = "Active Injection Path",
    Content = "Searching in: ReplicatedStorage.Assets.Bombs\nNote: Cloned objects are 'Client-Sided'. They require a network bypass to be thrown."
})

BombTab:CreateDropdown({
    Name = "Select Bomb Type",
    Options = BombTypes,
...
BombTab:CreateToggle({
   Name = "⚙️ Enable Eternal Ammo (Auto-Inject)",
   CurrentValue = false,
   Flag = "InfBomb",
   Callback = function(Value)
        ...
        if Value then
            Rayfield:Notify({Title = "Loop Started", Content = SelectedBombName .. " injected.", Duration = 3})
            InjectFreshBomb()
            ...
            Rayfield:Notify({Title = "Loop Stopped", Content = "Auto-generator turned off.", Duration = 3})
        end
   end,
})

BombTab:CreateButton({
   Name = "📦 Manually Inject x1 Bomb",
   Callback = function()
        local assets = ReplicatedStorage:FindFirstChild("Assets")
        ...
        if originalBomb then
            ...
            Rayfield:Notify({Title = "Bomb Cloned", Content = "Added: " .. SelectedBombName, Duration = 2})
        else
            Rayfield:Notify({Title = "Error", Content = "Bomb not found on the server.", Duration = 3})
        end
   end,
})

-- =====================================================================
-- TAB 10: MUTATIONS & CONFIG
-- =====================================================================
...
SecretTab:CreateSection("Visibility Bypass")

SecretTab:CreateButton({
   Name = "Force Open: Main Menu (Configure)",
   Callback = function()
       ...
       if configGui then
           ...
           if frame then
               frame.Visible = true
               Rayfield:Notify({Title = "Success", Content = "'Configure' menu revealed on screen.", Duration = 3})
           else
               Rayfield:Notify({Title = "Error", Content = "Main Frame not found.", Duration = 3})
           end
       else
           Rayfield:Notify({Title = "Not Found", Content = "Configure menu is not loaded in your PlayerGui.", Duration = 3})
       end
   end,
})

SecretTab:CreateButton({
   Name = "Force Open: Mutations Panel",
   Callback = function()
       local configGui = GetSecretGUI()
       if configGui and configGui:FindFirstChild("Frame") then
           ...
           if mutFrame then 
               ...
               Rayfield:Notify({Title = "Mutations Opened", Content = "Mutations panel is now visible.", Duration = 3})
           end
       end
   end,
})

SecretTab:CreateSection("Direct Actions (Log Replicators)")

local mutations = {"Wet", "Fire", "Aurora"}

for _, mutName in ipairs(mutations) do
    SecretTab:CreateButton({
       Name = "Activate Mutation: " .. mutName,
       Callback = function()
           local configGui = GetSecretGUI()
           if configGui then
               pcall(function()
                   ...
                   if btn then
                       ...
                       Rayfield:Notify({Title = "Button Revealed", Content = "Click on " .. mutName .. " on the screen.", Duration = 3})
                   else
                       Rayfield:Notify({Title = "Error", Content = "The button " .. mutName .. " does not exist or hasn't loaded yet.", Duration = 3})
                   end
               end)
           end
       end,
...
SecretTab:CreateSection("Utilities")

SecretTab:CreateButton({
   Name = "Reveal Hidden Sell Button",
   Callback = function()
       pcall(function()
           local sellGui = LocalPlayer.PlayerGui:FindFirstChild("Sell")
           if sellGui and sellGui:FindFirstChild("Frame") then
               ...
               Rayfield:Notify({Title = "Sell UI Revealed", Content = "The sell menu has been forced to become visible.", Duration = 3})
           end
       end)
   end,
})
