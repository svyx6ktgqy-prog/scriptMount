-- Universal Basketball 0.2 Script (Fixed for Delta iOS & Rayfield UI + Dynamic Auto-Aim, Steal Engine & Corrupt Magnet Steal)
...
-- ==========================================
-- FUNCIONES MOBILE GUI & TOUCH ENGINE REAL
-- ==========================================
...
local function simulateButtonDown(button)
	...
	VirtualInputManager:SendTouchEvent(1, Enum.UserInputState.Begin, center.X, center.Y)
end

local function simulateButtonUp(button)
	if not button then return end
	local center = button.AbsolutePosition + (button.AbsoluteSize / 2)
	VirtualInputManager:SendTouchEvent(1, Enum.UserInputState.End, center.X, center.Y)
end
...
local function simulateButtonTap(button)
	...
	-- Respaldo directo de señales por si el evento UI es ignorado por la cámara
	if firesignal then
...
-- Disparo directo a Remotes de Robo si existen
local function triggerDirectStealRemote()
	for _, obj in ipairs(Replicated:GetDescendants()) do
		if obj:IsA("RemoteEvent") and (obj.Name:find("Steal") or obj.Name:find("Reach") or obj.Name:find("Defense")) then
...
TabMobile:CreateToggle({
	Name = "⚡ Auto Steal Eficiente (Físico)",
	CurrentValue = false,
...
TabMobile:CreateToggle({
	Name = "🧲 Magnet Steal + Ball (Corrupt Mode)",
	CurrentValue = false,
	Flag = "MobileCorruptStealToggle",
...
task.spawn(function()
	...
	while task.wait(0.08) do
		...
		-- BUCLE AUTO STEAL ESTÁNDAR
		if Mobile_AutoSteal and hrp then
			local stealBtn = getButton({"Defense", "Steal"}) or getButton({"Defense", "Reach"})
			for _, p in ipairs(Players:GetPlayers()) do
				if p ~= player and p.Character and p.Character:FindFirstChild("HumanoidRootPart") then
					local ball = p.Character:FindFirstChild("Basketball") or p.Character:FindFirstChild("Ball") or p.Character:FindFirstChildOfClass("Tool")
					if ball then
						...
						if dist <= Mobile_StealDist then
							...
							if stealBtn then simulateButtonTap(stealBtn) end
							triggerDirectStealRemote()
							task.wait(0.2)
							break
						end
					end
		...
		-- BUCLE MAGNET STEAL + BALL (CORRUPT MODE)
		if Mobile_CorruptSteal and hrp then
			...
			for _, p in ipairs(Players:GetPlayers()) do
				if p ~= player and p.Character and p.Character:FindFirstChild("HumanoidRootPart") then
					...
					-- Buscar también si la pelota es un objeto físico adjunto
					if not ball then
						for _, child in ipairs(p.Character:GetChildren()) do
							if child:IsA("BasePart") and (child.Name:find("Ball") or child.Name:find("Basketball")) then
								ball = child
								break
							end
						end
					...
					if ball then
						local ballPos = ball:IsA("BasePart") and ball.Position or (ball:FindFirstChild("Handle") and ball.Handle.Position) or p.Character.HumanoidRootPart.Position
						local dist = (hrp.Position - ballPos).Magnitude

						if dist <= 12 then
							-- Fuerza orientación física directa a la pelota del enemigo
							hrp.CFrame = CFrame.lookAt(hrp.Position, Vector3.new(ballPos.X, hrp.Position.Y, ballPos.Z))
							
							-- Simulación de imán físico en el brazo/cuerpo
							local rightArm = character:FindFirstChild("Right Arm") or character:FindFirstChild("RightHand")
							if rightArm then
								pcall(function() rightArm.CFrame = CFrame.new(ballPos) end)
							end

							if stealBtn then simulateButtonTap(stealBtn) end
							triggerDirectStealRemote()
							
							-- Forzar toque de proximidad directo sobre la pelota si existe
