-- Universal Basketball 0.2 Script (Fixed for Delta iOS & Rayfield UI + Dynamic Auto-Aim, Steal & Smart Block)

-- ==========================================
-- SERVICIOS Y JUGADOR (Cargados primero para evitar bugs de UI)
-- ==========================================
...
local Mobile_AutoSteal = false
local Mobile_CorruptSteal = false 
local Mobile_SmartBlock = false
local Mobile_StealDist = 7
local Mobile_RimBlockDist = 18
...
-- ==========================================
-- INTERFAZ RAYFIELD (Generada de un tirón para evitar pestañas vacías)
-- ==========================================
...
-- ==========================================
-- BÚSQUEDA DINÁMICA DE ARO MÁS CERCANO (AUTO-AIM)
-- ==========================================
local function getClosestRim()
	local closestRim = nil
	local shortestDistance = math.huge

	for _, obj in ipairs(Workspace:GetDescendants()) do
		if obj.Name == "M_BB_Rim" and obj:IsA("BasePart") then
...
local function alignCharacterToRim(targetRim)
	if not targetRim or not hrp then return end
	local rimPos = targetRim.Position
	local targetLookVector = Vector3.new(rimPos.X, hrp.Position.Y, rimPos.Z)
	hrp.CFrame = CFrame.lookAt(hrp.Position, targetLookVector)
end
...
MangoImage.Size = UDim2.new(1.0, 0, 1.0, 0) -- 40% más grande que el círculo
...
-- ==========================================
-- ELEMENTOS DE LA INTERFAZ (RAYFIELD)
-- ==========================================
...
TabMobile:CreateToggle({
	...
	Callback = function(Value) ForcePerfectGreen = Value end,
})
...
TabMobile:CreateToggle({
	...
	Callback = function(Value) FloatBtn.Visible = Value end,
})

TabMobile:CreateSection("Defensa Automática Avanzada (NUEVO)")
TabMobile:CreateToggle({
	Name = "⚡ Auto Steal Eficiente (Toque Natural)",
	CurrentValue = false,
	Flag = "MobileStealToggle",
	Callback = function(Value) Mobile_AutoSteal = Value end,
})

TabMobile:CreateToggle({
	Name = "😈 Steal + Ball (Robo Corrupto/Agresivo)",
	CurrentValue = false,
	Flag = "MobileCorruptSteal",
	Callback = function(Value) Mobile_CorruptSteal = Value end,
})

TabMobile:CreateToggle({
	Name = "🛡️ Smart Auto Block / Rim Check",
	CurrentValue = false,
	Flag = "MobileBlockToggle",
	Callback = function(Value) Mobile_SmartBlock = Value end,
})

TabMobile:CreateSection("Acciones de GUI (Simulate Touch)")
TabMobile:CreateToggle({ Name = "🏀 Auto-Shoot Táctil Loop", CurrentValue = false, Flag = "MobileShootToggle", Callback = function(Value) Mobile_AutoShoot = Value end})
...
TabMobile:CreateToggle({ Name = "🏃 Auto-Sprint", CurrentValue = false, Flag = "MobileSprintToggle", Callback = function(Value) Mobile_AutoSprint = Value; if not Value then local sprintBtn = getButton({"Shared", "Sprint"}); if sprintBtn then simulateButtonUp(sprintBtn) end end end})
...
TabMobile:CreateButton({ Name = "💨 Forzar Dribble Rápido (S)", Callback = function() local dribbleBtn = getButton({"Offense", "OnBall", "DribbleMoveS"}); if dribbleBtn and dribbleBtn.Visible then simulateButtonDown(dribbleBtn); task.wait(0.05); simulateButtonUp(dribbleBtn) else Rayfield:Notify({Title = "Error", Content = "Necesitas el balón para hacer un dribble.", Duration = 2}) end end})

-- PESTAÑA UNIVERSAL
TabUniversal:CreateSection("Configuración de Tiro y Defensa")
TabUniversal:CreateButton({ Name = "📍 Marcar Aro", Callback = function() Uni_hoopPosition = hrp.Position; Rayfield:Notify({Title = "Universal", Content = "Posición del aro guardada.", Duration = 3}) end})
...
TabHoops:CreateButton({ Name = "📍 Marcar Aro", Callback = function() HL_hoopPos = hrp.Position; _G.HL_HoopPosition = HL_hoopPos; Rayfield:Notify({Title = "Hoops Life", Content = "Posición del aro guardada.", Duration = 3}) end})
...
TabHoops:CreateToggle({ Name = "🏀 Activar Auto Shoot (Remotes)", CurrentValue = false, Flag = "HLAutoShootToggle", Callback = function(Value) HL_autoShoot = Value; if Value then Rayfield:Notify({Title = "Auto Shoot", Content = "Activo.", Duration = 4}) end end})
TabHoops:CreateButton({ Name = "🔥 Disparar (Auto Green)", Callback = function() if HL_autoShoot then local shoot = Replicated:WaitForChild("Remotes"):WaitForChild("Shoot"); shoot:FireServer(0, "Starting", "up", false, 0, 0); task.wait(0.5); shoot:FireServer(100000, "Ending", "up", false, 0, 0) else Rayfield:Notify({Title = "Aviso", Content = "Activa Auto Shoot primero.", Duration = 3}) end end})
...
task.spawn(function()
	...
	while task.wait(0.1) do
		...
		-- BUCLE AUTO STEAL (REPARADO CON TOQUE NATURAL)
		if Mobile_AutoSteal and hrp then
			local stealBtn = getButton({"Defense", "Steal"}) or getButton({"Defense", "Reach"})
			if stealBtn and stealBtn.Visible then
				for _, p in ipairs(Players:GetPlayers()) do
					if p ~= player and p.Character and p.Character:FindFirstChild("HumanoidRootPart") then
						local hasBasketball = p.Character:FindFirstChild("Basketball") or p.Character:FindFirstChildOfClass("Tool")
						if hasBasketball then
							local enemyHrp = p.Character.HumanoidRootPart
							local dist = (hrp.Position - enemyHrp.Position).Magnitude
							if dist <= Mobile_StealDist then
								-- Alinear mirada
								local lookAtVector = Vector3.new(enemyHrp.Position.X, hrp.Position.Y, enemyHrp.Position.Z)
								hrp.CFrame = CFrame.lookAt(hrp.Position, lookAtVector)
								
								-- Toque Natural: Presiona, espera un instante, levanta
								simulateButtonDown(stealBtn)
								task.wait(0.05)
								simulateButtonUp(stealBtn)
		...
		-- BUCLE STEAL + BALL (ROBO CORRUPTO Y AGRESIVO)
		if Mobile_CorruptSteal and hrp then
			local stealBtn = getButton({"Defense", "Steal"}) or getButton({"Defense", "Reach"})
			for _, p in ipairs(Players:GetPlayers()) do
				if p ~= player and p.Character and p.Character:FindFirstChild("HumanoidRootPart") then
					local ball = p.Character:FindFirstChild("Basketball") or p.Character:FindFirstChildOfClass("Tool")
					if ball then
						local enemyHrp = p.Character.HumanoidRootPart
						local dist = (hrp.Position - enemyHrp.Position).Magnitude
						
						-- Rango más permisivo para forzar la acción
						if dist <= Mobile_StealDist * 1.8 then
							-- Te teletransporta y pega justo delante de él para asegurar el hitbox del robo
							hrp.CFrame = enemyHrp.CFrame * CFrame.new(0, 0, -1.2)
							...
							-- Intenta interactuar directamente con la pelota si tiene un ProximityPrompt
							local prompt = ball:FindFirstChildOfClass("ProximityPrompt", true)
							if prompt and fireproximityprompt then
								fireproximityprompt(prompt, 1)
							end
							task.wait(0.1)
		...
		-- BUCLE SMART AUTO BLOCK (RIVAL CERCA DEL ARO)
		if Mobile_SmartBlock and hrp then
			local blockBtn = getButton({"Defense", "Block"})
			if blockBtn and blockBtn.Visible then
				local targetRim = getClosestRim()
				if targetRim then
					for _, p in ipairs(Players:GetPlayers()) do
						if p ~= player and p.Character and p.Character:FindFirstChild("HumanoidRootPart") then
							local enemyHrp = p.Character.HumanoidRootPart
							local enemyToRim = (enemyHrp.Position - targetRim.Position).Magnitude
							local myToEnemy = (hrp.Position - enemyHrp.Position).Magnitude

							if enemyToRim <= Mobile_RimBlockDist and myToEnemy <= 10 then
								local lookAtVector = Vector3.new(enemyHrp.Position.X, hrp.Position.Y, enemyHrp.Position.Z)
								hrp.CFrame = CFrame.lookAt(hrp.Position, lookAtVector)
								
								simulateButtonDown(blockBtn)
								task.wait(0.05)
								simulateButtonUp(blockBtn)
...
RunService.RenderStepped:Connect(function()
	...
	if ForcePerfectGreen then
		...
		if passIndicator then
			...
			if circle then circle.BackgroundColor3 = Color3.fromRGB(0, 255, 0) end
