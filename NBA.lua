...
local function fireMobileButton(button)
    ...
    elseif getconnections then
        pcall(function()
            ...
            for _, conn in pairs(getconnections(button.TouchTap)) do conn:Fire() end
        end)
    end
end
...
TabMobile:CreateSection("Acciones de GUI Nativo (Seguro)")

TabMobile:CreateToggle({
	Name = "🏀 Auto-Shoot Táctil (GUI)",
	CurrentValue = false,
	Flag = "MobileShootToggle",
	Callback = function(Value)
		Mobile_AutoShoot = Value
	end,
})

TabMobile:CreateToggle({
	Name = "🏃 Auto-Sprint (Mantener)",
	CurrentValue = false,
...
TabMobile:CreateButton({
	Name = "🏐 Agarrar Balón (ProximityPrompt)",
	Callback = function()
		...
		if practiceCourt then
			...
			if rack and rack:FindFirstChild("PromptAttachment") then
				...
				if prompt and fireproximityprompt then
					...
					Rayfield:Notify({Title = "Éxito", Content = "Balón agarrado.", Duration = 2})
					return
				end
			end
		end
		Rayfield:Notify({Title = "Fallo", Content = "No se encontró el rack de balones cerca.", Duration = 2})
	end,
})
...
TabHoops:CreateToggle({
	Name = "🏀 Activar Auto Shoot (Remotes)",
	CurrentValue = false,
	Flag = "HLAutoShootToggle",
	Callback = function(Value)
		HL_autoShoot = Value
		if Value then
			Rayfield:Notify({Title = "Auto Shoot", Content = "Activo. Usa el botón de abajo para disparar.", Duration = 4})
		end
	end,
})

TabHoops:CreateButton({
	Name = "🔥 Disparar (Auto Green / Remotes)",
	Callback = function()
...
-- Lógica Universal y Mobile Binds
RunService.RenderStepped:Connect(function()
	if not character or not humanoid or not hrp then return end
	local autoPaused = joystickMoving
	
	-- Mobile Toggles
	if Mobile_AutoShoot then triggerShoot() end
	if Mobile_AutoSprint then triggerSprint() end
