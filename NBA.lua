--[[
	...
	+ MOBILE HOLD-BUTTON FIXES (VERIFIED WORKING)
	+ INTENSIVE DEEP SCAN & REAL-TIME UI MONITORING
]]
...
local Window = Rayfield:CreateWindow({
	...
	LoadingSubtitle = "by Irdk | Deep Scan Edition",
...
-- ==========================================
-- FUNCIONES MOBILE GUI SIMULANDO "HOLD" REAL
-- ==========================================
...
TabMobile:CreateToggle({ Name = "🏀 Auto-Shoot Táctil (Mantiene Presionado)", CurrentValue = false, Flag = "MobileShootToggle", Callback = function(Value) Mobile_AutoShoot = Value end})
TabMobile:CreateSlider({ Name = "Tiempo de Barra de Tiro (ms)", Range = {100, 1000}, Increment = 5, CurrentValue = 455, Flag = "MobileShootHoldTime", Callback = function(Value) Mobile_ShootHoldTime = Value end})
...
TabMobile:CreateSection("Desarrollo: Monitorización Profunda")

TabMobile:CreateToggle({
	Name = "🔍 Monitor Intensivo (Deep Scan)",
	CurrentValue = false,
	Flag = "InspectGreenToggle",
	Callback = function(Value)
		Inspect_GreenIndicator = Value
		if Value then
			Rayfield:Notify({Title = "Monitor Activo", Content = "Haz el movimiento. Capturaré cambios en tiempo real.", Duration = 4})
		end
	end,
})

local LogParagraph = TabMobile:CreateParagraph({
	Title = "Log de Interfaz Intensivo:",
	Content = "Esperando la aparición de PassIndicator / Meter..."
})

-- Función auxiliar para no congelar la UI de Rayfield con spam de texto
local function ThrottleUIUpdate()
	if tick() - LastUIUpdate > 0.5 then -- Actualiza la interfaz cada medio segundo maximo
		-- Limitamos el texto a los últimos 8000 caracteres para evitar lag en Delta
		local safeText = string.len(CapturedLog) > 8000 and "..." .. string.sub(CapturedLog, -8000) or CapturedLog
		LogParagraph:Set({Title = "Log Intensivo (Últimos Eventos):", Content = safeText})
		LastUIUpdate = tick()
	end
end

TabMobile:CreateButton({
	Name = "📋 COPIAR LOG COMPLETO AL PORTAPAPELES",
	Callback = function()
		if setclipboard then
			setclipboard(CapturedLog)
			Rayfield:Notify({Title = "Éxito", Content = "Log completo copiado. " .. string.len(CapturedLog) .. " caracteres.", Duration = 3})
		else
			Rayfield:Notify({Title = "Fallo", Content = "Tu ejecutor no soporta setclipboard().", Duration = 3})
		end
	end,
})
...
TabMobile:CreateButton({
	...
	Callback = function()
		CapturedLog = ""
		LogParagraph:Set({Title = "Log de Interfaz Intensivo:", Content = "Log limpiado."})
	end,
})

-- ==========================================
-- MOTOR DE DEEP SCAN Y REAL-TIME MONITORING
-- ==========================================
local function safeRead(obj, prop)
	local success, result = pcall(function() return tostring(obj[prop]) end)
	return success and result or "N/A"
end

local function DeepScan(obj, depth)
	local indent = string.rep("  ", depth)
	local log = indent .. "► " .. obj.Name .. " [" .. obj.ClassName .. "]\n"
	
	-- Leer propiedades visuales y matemáticas clave
	local props = {"Size", "Position", "Visible", "Image", "ImageColor3", "BackgroundColor3", "BackgroundTransparency", "Rotation"}
	for _, prop in ipairs(props) do
		local val = safeRead(obj, prop)
		if val ~= "N/A" then
			log = log .. indent .. "  - " .. prop .. ": " .. val .. "\n"
		end
	end

	-- Verificar si hay un script
	if obj:IsA("LocalScript") or obj:IsA("ModuleScript") then
		log = log .. indent .. "  [!] SCRIPT DETECTADO: " .. obj.Name .. "\n"
		-- Intento de decompile (La mayoría de ejecutores móviles fallarán aquí, pero se intenta)
		pcall(function()
			if decompile then
				log = log .. indent .. "  [+] Código fuente disponible vía ejecutor.\n"
			end
		end)
	end

	for _, child in ipairs(obj:GetChildren()) do
		log = log .. DeepScan(child, depth + 1)
	end
	return log
end

local monitoredObjects = {} -- Para evitar doble monitoreo

local function StartRealTimeMonitor(obj)
	if monitoredObjects[obj] then return end
	monitoredObjects[obj] = true

	if obj:IsA("GuiObject") then
		obj.Changed:Connect(function(prop)
			if prop == "Size" or prop == "Position" or prop == "ImageColor3" or prop == "BackgroundColor3" or prop == "Visible" then
				local val = safeRead(obj, prop)
				CapturedLog = CapturedLog .. string.format("[T: %.2f] %s cambió %s -> %s\n", tick() % 100, obj.Name, prop, val)
				ThrottleUIUpdate()
			end
		end)
	end
	...
	for _, child in ipairs(obj:GetChildren()) do
		StartRealTimeMonitor(child)
	end
	
	-- Escuchar si le agregan algo nuevo en tiempo real
	obj.ChildAdded:Connect(function(child)
		CapturedLog = CapturedLog .. "[NUEVO OBJETO INYECTADO] -> " .. child.Name .. " (" .. child.ClassName .. ")\n"
		StartRealTimeMonitor(child)
	end)
end
...
PlayerGui.DescendantAdded:Connect(function(descendant)
	...
	if name:match("passindicator") or name:match("meter") or name:match("green") or name:match("indicator") then
		task.delay(0.05, function()
			local initLog = "\n================================\n"
			initLog = initLog .. "🕵️ DEEP SCAN INICIADO: " .. descendant.Name .. "\n"
			initLog = initLog .. "Ruta: " .. descendant:GetFullName() .. "\n\n"
			initLog = initLog .. "[ ÁRBOL DE PROPIEDADES ESTÁTICO ]\n"
			
			initLog = initLog .. DeepScan(descendant, 0)
			
			initLog = initLog .. "\n[ INICIANDO MONITOR DE EVENTOS EN TIEMPO REAL... ]\n"
			CapturedLog = CapturedLog .. initLog
			ThrottleUIUpdate()
			
			StartRealTimeMonitor(descendant)
		end)
	end
end)
