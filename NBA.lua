...
local Window = Rayfield:CreateWindow({
	...
	LoadingSubtitle = "by Irdk | Fixed Mobile Holds",
...
local Mobile_ShootHoldTime = 455 -- Tiempo que mantendrá presionado en milisegundos
local Mobile_AutoSprint = false
local Mobile_AutoGrab = false
local Inspect_GreenIndicator = false
local joystickMoving = false
local CapturedLog = "" -- Variable para guardar el log del inspector
...
-- Simular Dribble/Touch (Un solo toque)
...
task.spawn(function()
	...
	while task.wait(0.1) do
		...
		-- 1. Auto Shoot Manteniendo Presionado (Hold)
		if Mobile_AutoShoot then
			local shootBtn = getButton({"Offense", "OnBall", "Shoot"})
			if shootBtn and shootBtn.Visible then
				simulateButtonDown(shootBtn) -- Empieza a presionar
				task.wait(Mobile_ShootHoldTime / 1000) -- Mantiene el tiempo del slider
				simulateButtonUp(shootBtn) -- Suelta el botón
				task.wait(1.5) -- Cooldown para no volver a disparar inmediatamente
			end
		end

		-- 2. Auto Sprint Manteniendo Presionado
		...
		-- 3. Auto Recoger Balón Infinito (Lógica original del botón)
...
TabMobile:CreateToggle({
	Name = "🏀 Auto-Shoot Táctil (Mantiene Presionado)",
	CurrentValue = false,
	Flag = "MobileShootToggle",
	Callback = function(Value) Mobile_AutoShoot = Value end,
})

TabMobile:CreateSlider({
	Name = "Tiempo de Barra de Tiro (ms)",
	Range = {100, 1000},
	Increment = 5,
	CurrentValue = 455,
	Flag = "MobileShootHoldTime",
	Callback = function(Value)
		Mobile_ShootHoldTime = Value
	end,
})

TabMobile:CreateToggle({
	Name = "🏃 Auto-Sprint (Mantiene al moverse)",
	CurrentValue = false,
	Flag = "MobileSprintToggle",
	Callback = function(Value) 
		Mobile_AutoSprint = Value 
		if not Value then
			local sprintBtn = getButton({"Shared", "Sprint"})
			if sprintBtn then simulateButtonUp(sprintBtn) end
		end
	end,
})
...
TabMobile:CreateToggle({
	...
	Callback = function(Value) Mobile_AutoGrab = Value end,
})
...
TabMobile:CreateSection("Herramientas de Desarrollo: Indicador")

TabMobile:CreateToggle({
	Name = "🔍 Habilitar Rastreo Green Indicator",
	CurrentValue = false,
	Flag = "InspectGreenToggle",
	Callback = function(Value)
		Inspect_GreenIndicator = Value
		if Value then
			Rayfield:Notify({Title = "Activo", Content = "Haz un tiro. El log aparecerá abajo.", Duration = 3})
		end
	end,
})

-- Cuadro de texto donde se mostrará el Log
local LogParagraph = TabMobile:CreateParagraph({
	Title = "Log de Interfaz:",
	Content = "Aquí aparecerán los datos del Green Indicator..."
})

TabMobile:CreateButton({
	Name = "📋 COPIAR LOG AL PORTAPAPELES",
	Callback = function()
		if setclipboard then
			setclipboard(CapturedLog)
			Rayfield:Notify({Title = "Éxito", Content = "¡Log copiado! Pégalo donde necesites.", Duration = 3})
		else
...
-- INSPECTOR LOGGER VISUAL: Añade datos al párrafo de la UI
PlayerGui.DescendantAdded:Connect(function(descendant)
	...
	if name:match("meter") or name:match("green") or name:match("indicator") or name:match("bar") or name:match("timing") then
		task.delay(0.1, function()
			local data = "🟩 GREEN INDICATOR CAPTURADO 🟩\n"
			data = data .. "Ruta: " .. descendant:GetFullName() .. "\n"
			data = data .. "Clase: " .. descendant.ClassName .. "\n"
			pcall(function() data = data .. "Tamaño: " .. tostring(descendant.Size) .. "\n" end)
			pcall(function() data = data .. "Posición: " .. tostring(descendant.Position) .. "\n" end)
			
			data = data .. "--- Sub-Elementos ---\n"
			for _, child in pairs(descendant:GetChildren()) do
				data = data .. " ↳ " .. child.Name .. " | " .. child.ClassName .. "\n"
			end
			data = data .. "================================\n\n"
			
			-- Guardar en la variable global y actualizar Párrafo UI
			CapturedLog = CapturedLog .. data
			LogParagraph:Set({Title = "Log de Interfaz:", Content = CapturedLog})
		end)
	end
end)
