--[[
    Catalog Native Buyer v11
    - Diagnóstico: taint estructural confirmado
    - Fallback real: copia el link del catálogo automáticamente
    - Botón nunca se congela
    - Menú se abre solo
]]
...
local function showErrorAlert(fullLog, assetId, catalogLink)
    ...
    alert.Size = isMobile and UDim2.new(0.94, 0, 0, 420) or UDim2.new(0, 440, 0, 440)
    alert.Position = isMobile and UDim2.new(0.03, 0, 0.5, -210) or UDim2.new(0.5, -220, 0.5, -220)
    ...
    title.Text = "⚠ PROMPT BLOQUEADO (Taint estructural)"
    ...
    linkLabel.Size = UDim2.new(1, -20, 0, 18)
    linkLabel.Position = UDim2.new(0, 10, 0, 54)
    linkLabel.BackgroundTransparency = 1
    linkLabel.Text = "Link copiado automáticamente al portapapeles"
    linkLabel.Font = Enum.Font.Gotham
    linkLabel.TextSize = 12
    linkLabel.TextColor3 = Color3.fromRGB(130, 200, 140)
    linkLabel.TextXAlignment = Enum.TextXAlignment.Left
    linkLabel.ZIndex = 501
    linkLabel.Parent = alert
    ...
    local textSize = TextService:GetTextSize(fullLog, 11, Enum.Font.Code, Vector2.new(400, 9999))
    ...
    copyLinkBtn.MouseButton1Click:Connect(function()
        local ok = setClipboard(catalogLink)
        if ok then
            copyLinkBtn.Text = "✅ LINK COPIADO"
            notify("Link copiado", "Pégalo en el navegador o en Roblox", 4)
        else
...
local function promptNativePurchase(assetId, itemType)
    ...
    local log = {}
    table.insert(log, "=== LOG PROMPT NATIVO v11 ===")
    ...
    -- Métodos Thread Washing (con timeout)
    ...
    pcall(function()
        for _, obj in pairs(CoreGui:GetDescendants()) do
            ...
            if (n:find("purchase") or n:find("prompt") or n:find("buy")) and obj.Visible ~= false then
                promptVisible = true
                break
            end
        end
    ...
    table.insert(log, "Prompt visible: " .. tostring(promptVisible))
    table.insert(log, "")
    table.insert(log, "CONCLUSIÓN: Taint estructural del ejecutor.")
    table.insert(log, "El prompt nativo está bloqueado a nivel profundo.")
    table.insert(log, "Se copió el link del catálogo como alternativa.")

    local fullLog = table.concat(log, "\n")
    warn(fullLog)

    -- Siempre intentar copiar el link
    local copied = setClipboard(catalogLink)

    if not promptVisible then
        showErrorAlert(fullLog, assetId, catalogLink)
        if copied then
            notify("Link copiado", "El prompt está bloqueado. Se copió el link del catálogo.", 5)
        else
            notify("Prompt bloqueado", "No se pudo copiar el link automáticamente.", 4)
        end
        return false
    else
        notify("Prompt detectado", "Menú nativo visible", 3)
        return true
    end
end
...
local function createMenu()
    pcall(function()
        local old = CoreGui:FindFirstChild("CatalogNativeBuyerV11")
        if old then old:Destroy() end
        if gethui then
            local h = gethui():FindFirstChild("CatalogNativeBuyerV11")
            if h then h:Destroy() end
        end
    end)

    ScreenGui = Instance.new("ScreenGui")
    ScreenGui.Name = "CatalogNativeBuyerV11"
    ...
    Title.BackgroundTransparency = 1
    Title.Text = "CATALOG • NATIVE BUY v11"
    Title.Font = Enum.Font.GothamBold
    Title.TextSize = 15
    ...
    Status.Text = "Menú abierto • Fallback de link activo"
    ...
    print("[Catalog Native Buyer v11] Listo – Fallback de link activo")
    notify("v11 Listo", "Menú abierto • Si el prompt falla se copia el link", 4)
end
