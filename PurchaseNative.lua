--[[
    Catalog Native Buyer v13
    - Abre el link del catálogo en el navegador (GuiService)
    - Siempre copia el link
    - Intenta descargar la imagen (writefile si existe)
    - Detección estricta de prompt
    - Menú se abre solo
]]
...
-- Intentar descargar imagen (solo writefile si existe)
local function trySaveImage(assetId, itemName)
    local thumbUrl = "https://www.roblox.com/asset-thumbnail/image?assetId=" .. tostring(assetId) .. "&width=420&height=420&format=png"
    -- Alternativa más limpia:
    local cdnUrl = "https://thumbnails.roblox.com/v1/assets?assetIds=" .. tostring(assetId) .. "&size=420x420&format=Png&isCircular=false"

    local success, body = pcall(function()
        return game:HttpGet(thumbUrl)
    end)

    if success and body and #body > 100 and writefile then
        local fileName = "CatalogItem_" .. tostring(assetId) .. ".png"
...
local function showErrorAlert(fullLog, assetId, catalogLink)
    ...
    linkInfo.Text = "Link copiado + se puede abrir en navegador"
    ...
    -- Botones
    local openBrowserBtn = Instance.new("TextButton")
    openBrowserBtn.Size = UDim2.new(1, -20, 0, 40)
    openBrowserBtn.Position = UDim2.new(0, 10, 1, -140)
    openBrowserBtn.BackgroundColor3 = Color3.fromRGB(40, 110, 200)
    openBrowserBtn.Text = "🌐 ABRIR EN NAVEGADOR"
    openBrowserBtn.Font = Enum.Font.GothamBold
    openBrowserBtn.TextSize = 14
    openBrowserBtn.TextColor3 = Color3.new(1,1,1)
    openBrowserBtn.ZIndex = 501
    openBrowserBtn.Parent = alert
    Instance.new("UICorner", openBrowserBtn).CornerRadius = UDim.new(0, 9)

    local copyLinkBtn = Instance.new("TextButton")
    copyLinkBtn.Size = UDim2.new(0.48, -8, 0, 40)
    copyLinkBtn.Position = UDim2.new(0, 10, 1, -90)
    ...
    copyLogBtn.Size = UDim2.new(0.48, -8, 0, 40)
    copyLogBtn.Position = UDim2.new(0.52, 0, 1, -90)
    ...
    openBrowserBtn.MouseButton1Click:Connect(function()
        local ok = openInBrowser(catalogLink)
        if ok then
            openBrowserBtn.Text = "✅ ABIERTO"
            notify("Navegador", "Se abrió el link del catálogo", 3)
        else
            openBrowserBtn.Text = "❌ FALLÓ"
            notify("Error", "No se pudo abrir el navegador", 3)
        end
        task.delay(1.8, function() openBrowserBtn.Text = "🌐 ABRIR EN NAVEGADOR" end)
    end)
...
local function promptNativePurchase(assetId, itemType, itemName)
    ...
    -- Siempre copiar el link
    local linkCopied = setClipboard(catalogLink)

    -- Intentar abrir navegador automáticamente (opcional, puedes comentar si no quieres)
    -- openInBrowser(catalogLink)

    local log = {}
    table.insert(log, "=== LOG PROMPT NATIVO v13 ===")
    ...
    try("3. Hijacking", function()
        local target = nil
        for _, p in ipairs({LocalPlayer.PlayerGui, CoreGui}) do
            if p then
                for _, obj in pairs(p:GetDescendants()) do
                    if obj:IsA("GuiButton") and obj.Visible and obj.AbsoluteSize.X > 30 then
                        target = obj
                        break
                    end
                end
        ...
        if not target then error("sin botón") end
    ...
    local imgOk, imgFile = trySaveImage(assetId, itemName)
    table.insert(log, "Imagen guardada (writefile): " .. tostring(imgOk) .. (imgFile and (" → " .. imgFile) or ""))
    ...
    if not promptVisible then
        ...
        notify("Prompt no apareció", "Link copiado. Usa 'Abrir en Navegador'", 5)
        return false
    else
        notify("Prompt detectado", "Si no ves la compra, usa el link / navegador", 4)
        return true
    end
end
...
local function createMenu()
    pcall(function()
        local old = CoreGui:FindFirstChild("CatalogNativeBuyerV13")
        if old then old:Destroy() end
        if gethui then
            local h = gethui():FindFirstChild("CatalogNativeBuyerV13")
            if h then h:Destroy() end
        end
    end)

    ScreenGui = Instance.new("ScreenGui")
    ScreenGui.Name = "CatalogNativeBuyerV13"
    ...
    Title.BackgroundTransparency = 1
    Title.Text = "CATALOG • NATIVE BUY v13"
    Title.Font = Enum.Font.GothamBold
    Title.TextSize = 15
    ...
    Status.Text = "Menú abierto • Link + Navegador disponibles"
    ...
    print("[Catalog Native Buyer v13] Listo – Navegador + Link + Imagen")
    notify("v13 Listo", "Menú abierto • Al comprar se copia link y puedes abrir navegador", 4)
end
