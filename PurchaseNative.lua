--[[
    Catalog Native Buyer v12
    - Detección de prompt mucho más estricta (evita falsos positivos)
    - Siempre copia el link del catálogo al comprar
    - Menú se abre solo
    - Timeouts + botón seguro
]]
...
local function showErrorAlert(fullLog, assetId, catalogLink)
    ...
    alert.Size = isMobile and UDim2.new(0.94, 0, 0, 430) or UDim2.new(0, 450, 0, 450)
    alert.Position = isMobile and UDim2.new(0.03, 0, 0.5, -215) or UDim2.new(0.5, -225, 0.5, -225)
    ...
    linkInfo.Text = "Link del catálogo copiado al portapapeles"
    ...
    local textSize = TextService:GetTextSize(fullLog, 11, Enum.Font.Code, Vector2.new(410, 9999))
    ...
    copyLinkBtn.MouseButton1Click:Connect(function()
        if setClipboard(catalogLink) then
            ...
            notify("Link copiado", "Pégalo en el navegador o Roblox", 4)
        else
...
local function promptNativePurchase(assetId, itemType)
    ...
    -- Siempre copiar el link (como pediste)
    local linkCopied = setClipboard(catalogLink)

    local log = {}
    table.insert(log, "=== LOG PROMPT NATIVO v12 ===")
    table.insert(log, "Asset ID: " .. tostring(assetId))
    table.insert(log, "Link: " .. catalogLink)
    table.insert(log, "Link copiado automáticamente: " .. tostring(linkCopied))
    ...
    table.insert(log, "")
    table.insert(log, "Si no aparece la alerta de compra → taint del ejecutor.")
    ...
    if not promptVisible then
        ...
        notify("Prompt no apareció", "Se copió el link del catálogo. Úsalo manualmente.", 5)
        return false
    else
        notify("Prompt detectado", "Si no ves la alerta de compra, usa el link copiado", 4)
        return true
    end
end
...
local function createMenu()
    pcall(function()
        local old = CoreGui:FindFirstChild("CatalogNativeBuyerV12")
        if old then old:Destroy() end
        if gethui then
            local h = gethui():FindFirstChild("CatalogNativeBuyerV12")
            if h then h:Destroy() end
        end
    end)

    ScreenGui = Instance.new("ScreenGui")
    ScreenGui.Name = "CatalogNativeBuyerV12"
    ...
    Title.BackgroundTransparency = 1
    Title.Text = "CATALOG • NATIVE BUY v12"
    Title.Font = Enum.Font.GothamBold
    Title.TextSize = 15
    ...
    Status.Text = "Menú abierto • Link se copia siempre"
    ...
    print("[Catalog Native Buyer v12] Listo – Link siempre se copia")
    notify("v12 Listo", "Menú abierto • Al comprar se copia el link del catálogo", 4)
end
