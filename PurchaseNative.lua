--[[
    Catalog Native Buyer v16 (Delta/Mobile Ultimate Fix)
    - Fix: Bypass al bloqueo de red del ejecutor usando RoProxy y User-Agents para descargar imágenes.
    - Nuevo: Sistema Smart Browser. Abre el navegador de Roblox y, si falla, fuerza Safari/Chrome externo usando funciones del ejecutor (openurl).
]]
...
-- ===================== SMART BROWSER (FIX IOS/ANDROID) =====================
local function openSmartBrowser(url)
    -- 1. Intentar in-game primero (Menos intrusivo)
    local inGameOk = pcall(function() GuiService:OpenBrowserWindow(url) end)
    if inGameOk then return "INGAME" end

    -- 2. Si falla in-game, forzar Safari/Chrome externo (Método Delta Keys)
...
local function fetchUrl(url)
    if http_request then
        ...
        if ok and res and (res.StatusCode == 200 or res.StatusCode == 301 or res.StatusCode == 302) then
            return res.Body
        end
    end
    -- Fallback
...
local function saveImageHD(assetId, itemName)
    if not writefile then return false, "writefile no disponible en tu ejecutor" end

    -- Usamos RoProxy porque los ejecutores bloquean roblox.com HTTP directos
    local urls = {
        "https://thumbnails.roproxy.com/v1/assets?assetIds=" .. tostring(assetId) .. "&size=420x420&format=Png&isCircular=false",
        "https://www.roproxy.com/asset-thumbnail/image?assetId=" .. tostring(assetId) .. "&width=420&height=420&format=png"
    }
    ...
    for _, url in ipairs(urls) do
        ...
        if data and #data > 500 then
            -- Si obtenemos JSON (thumbnails API)
            if data:sub(1,1) == "{" then
                ...
                if success and json and json.data and json.data[1] and json.data[1].imageUrl then
                    -- La URL final apunta a rbxcdn.com (No está bloqueada por los ejecutores)
    ...
    if not imageData then
        return false, "No se pudo descargar. (Error de red o archivo vacío)"
    end
...
local function showErrorAlert(fullLog, assetId, catalogLink)
    ...
    openLinkBtn.MouseButton1Click:Connect(function()
        ...
        if result == "INGAME" then openLinkBtn.Text = "✅ ABRIENDO"
        elseif result == "EXTERNAL" then openLinkBtn.Text = "🌍 SAFARI/CHROME"
        else
...
local function promptNativePurchase(assetId, itemType, itemName)
    ...
    local log = {}
    table.insert(log, "=== LOG PROMPT NATIVO v16 (Delta) ===")
...
local function createMenu()
    pcall(function()
        local old = CoreGui:FindFirstChild("CatalogNativeBuyerV16")
        if old then old:Destroy() end
        if gethui then
            local h = gethui():FindFirstChild("CatalogNativeBuyerV16")
            if h then h:Destroy() end
        end
    end)

    ScreenGui = Instance.new("ScreenGui")
    ScreenGui.Name = "CatalogNativeBuyerV16"
    ...
    Title.BackgroundTransparency = 1
    Title.Text = "CATALOG • NATIVE BUY v16"
    Title.Font = Enum.Font.GothamBold
    Title.TextSize = 15
    ...
    Status.Text = "Toca miniatura = HD | Toca 🌐 = Safari/Chrome"
    ...
    local function createItemCard(item)
        ...
        thumb.MouseButton1Click:Connect(function()
            notify("Descargando...", "Obteniendo datos vía RoProxy...", 3)
            task.spawn(function()
        ...
        browserBtn.MouseButton1Click:Connect(function()
            ...
            if result == "INGAME" then notify("Abriendo...", "Cargando en ventana de Roblox...", 3)
            elseif result == "EXTERNAL" then notify("Abriendo...", "Abierto en Safari/Chrome...", 3)
    ...
    print("[Catalog Native Buyer v16] Smart Browser + Proxy Fix Listos")
    notify("v16 Listo", "Proxy activado para imágenes. Navegador inteligente configurado.", 5)
end
