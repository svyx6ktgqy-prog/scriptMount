--[[
    Catalog Native Buyer v19 (Pagination + ItemSavePNG folder)
    - V19: Botones Siguiente / Atrás para paginar resultados (cursor API).
    - V19: Imágenes guardadas en la carpeta /ItemSavePNG
    - Mantiene: Anti-freeze (task.spawn), rbxcdn HD, openurl prioritario, prompt nativo, UI completa.
]]
...
local currentCursor = nil          -- cursor actual (nil = primera página)
local nextPageCursor = nil
local previousPageCursor = nil
local currentPage = 1
...
-- ===================== SMART BROWSER V18 PRIORITY (Safari/Chrome primero) =====================
...
-- ===================== DESCARGA DE IMAGEN HD (ANTI-CONGELAMIENTO + rbxcdn + carpeta) =====================
...
local function saveImageHD(assetId, itemName)
    ...
    -- 1. Obtener la URL pura de la imagen (rbxcdn)
    ...
    -- 3. Guardar en carpeta ItemSavePNG/
    local safeName = tostring(itemName or "Item"):gsub("[^%w%-_]", "_"):sub(1, 30)
    local fileName = "ItemSavePNG/CatalogHD_" .. tostring(assetId) .. "_" .. safeName .. ".png"

    -- Intentar crear la carpeta si el ejecutor lo soporta
    pcall(function()
        if makefolder then
            makefolder("ItemSavePNG")
        end
    end)
...
local function showErrorAlert(fullLog, assetId, catalogLink)
    ...
    openLinkBtn.MouseButton1Click:Connect(function()
        ...
        elseif result == "INGAME" then openLinkBtn.Text = "✅ ABRIENDO"
            ...
            notify("Error", "No se pudo abrir web. Link copiado.", 4)
        end
    ...
    copyLogBtn.MouseButton1Click:Connect(function()
        if setClipboard(fullLog) then copyLogBtn.Text = "✅ COPIADO" else copyLogBtn.Text = "❌ FALLÓ" end
...
local function promptNativePurchase(assetId, itemType, itemName)
    ...
    local log = {}
    table.insert(log, "=== LOG PROMPT NATIVO v19 (Delta + Pagination) ===")
    ...
    if not promptVisible then
        ...
        notify("Prompt no apareció", "Usa el icono 🌐 para abrir en web externa", 5)
        return false
    else
...
-- ===================== BÚSQUEDA CON CURSOR (PAGINACIÓN) =====================
local function searchCatalog(keyword, cursor)
    local url = "https://catalog.roblox.com/v1/search/items/details?category=" .. tostring(DEFAULT_CATEGORY) 
    ...
    if not data then return nil, nil, nil, "Error de red" end
    local ok, decoded = pcall(function() return HttpService:JSONDecode(data) end)
    if not ok or not decoded or not decoded.data then return nil, nil, nil, "Error JSON" end
    
    return decoded.data, decoded.nextPageCursor, decoded.previousPageCursor, "OK"
end

-- ===================== MENÚ =====================
local function createMenu()
    pcall(function()
        local old = CoreGui:FindFirstChild("CatalogNativeBuyerV19")
        if old then old:Destroy() end
        if gethui then
            local h = gethui():FindFirstChild("CatalogNativeBuyerV19")
            if h then h:Destroy() end
        end
    end)

    ScreenGui = Instance.new("ScreenGui")
    ScreenGui.Name = "CatalogNativeBuyerV19"
    ...
    Main.Name = "Main"
    Main.Size = isMobile and UDim2.new(0.95, 0, 0.82, 0) or UDim2.new(0, 460, 0, 600)
    Main.Position = isMobile and UDim2.new(0.025, 0, 0.09, 0) or UDim2.new(0.5, -230, 0.5, -300)
    ...
    Title.BackgroundTransparency = 1
    Title.Text = "CATALOG • NATIVE BUY v19"
    Title.Font = Enum.Font.GothamBold
    Title.TextSize = 15
    ...
    Status.Text = "🌐 = Safari/Chrome (Directo)  •  Imágenes → ItemSavePNG/"
    ...
    PageBar.Size = UDim2.new(1, -28, 0, 40)
    PageBar.Position = UDim2.new(0, 14, 0, 140)
    PageBar.BackgroundTransparency = 1
    PageBar.ZIndex = 21
    PageBar.Parent = Main

    local PrevBtn = Instance.new("TextButton")
    PrevBtn.Size = UDim2.new(0, 110, 0, 36)
    PrevBtn.Position = UDim2.new(0, 0, 0, 0)
    PrevBtn.BackgroundColor3 = Color3.fromRGB(45, 45, 60)
    PrevBtn.Text = "◀ ATRÁS"
    ...
    PageLabel.Size = UDim2.new(0, 120, 0, 36)
    PageLabel.Position = UDim2.new(0.5, -60, 0, 0)
    ...
    NextBtn.BackgroundColor3 = Color3.fromRGB(45, 45, 60)
    ...
    Results.Size = UDim2.new(1, -28, 1, -200)
    Results.Position = UDim2.new(0, 14, 0, 188)
    ...
    local function updatePageButtons()
        PrevBtn.BackgroundColor3 = previousPageCursor and Color3.fromRGB(55, 100, 170) or Color3.fromRGB(40, 40, 50)
        PrevBtn.TextColor3 = previousPageCursor and Color3.new(1,1,1) or Color3.fromRGB(120, 120, 130)
        PrevBtn.Active = previousPageCursor ~= nil

        NextBtn.BackgroundColor3 = nextPageCursor and Color3.fromRGB(55, 100, 170) or Color3.fromRGB(40, 40, 50)
        NextBtn.TextColor3 = nextPageCursor and Color3.new(1,1,1) or Color3.fromRGB(120, 120, 130)
        NextBtn.Active = nextPageCursor ~= nil

        PageLabel.Text = "Página " .. tostring(currentPage)
    end
    ...
    local function createItemCard(item)
        ...
        thumb.MouseButton1Click:Connect(function()
            notify("Descargando...", "Extrayendo imagen → ItemSavePNG/", 3)
            task.spawn(function()
        ...
        browserBtn.MouseButton1Click:Connect(function()
            ...
            else notify("Copiado", "Se copió al portapapeles. Pégalo en tu navegador.", 4) end
        end)
    ...
    local function loadPage(cursor, pageDelta)
        if searching then return end
        if currentKeyword == "" then return end

        searching = true
        clearResults()
        Status.Text = "Cargando página..."
        ...
        task.spawn(function()
            local items, nextC, prevC, msg = searchCatalog(currentKeyword, cursor)
            
            if items and #items > 0 then
                for _, item in ipairs(items) do createItemCard(item) end
                nextPageCursor = nextC
                previousPageCursor = prevC
                currentCursor = cursor
                currentPage = currentPage + (pageDelta or 0)
                if currentPage < 1 then currentPage = 1 end

                Status.Text = #items .. " resultados • Página " .. currentPage
    ...
    PrevBtn.MouseButton1Click:Connect(function()
        if previousPageCursor and not searching then
            loadPage(previousPageCursor, -1)
        end
    end)

    NextBtn.MouseButton1Click:Connect(function()
        if nextPageCursor and not searching then
            loadPage(nextPageCursor, 1)
        end
    end)

    updatePageButtons()

    print("[Catalog Native Buyer v19] Paginación + ItemSavePNG + Anti-Lag + rbxcdn + Navegador Híbrido Activos")
    notify("v19 Listo", "Páginas ◀ ▶ + Imágenes en ItemSavePNG/", 5)
end
