--[[
    Catalog Native Buyer v10
    - Botón COMPRAR ya no se queda en "..." eternamente
    - Timeouts estrictos en todos los métodos
    - Thread Washing + manejo de errores robusto
    - Menú se abre automáticamente
]]
...
local function showErrorAlert(fullLog, assetId)
    ...
    title.Text = "⚠ PROMPT BLOQUEADO / TIMEOUT (v10)"
...
local function promptNativePurchase(assetId, itemType)
    ...
    table.insert(log, "=== LOG PROMPT NATIVO v10 (TIMEOUTS) ===")
    ...
    -- MÉTODO 1: VirtualUser Thread Washing (con timeout)
    try("1. VirtualUser Thread Washing", function()
        local gui = Instance.new("ScreenGui")
        gui.Name = "BypassGUI_" .. math.random(10000,99999)
        ...
        VirtualUser:ClickButton1(Vector2.new(30, 30))
        task.wait(0.1)
        VirtualUser:ClickButton1(Vector2.new(30, 30))
    ...
    -- MÉTODO 3: Hijacking (rápido)
    try("3. Native GUI Hijacking", function()
        local target = nil
        for _, parent in ipairs({LocalPlayer:FindFirstChild("PlayerGui"), CoreGui}) do
            if parent then
                for _, obj in pairs(parent:GetDescendants()) do
                    if obj:IsA("GuiButton") and obj.Visible and obj.AbsoluteSize.X > 25 then
                        target = obj
                        break
                    end
                end
        ...
        if not target then error("No hay botón nativo") end

        local conn
        conn = target.MouseButton1Click:Connect(function()
    ...
    try("5. LocalScript temporal", function()
        ...
        ls.Source = string.format([[
            ...
            pcall(function() if setthreadidentity then setthreadidentity(2) end end)
            ms:PromptPurchase(game.Players.LocalPlayer, %d)
            task.wait(0.3)
            script:Destroy()
        ]], assetId)
        local parent = LocalPlayer:FindFirstChild("PlayerScripts") or LocalPlayer.PlayerGui or LocalPlayer.Character
    ...
    -- Esperar un poco y detectar prompt
    task.wait(0.7)

    pcall(function()
        for _, obj in pairs(CoreGui:GetDescendants()) do
            local n = string.lower(tostring(obj.Name))
            if (n:find("purchase") or n:find("prompt") or n:find("buy") or n:find("product")) 
               and (obj:IsA("Frame") or obj:IsA("ScreenGui")) and obj.Visible ~= false then
                promptVisible = true
                break
            end
        end
    ...
    table.insert(log, "")
    table.insert(log, "Si sigue fallando → el ejecutor tiene taint muy fuerte.")
    ...
    if not promptVisible then
        ...
        notify("Prompt no apareció", "Se abrió el log de error", 4)
        return false
    else
...
local function createMenu()
    pcall(function()
        local old = CoreGui:FindFirstChild("CatalogNativeBuyerV10")
        if old then old:Destroy() end
        if gethui then
            local h = gethui():FindFirstChild("CatalogNativeBuyerV10")
            if h then h:Destroy() end
        end
    end)

    ScreenGui = Instance.new("ScreenGui")
    ScreenGui.Name = "CatalogNativeBuyerV10"
    ...
    Title.BackgroundTransparency = 1
    Title.Text = "CATALOG • NATIVE BUY v10"
    Title.Font = Enum.Font.GothamBold
    Title.TextSize = 15
    ...
    Status.Text = "Menú abierto • Timeouts activos (v10)"
    ...
    local function createItemCard(item)
        ...
        buy.MouseButton1Click:Connect(function()
            if buy.Text == "..." then return end -- evitar doble click
            buy.Text = "..."
            ...
            task.spawn(function()
                ...
                local ok, err = pcall(function()
                    ...
                    success = promptNativePurchase(item.id, itemType)
                end)

                -- SIEMPRE restaurar el botón (incluso si hubo error o timeout)
                buy.Text = "COMPRAR"
                buy.BackgroundColor3 = Color3.fromRGB(45, 105, 195)

                if not ok then
                    warn("[Buy Button] Error interno: " .. tostring(err))
                end
            end)
        end)
    end
    ...
    print("[Catalog Native Buyer v10] Listo – Timeouts + botón seguro")
    notify("v10 Listo", "Menú abierto • Botón ya no se congela", 4)
end
