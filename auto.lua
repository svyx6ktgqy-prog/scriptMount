...
-- Fix Absoluto: Alineación 3D + Head/Chest Target
...
local TargetPartName = "Cabeza" -- "Cabeza", "Pecho", "Centro"
...
-----------------------------------------------------
-- OBTENCIÓN DE PARTE DEL CUERPO (HEAD / UPPER TORSO)
-----------------------------------------------------
...
-----------------------------------------------------
-- LÓGICA DE APUNTADO ABSOLUTO (CÁMARA + PERSONAJE 3D)
-----------------------------------------------------
...
local function AimAtPosition(targetPos)
    ...
    -- 1. Fijar Cámara directamente al objetivo exacto en 3D
    Camera.CFrame = CFrame.lookAt(Camera.CFrame.Position, targetPos)
    
    -- 2. Fijar Personaje/Arma en 3D absoluto hacia el objetivo
    humanoid.AutoRotate = false -- Desactiva la rotación nativa de Roblox
...
-- Bucle de renderizado (Por cada frame)
RunService.RenderStepped:Connect(function()
    local isAiming = false

    -- Prioridad 1: Objetivo Específico
...
-----------------------------------------------------
-- INTERFAZ FLOTANTE (COMPATIBLE CON DELTA iOS)
-----------------------------------------------------
...
local Window = Rayfield:CreateWindow({
   ...
   LoadingSubtitle = "Fix Absoluto 3D",
   ConfigurationSaving = {
      Enabled = false
   }
})
