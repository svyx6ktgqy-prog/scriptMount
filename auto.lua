...
-- Modo: Órbita Natural Personaje + Cámara + Mira
...
-----------------------------------------------------
-- APUNTADO NATURAL Y SINCRONIZADO (CÁMARA ÓRBITA + HR)
-----------------------------------------------------
...
local function AimAtPosition(targetPos)
    ...
    local dirToTarget = (targetPos - hrpPos)
    if dirToTarget.Magnitude < 0.1 then return end
    
    local dirUnit = dirToTarget.Unit

    -- 1. Calcular la distancia actual de la cámara al personaje para no romper el Zoom TPS
    local currentCamDist = (Camera.CFrame.Position - hrpPos).Magnitude
    currentCamDist = math.clamp(currentCamDist, 5, 20) -- Límite natural de tercera persona

    -- 2. Posicionar la cámara en órbita detrás del personaje orientada al enemigo
    -- Esto mueve la MIRA de forma fluida y natural al objetivo
    local newCamPos = hrpPos - (dirUnit * currentCamDist)
    Camera.CFrame = CFrame.lookAt(newCamPos, targetPos)

    -- 3. Rotar al personaje hacia la dirección del objetivo en el plano horizontal
...
RunService.RenderStepped:Connect(function()
    ...
    -- Prioridad 1: Selector Específico
...
local Window = Rayfield:CreateWindow({
   ...
   LoadingSubtitle = "Sincronización Órbita Natural",
   ConfigurationSaving = {
      Enabled = false
   }
})
