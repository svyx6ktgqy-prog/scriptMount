...
-- Fix Preciso: Separación de Físicas HRP y Cámara
-- ==========================================
...
local function AimAtPosition(targetPos)
    ...
    -- 1. Fijar la CÁMARA en 3D absoluto hacia la posición exacta del objetivo
    Camera.CFrame = CFrame.lookAt(Camera.CFrame.Position, targetPos)
    
    -- 2. Fijar el CUERPO (HRP) únicamente en el eje horizontal (Eje Y)
    -- Esto evita que el personaje se incline verticalmente y rompa las físicas del suelo
    ...
    -- 3. Inclinación del Torso (Ajuste visual del arma sin romper físicas)
    local upperTorso = char:FindFirstChild("UpperTorso")
    local waist = upperTorso and upperTorso:FindFirstChild("Waist")
    if waist then
        local direction = (targetPos - hrpPos).Unit
        local pitch = math.asin(direction.Y) -- Ángulo de inclinación vertical
...
local Window = Rayfield:CreateWindow({
   ...
   LoadingSubtitle = "Fix Estabilidad Quirúrgico",
   ConfigurationSaving = {
      Enabled = false
   }
})
