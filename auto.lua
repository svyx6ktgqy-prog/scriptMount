...
-- Modo: Fix Definitivo de Cámara (Sin Drift / CameraType Scriptable)
...
-- Guardar C0s originales de las articulaciones anatómicas
local OriginalC0s = {}

-- Offset fijo TPS sobre el hombro derecho (Evita que la cámara se descalibre)
local CAMERA_OFFSET = Vector3.new(2.2, 1.8, 7.5) 
...
-----------------------------------------------------
-- APUNTADO QUIRÚRGICO TPS (CÁMARA ESTABLE CERO-DRIFT)
-----------------------------------------------------
...
local function AimAtPosition(targetPos)
    ...
    -- 1. Tomar control absoluto de la cámara para que el script nativo de Roblox no acumule error
    if Camera.CameraType ~= Enum.CameraType.Scriptable then
        Camera.CameraType = Enum.CameraType.Scriptable
    end

    -- 2. Rotar el cuerpo en el plano horizontal
    ...
    -- 3. Posicionar la cámara fijada al CFrame de tu personaje con Offset TPS estricto
    -- Esto garantiza que NUNCA se descalibre ni se aleje progresivamente
    local cameraCFrameAbsolute = hrp.CFrame * CFrame.new(CAMERA_OFFSET)
    Camera.CFrame = CFrame.lookAt(cameraCFrameAbsolute.Position, targetPos)

    -- 4. Inclinación Anatómica de Tu Personaje
...
local function ResetCharacterState()
    ...
    -- Restaurar la cámara al modo normal del juego al dejar de apuntar
...
local Window = Rayfield:CreateWindow({
   ...
   LoadingSubtitle = "Fix Cámaras / Cero Drift",
   ConfigurationSaving = {
      Enabled = false
   }
})
