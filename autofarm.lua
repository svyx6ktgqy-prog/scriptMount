...
local function mineCrystal(crystal)
    ...
    -- MICRO-PAUSA CLAVE para el servidor
    task.wait(0.15)
    ...
    while mining and crystal and crystal.Parent do
        ...
        -- NUEVO BYPASS DE MONTAÑAS Y PAREDES
        if prompt and prompt.Parent then
            pcall(function()
                prompt.RequiresLineOfSight = false -- Ignorar paredes
                if prompt.MaxActivationDistance < 30 then
                    prompt.MaxActivationDistance = 30 -- Expandir rango para evitar fallos por distancia
                end
            end)
            firePrompt(prompt)
        end
    ...
    -- LIMPIEZA AUTOMÁTICA DEL ESCÁNER
    if scannerBox then
        scannerBox:Destroy()
    end
    ...
    Callback = function(Value)
        ...
        if mining then
            ...
            Rayfield:Notify({
                ...
                Content = "Modo ráfaga + Wall Bypass activado",
                Duration = 2,
                Image = 4483362458,
            })
            task.spawn(autoMine)
...
print("Rayfield Hub Cargado. Wall-Bypass para montañas implementado.")
