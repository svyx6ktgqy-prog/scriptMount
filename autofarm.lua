...
-- NUEVO: Mapeo de colores para el Escáner Cúbico por nivel
local colorMapping = {
...
local function startStick(part)
    ...
    stickConnection = RunService.Heartbeat:Connect(function()
        ...
        if hrp then
            -- Posicionado un poco atrás (2.2 studs) y mirando de frente al diamante
...
local function mineCrystal(crystal)
    ...
    -- -------------------------------------------------------------------
    -- NUEVO: CREACIÓN DEL ESCÁNER CÚBICO VISUAL
    -- -------------------------------------------------------------------
    ...
    scannerBox.LineThickness = 0.04 -- Grosor de las líneas del cubo
    -- Toque estético pro: Hace que las caras del cubo sean ligeramente translúcidas
    ...
    -- TU METODO DE GOLPEO ORIGINAL (COMPLETAMENTE INTACTO)
    ...
    -- -------------------------------------------------------------------
    -- NUEVO: LIMPIEZA AUTOMÁTICA AL TERMINAR O PASAR AL SIGUIENTE
    -- -------------------------------------------------------------------
    if scannerBox then
        scannerBox:Destroy()
    end
...
print("Rayfield Hub Cargado. Efectos de escáner activos.")
