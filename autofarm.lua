...
local function startStick(part)
    ...
    stickConnection = RunService.Heartbeat:Connect(function()
        ...
        if hrp then
            -- Posicionado atrás (2.2 studs) y mirando de frente a la pieza objetivo
...
local function mineCrystal(crystal)
    ...
    -- FIX CRÍTICO: Apuntar a prompt.Parent (la pieza exacta del botón) en vez del modelo gigante entero
    ...
    -- Teletransporte inicial directo a la pieza del botón
    hrp.CFrame = CFrame.new(part.Position + Vector3.new(0, 1.2, 2.2), part.Position)
    startStick(part)
    ...
    -- BUCLE DE ATAQUE ULTRA RÁPIDO CON PREVENCIÓN DE OBSTÁCULOS
    while mining and crystal and crystal.Parent do
        ...
        -- FIX DE CONOCIMIENTO: Desbloquea el botón si está enterrado en el suelo o montañas altas
        if prompt and prompt.Parent then
            pcall(function()
                prompt.RequiresLineOfSight = false -- Traspasar suelo/paredes visuales
                if prompt.MaxActivationDistance < 50 then
                    prompt.MaxActivationDistance = 50 -- Ampliar rango para absorber el tamaño gigante del diamante
                end
            end)
            firePrompt(prompt)
        end
    ...
    while mining do
        ...
        for _, crystal in ipairs(crystalFolder:GetChildren()) do
            ...
            if tierCode and table.find(codes, tierCode) then
                -- Para el buscador de distancias también priorizamos la pieza del prompt
                local prompt = crystal:FindFirstChildWhichIsA("ProximityPrompt", true)
    ...
    Callback = function(Value)
        ...
        if mining then
            ...
            Rayfield:Notify({
                ...
                Content = "Modo ráfaga + Fix de Tamaño activado",
                Duration = 2,
                Image = 4483362458,
            })
            task.spawn(autoMine)
...
print("Rayfield Hub Cargado. Fix definitivo para tamaños e inclinaciones completado.")
