...
local Window = Rayfield:CreateWindow({
   Name = "Ninja Harvester 💎 | V6 Economy & Physics",
...
local MinPriceFilter = 0 -- 0 = Busca todos
...
-- Este "Stick" se usa solo para el AutoFarm en masa
...
-- CAJA DE TEXTO MEJORADA (Maneja el borrado de texto)
FarmTab:CreateInput({
   Name = "💰 Precio Mínimo ($)",
...
-- EXTRACCIÓN MANUAL CON CAÍDA POR GRAVEDAD
   ...
   Callback = function()
        ...
        if part and prompt then
            task.spawn(function()
                ...
                if hrp then
                    hrp.Anchored = false -- Asegurar que no esté congelado
                    
                    -- Lo posicionamos un poco arriba y atrás para que caiga sobre el cristal
                    hrp.CFrame = CFrame.new(part.Position + Vector3.new(0, 4, 1.5), part.Position)
                    
                    -- Le damos un ligero impulso hacia abajo para que la gravedad actúe instantáneamente y toque el suelo
                    hrp.Velocity = Vector3.new(0, -10, 0) 
                end
                
                -- Pausa crucial para dejar que el personaje caiga y el servidor lo registre físicamente
                task.wait(0.25)
