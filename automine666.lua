-- Cargar la librería Rayfield para una interfaz limpia y profesional
local Rayfield = loadstring(game:HttpGet('https://raw.githubusercontent.com/svyx6ktgqy-prog/rayfield/refs/heads/main/source.lua'))()

-- Crear la ventana principal del Minador
local Window = Rayfield:CreateWindow({
   Name = "Ninja Harvester Pro 💎",
   LoadingTitle = "Cargando Motor de Minado Perfecto...",
   LoadingSubtitle = "Especializado para Delta",
   ConfigurationSaving = { Enabled = false },
   Discord = { Enabled = false },
   KeySystem = false
})

-- Servicios de Roblox
local Players = game:GetService("Players")
local Workspace = game:GetService("Workspace")
local LocalPlayer = Players.LocalPlayer

-- Variables de Control del Script
local AutoFarmActive = false
local AutoSellActive = false
local SelectedTierOption = "Todos" -- Filtro por defecto
local TeleportDelay = 0.12 -- Margen ultra optimizado para evitar kick
local AutoTeleport = true -- Moverse al cristal automáticamente

-- Mapeo de Tiers
local tierMapping = {
    ["Common"]    = "T1",
    ["Uncommon"]  = "T2",
    ["Rare"]      = "T3",
    ["Epic"]      = "T4",
    ["Legendary"] = "T5",
    ["Mythic"]    = "T6"
}

-- Pestañas
local FarmTab = Window:CreateTab("Auto-Minar ⛏️", 4483362458)
local ConfigTab = Window:CreateTab("Configuración ⚙️", 4483362458)

-- ==========================================
-- FUNCIONES AUXILIARES DE PROCESAMIENTO
-- ==========================================

-- Función para separar y limpiar los datos del texto del ProximityPrompt
local function ParseCrystalDetails(text)
    local name = "Desconocido"
    local kg = "N/A"
    local price = "N/A"
    
    if text and text ~= "" then
        local parts = {}
        -- Separar por el punto decorativo "•"
        for part in string.gmatch(text, "[^•]+") do
            -- Limpiar espacios en blanco innecesarios
            local cleaned = string.match(part, "^%s*(.-)%s*$")
            if cleaned then
                table.insert(parts, cleaned)
            end
        end
        
        -- Asignar las partes extraídas
        name = parts[1] or name
        kg = parts[2] or kg
        price = parts[3] or price
    end
    return name, kg, price
end

-- Traduce la selección visual al formato del juego ("T1", "T2", etc.)
local function GetSelectedTierCode()
    if SelectedTierOption == "Todos" then return "Todos" end
    return string.split(SelectedTierOption, " ")[1] 
end

-- Encuentra el cristal más cercano que coincida con el Tier seleccionado
local function GetClosestCrystal()
    local root = LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart")
    if not root then return nil end

    local crystalsFolder = Workspace:FindFirstChild("Things") and Workspace.Things:FindFirstChild("Crystals")
    if not crystalsFolder then return nil end

    local closestCrystal = nil
    local shortestDistance = math.huge
    local targetTier = GetSelectedTierCode()

    for _, crystal in ipairs(crystalsFolder:GetChildren()) do
        local prompt = crystal:FindFirstChildOfClass("ProximityPrompt")
        if prompt and prompt.Enabled then
            local matches = false
            if targetTier == "Todos" then
                matches = true
            elseif string.find(crystal.Name, targetTier) then
                matches = true
            end

            if matches then
                local part = crystal:IsA("BasePart") and crystal or crystal:FindFirstChildOfClass("BasePart") or crystal.PrimaryPart
                if part then
                    local distance = (root.Position - part.Position).Magnitude
                    if distance < shortestDistance then
                        shortestDistance = distance
                        closestCrystal = {
                            Instance = crystal,
                            Prompt = prompt,
                            Position = part.CFrame,
                            Distance = distance
                        }
                    end
                end
            end
        end
    end
    return closestCrystal
end

-- ==========================================
-- INTERFAZ: PESTAÑA AUTO-MINAR
-- ==========================================

FarmTab:CreateToggle({
   Name = "Activar Minado Perfecto Automático",
   CurrentValue = false,
   Flag = "AutoFarmToggle",
   Callback = function(Value)
        AutoFarmActive = Value
        if Value then
            Rayfield:Notify({Title = "Minado Iniciado", Content = "Detectando y saltando entre cristales...", Duration = 3})
        else
            Rayfield:Notify({Title = "Minado Pausado", Content = "Auto-farm desactivado.", Duration = 3})
        end
   end,
})

FarmTab:CreateDropdown({
   Name = "Filtrar por Poder / Tier",
   Options = {"Todos", "T1 (Common)", "T2 (Uncommon)", "T3 (Rare)", "T4 (Epic)", "T5 (Legendary)", "T6 (Mythic)"},
   CurrentOption = "Todos",
   Flag = "TierSelector",
   Callback = function(Option)
        SelectedTierOption = Option
        Rayfield:Notify({Title = "Filtro Cambiado", Content = "Buscando categoría: " .. Option, Duration = 2})
   end,
})

-- Párrafo informativo dinámico donde se dividen los datos
local InfoParagraph = FarmTab:CreateParagraph({
    Title = "Estado del Minador",
    Content = "Esperando la activación..."
})

FarmTab:CreateToggle({
   Name = "Auto-Vender Cristales (Crystal Buyer)",
   CurrentValue = false,
   Flag = "AutoSellToggle",
   Callback = function(Value)
        AutoSellActive = Value
        if Value then
            Rayfield:Notify({Title = "Auto-Venta Activa", Content = "Se venderá periódicamente al llenar la mochila.", Duration = 3})
        end
   end,
})

FarmTab:CreateButton({
   Name = "Vender Ahora Manualmente",
   Callback = function()
        local sellPrompt = Workspace:FindFirstChild("Things") and Workspace.Things:FindFirstChild("SellProx") and Workspace.Things.SellProx:FindFirstChild("ProximityPrompt")
        local root = LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart")
        
        if sellPrompt and root then
            local originalPosition = root.CFrame
            root.CFrame = Workspace.Things.SellProx.CFrame * CFrame.new(0, 3, 0)
            task.wait(0.2)
            
            if fireproximityprompt then
                fireproximityprompt(sellPrompt)
            end
            task.wait(0.2)
            
            root.CFrame = originalPosition
            Rayfield:Notify({Title = "Venta Exitosa", Content = "Vendido al Crystal Buyer.", Duration = 3})
        else
            Rayfield:Notify({Title = "Error", Content = "No se detectó el punto de venta SellProx.", Duration = 3})
        end
   end,
})

-- ==========================================
-- INTERFAZ: PESTAÑA CONFIGURACIÓN
-- ==========================================

ConfigTab:CreateToggle({
   Name = "Teletransportación al Cristal",
   CurrentValue = true,
   Flag = "TeleportToggle",
   Callback = function(Value)
        AutoTeleport = Value
   end,
})

ConfigTab:CreateSlider({
   Name = "Fisica Delay (Bypass Anti-Cheat)",
   Min = 0.05,
   Max = 0.5,
   Default = 0.12,
   Color = Color3.fromRGB(0, 255, 150),
   Increment = 0.01,
   ValueName = "s",
   Callback = function(Value)
        TeleportDelay = Value
   end,
})

-- ==========================================
-- BUCLE PRINCIPAL DE EJECUCIÓN (LÓGICA PERFECTA)
-- ==========================================

-- Hilo Principal de Minado Continuo e Instantáneo
task.spawn(function()
    while true do
        task.wait(0.05) -- Ciclo rápido de verificación
        
        if AutoFarmActive then
            local target = GetClosestCrystal()
            if target then
                local root = LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart")
                if root then
                    -- 1. Separar los datos usando el analizador inteligente
                    local rawText = target.Prompt.ObjectText
                    local crystalName, crystalWeight, crystalPrice = ParseCrystalDetails(rawText)
                    local distance = math.floor(target.Distance)
                    
                    -- 2. Mostrar la información estructurada en el panel
                    InfoParagraph:Set({
                        Title = "💎 Minando: " .. crystalName,
                        Content = string.format(
                            "⚖️ Peso: %s\n💰 Precio: %s\n📏 Distancia: %d studs",
                            crystalWeight, crystalPrice, distance
                        )
                    })
                    
                    -- 3. Teletransportarse arriba del cristal de forma segura
                    if AutoTeleport then
                        root.CFrame = target.Position * CFrame.new(0, 2.5, 0)
                        task.wait(TeleportDelay) -- Pequeño margen para la estabilidad de la física
                    end
                    
                    -- 4. Activar la recolección
                    if fireproximityprompt then
                        fireproximityprompt(target.Prompt)
                    else
                        target.Prompt:InputHoldBegin()
                        task.wait(target.Prompt.HoldDuration + 0.05)
                        target.Prompt:InputHoldEnd()
                    end
                    
                    -- 5. TRANSICIÓN PERFECTA AL INSTANTE:
                    -- Espera activamente en un bucle ultra-rápido hasta que el cristal actual
                    -- deje de existir en el juego o se desactive su prompt.
                    local maxWaitTime = tick()
                    while AutoFarmActive and target.Instance:IsDescendantOf(Workspace) and target.Prompt.Enabled do
                        -- Margen de seguridad por si el cristal se bugea y no desaparece en 3 segundos
                        if tick() - maxWaitTime > 3 then 
                            break 
                        end
                        task.wait(0.02) -- Frecuencia de escaneo súper rápida
                    end
                    
                    -- En cuanto el bucle anterior termina, la ejecución sigue de inmediato y busca el siguiente cristal.
                end
            else
                InfoParagraph:Set({
                    Title = "Buscando...",
                    Content = "No se encontraron cristales de la categoría seleccionada en el mapa."
                })
            end
        end
    end
end)

-- Hilo Secundario: Auto-Venta Periódica (Vende cada 20 segundos si estás lleno)
task.spawn(function()
    while true do
        task.wait(20)
        if AutoSellActive and AutoFarmActive then
            local sellPrompt = Workspace:FindFirstChild("Things") and Workspace.Things:FindFirstChild("SellProx") and Workspace.Things.SellProx:FindFirstChild("ProximityPrompt")
            local root = LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart")
            
            if sellPrompt and root then
                local oldPos = root.CFrame
                
                -- Viaje rápido al vendedor
                root.CFrame = Workspace.Things.SellProx.CFrame * CFrame.new(0, 3, 0)
                task.wait(0.25)
                
                if fireproximityprompt then
                    fireproximityprompt(sellPrompt)
                end
                task.wait(0.2)
                
                -- Regreso instantáneo para seguir recolectando
                root.CFrame = oldPos
            end
        end
    end
end)
