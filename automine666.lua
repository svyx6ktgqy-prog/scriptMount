...
local Window = Rayfield:CreateWindow({
   Name = "Ninja Harvester 💎 | V7 Tracker",
   LoadingTitle = "Cargando Motores Visuales...",
...
-- Variables de Exploración Manual y ESP
local manualList = {}
local manualIndex = 1
local manualTarget = nil
local ESPGuideActive = false

-- Almacén para los objetos del ESP visual
local ESPElements = {
    Beam = nil,
    AttPlayer = nil,
    AttTarget = nil,
...
-- =======================================================================
-- FUNCIONES DEL ESP VISUAL Y GUÍA
-- =======================================================================
...
local function CreateESP(target)
    ...
    -- 1. Crear los anclajes para la línea (Beam)
    ...
    -- 2. Crear la Malla Cuadrada (BoxHandleAdornment)
    local meshBox = Instance.new("BoxHandleAdornment", part)
    meshBox.Adornee = part
    meshBox.Size = part.Size + Vector3.new(0.5, 0.5, 0.5) -- Ligeramente más grande que el diamante
    meshBox.Color3 = Color3.fromRGB(0, 255, 255)
    meshBox.Transparency = 0.6
    meshBox.ZIndex = 10
    meshBox.AlwaysOnTop = true

    -- 3. Crear el Texto de Distancia Flotante
    local bgui = Instance.new("BillboardGui", part)
    bgui.Size = UDim2.new(0, 200, 0, 50)
    bgui.StudsOffset = Vector3.new(0, 3, 0) -- Un poco arriba del cristal
    ...
    -- Guardar en memoria para poder borrarlo luego
    ESPElements.AttPlayer = att0
    ESPElements.AttTarget = att1
    ESPElements.Beam = beam
    ESPElements.MeshBox = meshBox
    ESPElements.BillGui = bgui
    ESPElements.Text = txt
    
    -- Bucle para actualizar los metros en tiempo real mientras caminas
    ESPElements.Loop = RunService.Heartbeat:Connect(function()
        if hrp and part then
            local dist = (hrp.Position - part.Position).Magnitude
            txt.Text = string.format("💎 %d Metros", math.floor(dist))
        else
            ClearESP()
        end
    end)
...
-- =======================================================================
FarmTab:CreateSection("⛏️ Controles Globales (Auto y Manual)")
...
FarmTab:CreateInput({
   Name = "💰 Precio Mínimo ($)",
   PlaceholderText = "Borra el texto para buscar Todos",
   RemoveTextAfterFocusLost = false,
   Callback = function(Text)
        if Text == "" or Text == nil then
            MinPriceFilter = 0
            Rayfield:Notify({Title = "Filtro Restablecido", Content = "Buscando todos los precios (Default).", Duration = 3})
        else
...
FarmTab:CreateSection("🔍 Explorador Manual (Físicas Naturales)")

-- NUEVO INTERRUPTOR ESP
FarmTab:CreateToggle({
   Name = "Activar ESP de Seguimiento (Guía, Línea y Malla)",
   CurrentValue = false,
   Flag = "ESPGuideToggle",
   Callback = function(Value)
        ESPGuideActive = Value
        if Value and manualTarget then
            CreateESP(manualTarget)
        else
            ClearESP()
        end
   end,
...
local function CycleManual(direction)
    ...
    if #manualList == 0 then
        ManualInfoParagraph:Set({Title = "Error", Content = "No hay cristales que cumplan los filtros actuales."})
        ClearESP()
        manualTarget = nil
        return
    end
    ...
    -- Actualizar el ESP al nuevo objetivo si está encendido
    CreateESP(manualTarget)
end
   ...
   Callback = function()
        ...
        if part and prompt then
            task.spawn(function()
                ...
                ClearESP() -- Borrar la guía al extraerlo
