...
local Window = Rayfield:CreateWindow({
   ...
   LoadingTitle = "Iniciando ViewModel Framework...",
   LoadingSubtitle = "Sistema Experto (Focal Calibrated)",
...
-- ==========================================
-- MATH & PROCEDURAL UTILITIES (MODO EXPERTO)
-- ==========================================

-- Framerate-independent Lerp (Decaimiento exponencial)
local function expLerp(a, b, rate, dt)
    return a + (b - a) * (1 - math.exp(-rate * dt))
end

-- Cálculo trigonométrico para sensibilidad 1:1 (Focal Length Tracking)
local function getFocalSensitivity(baseSens, currentFOV, baseFOV)
    local radCurrent = math.rad(currentFOV)
    local radBase = math.rad(baseFOV)
    return baseSens * (math.tan(radCurrent / 2) / math.tan(radBase / 2))
end
   ...
   Callback = function()
       local success, errorMessage = pcall(function()
           ...
           if Player.Backpack:FindFirstChild("Binoculars_ULTRA") or character:FindFirstChild("Binoculars_ULTRA") then
               Rayfield:Notify({Title = "Aviso", Content = "El equipo ya está desplegado.", Duration = 3})
               return
           end

           -- Tool completamente vacío y abstracto (Invisible en la mano)
           ...
           local defaultFOV = Camera.FieldOfView -- Dinámico basado en las preferencias del usuario
           ...
           local function toggleAim()
               ...
               if isAiming then
                   ...
                   -- Restaurar sensibilidad base sin alterar
           ...
           -- LÓGICA DE EQUIPADO (CREACIÓN DEL VIEWMODEL CLONADO Y ROBUSTO)
           tool.Equipped:Connect(function()
               aimButton.Visible = true 
               
               -- 1. Crear el ViewModel Local de forma Quirúrgica
               ...
               binoPart.Anchored = true 
               binoPart.CanCollide = false
               binoPart.Massless = true
               binoPart.CastShadow = false
               binoPart.Parent = viewModel
               
               -- Carga de Mesh robusta (previene crasheos si el AssetID falla)
               ...
               -- 2. RenderStepped LOOP (Motor Procedural)
               renderConnection = RunService.RenderStepped:Connect(function(deltaTime)
                   local lerpRate = 12
                   local alpha = 1 - math.exp(-lerpRate * deltaTime) -- Independencia de FPS

                   -- A. MATEMÁTICA DE ZOOM Y CALIBRACIÓN DE SENSIBILIDAD
                   local targetFOV = isAiming and (defaultFOV / targetZoomLevel) or defaultFOV
                   currentFOV = expLerp(currentFOV, targetFOV, lerpRate, deltaTime)
                   Camera.FieldOfView = currentFOV
                   
                   if isAiming then
                       actualSliderY = expLerp(actualSliderY, targetSliderY, lerpRate, deltaTime)
                       sliderKnob.Position = UDim2.new(0, 0, 0, actualSliderY)
                       
                       -- Sensibilidad Quirúrgica 1:1 Focal
                       pcall(function()
                           userSettings.MouseSensitivity = getFocalSensitivity(baseSensitivity, currentFOV, defaultFOV)
                       end)
                   end
                   ...
                   local swayMult = isAiming and 0.0005 or 0.002
           ...
           tool.Unequipped:Connect(function()
               ...
               -- Restauración de seguridad rigurosa
               pcall(function() userSettings.MouseSensitivity = baseSensitivity end)
           end)

           tool.Parent = Player.Backpack
           Rayfield:Notify({Title = "ULTRA SYSTEM ACTIVE", Content = "Procedural ViewModel & Focal Calibration Injected.", Duration = 5})
       end)

       if not success then
           warn("ULTRA CRITICAL ERROR: " .. tostring(errorMessage))
           Rayfield:Notify({ Title = "SYSTEM FAULT", Content = tostring(errorMessage), Duration = 10 })
       end
   end,
})
