...
local Window = Rayfield:CreateWindow({
   Name = "Tactical Gear: ULTRA V5.0",
   LoadingTitle = "Applying Surgical Smoothing...",
   LoadingSubtitle = "Dual Calibrators & Smooth Pan",
...
Tab:CreateButton({
   ...
   Callback = function()
       local success, errorMessage = pcall(function()
           ...
           -- Calibrador de ZOOM (Vertical Derecho)
           ...
           -- Calibrador de SENSIBILIDAD (Horizontal Superior)
           local sliderSensBg = Instance.new("TextButton")
           sliderSensBg.Size = UDim2.new(0.3, 0, 0, 40)
           sliderSensBg.Position = UDim2.new(0.65, -20, 0.05, 0) -- Área superior derecha
           ...
           -- Variables Zoom Slider
           local targetZoomSliderY = 0
           local actualZoomSliderY = 0
           local draggingZoom = false
           
           -- Variables Sensibilidad Slider
           local targetSensSliderX = 0
           local actualSensSliderX = 0
           local draggingSens = false
           local lentitudMultiplier = 1 -- 1x a 100x de lentitud
           
           -- Variables Smooth Camera Pan
           ...
           local function updateSensSlider(inputPos)
               ...
               targetSensSliderX = relativeX
               local percentage = relativeX / maxX
               lentitudMultiplier = 1 + (percentage * 99) -- Rango de 1 a 100
               sliderSensBg.Text = "PRECISION x" .. math.floor(lentitudMultiplier)
           end

           -- Conexiones Zoom Slider
           ...
           -- Conexiones Sensibilidad Slider
           ...
           UserInputService.InputChanged:Connect(function(input, gameProcessed)
               ...
               -- INTERCEPTAR CÁMARA TÁCTIL/RATÓN SIN SALTOS
               if isAiming and not draggingZoom and not draggingSens and not gameProcessed then
                   if input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch then
                       local delta = input.Delta
                       local fovRatio = currentFOV / defaultFOV
                       
                       -- Sensibilidad base pura de cámara procedural
                       local baseTurnSpeed = 0.005 
                       -- Reducir velocidad por el FOV (zoom) y por nuestro Slider (Lentitud x1 a x100)
                       local finalMultiplier = (baseTurnSpeed * fovRatio) / lentitudMultiplier
                       
                       targetCamRotation -= Vector2.new(delta.Y * finalMultiplier, delta.X * finalMultiplier)
                       -- Limitar rotación arriba/abajo para no dar volteretas
           ...
           local function toggleAim()
               ...
               if isAiming then
                   ...
                   -- Inicializar variables de rotación para evitar saltos al apuntar
                   ...
                   Camera.CameraType = Enum.CameraType.Scriptable -- Forzamos control total manual
                   ...
                   Camera.CameraType = previousCameraType -- Restaurar cámara normal
           ...
           -- ==========================================
           -- RENDER LOOP (ANIMACIONES Y LERP)
           -- ==========================================
           tool.Equipped:Connect(function()
               ...
               renderConnection = RunService.RenderStepped:Connect(function(deltaTime)
                   ...
                   if isAiming then
                       ...
                       actualSensSliderX = lerp(actualSensSliderX, targetSensSliderX, smoothSpeed)
                       sliderSensKnob.Position = UDim2.new(0, actualSensSliderX, 0, 0)
                       
                       -- [FIX DEFINITIVO TÁCTIL]: Suavizado de Cámara Scriptable
                       ...
                       -- Anclar la cámara a la cabeza del jugador
                       local char = Player.Character
                       if char and char:FindFirstChild("Head") then
                           local headPos = char.Head.Position + Vector3.new(0, 0.5, 0)
                           Camera.CFrame = CFrame.new(headPos) * CFrame.Angles(currentCamRotation.X, currentCamRotation.Y, 0)
                       end
                   end
           ...
           Rayfield:Notify({Title = "V5.0 ACTIVE", Content = "Dual Calibrators & Scriptable Pan Injected.", Duration = 4})
       end)
