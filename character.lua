-- ==========================================================
-- LOOKS PRO VISUAL v8
-- Miniaturas PRO: personaje base + items del Look renderizados
-- Keep on Death ON • 👁 items/partes • Long-press save
-- ==========================================================
...
local CurrentPage       = 1
local PAGE_SIZE         = 10          -- un poco menos porque los Viewport pesan más
...
-- ==========================================================
-- RENDER DE MINIATURA PRO (Viewport + personaje base + items)
-- ==========================================================
local function RenderLookThumbnail(viewport, look)
    ...
    task.spawn(function()
        local baseChar = LocalPlayer.Character
        if not baseChar then return end

        baseChar.Archivable = true
        local dummy = baseChar:Clone()
        baseChar.Archivable = false

        -- Limpiar el dummy completamente
        for _, v in ipairs(dummy:GetChildren()) do
            if v:IsA("Accessory") or v:IsA("Shirt") or v:IsA("Pants") or v:IsA("ShirtGraphic")
                or v:IsA("Script") or v:IsA("LocalScript") or v:IsA("Tool") then
                v:Destroy()
            end
        end

        -- Asegurar que las partes sean visibles
        ...
        -- Forzar assets individuales también (por si el OutfitId no cubre todo)
        local items = look.items or look.assets or look.assetsIncludingBundleParts or {}
        for _, it in ipairs(items) do
            local id = it.id or it.assetId
            if id then
                ForceEquipAsset(id, dummy)
            end
        end
        ...
        if hrp then
            -- Ángulo pro (ligeramente desde arriba y de lado)
            cam.CFrame = CFrame.new(
                hrp.Position + Vector3.new(0, 1.8, 5.2),
...
EyeBtn.Position = UDim2.new(1, IS_MOBILE and -72 or -66, 0.32, 0)
...
Main.Size = IS_MOBILE and UDim2.new(0.92, 0, 0.82, 0) or UDim2.new(0, 420, 0, 580)
Main.BackgroundColor3 = Color3.fromRGB(16, 16, 16)
Main.Visible = false
Main.ClipsDescendants = true
Main.Parent = Gui
Instance.new("UICorner", Main).CornerRadius = UDim.new(0, 16)
Instance.new("UISizeConstraint", Main).MaxSize = Vector2.new(500, 680)
...
Title.Text = "👀 Looks Pro v8 • Miniaturas PRO"
...
StatusLbl.Text = "Keep on Death ON • Miniaturas con personaje + items"
...
Grid.CellSize = UDim2.new(0, 118, 0, 155)   -- un poco más alto para el Viewport
...
local function CreateCard(look)
    ...
    -- ViewportFrame = miniatura pro
    local Viewport = Instance.new("ViewportFrame")
    Viewport.Size = UDim2.new(1, -8, 0, 105)
    ...
    -- Renderiza el personaje base + items del Look
    RenderLookThumbnail(Viewport, look)

    local Name = Instance.new("TextLabel")
    Name.Size = UDim2.new(1, -6, 0, 34)
    Name.Position = UDim2.new(0, 3, 0, 112)
...
SearchBox:GetPropertyChangedSignal("Text"):Connect(function()
    ...
    searchTask = task.spawn(function()
        ...
        for i, id in ipairs(ids) do
            ...
            if #batch >= 3 or i == math.min(#ids, 24) then
                local threads = {}
...
FloatBtn.InputEnded:Connect(function(input)
    if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
        if holding and (tick() - holdStart) >= 0.55 then
            ...
            Skins[name] = { name = name, ids = ids, lookIds = SavedLookIds, savedAt = os.time() }
            SaveJSON(SKINS_FILE, Skins)
            Notify("💾 Guardado", name .. " → skins.json", 3)
        else
...
Notify("Looks Pro v8 listo", "Miniaturas PRO: personaje base + items del Look", 5)
