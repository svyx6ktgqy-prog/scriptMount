...
local customAnimators = {} -- Tabla para manejar animaciones de los clones
...
-- ==========================================
-- MOTOR DE ANIMACIONES FLUIDAS (CORREGIDO)
-- ==========================================
local function setupAnimations(character)
    ...
    -- Limpieza estricta del script Animate nativo para evitar interferencias en clones
    if character ~= originalCharacter then
...
local function updateAnimations()
    ...
    for _, body in ipairs(bodies) do
        -- Controlamos las animaciones de los clones (o del original si se convirtió en IA)
        if body ~= LocalPlayer.Character then
            ...
            if root and hum and animData then
                -- Uso de AssemblyLinearVelocity (estándar físico moderno de Roblox)
                ...
                -- Jerarquía de estados limpia sin saltos bruscos
                if humState == Enum.HumanoidStateType.Jumping then
                    targetState = "jump"
                elseif humState == Enum.HumanoidStateType.Freefall then
                    targetState = "fall"
                elseif flatSpeed > 10 then
                    targetState = "run"
                elseif flatSpeed > 0.8 then
                    targetState = "walk"
                end

                -- Transición suave entre animaciones (FadeTime de 0.25s)
                if animData.currentState ~= targetState then
                    if animData.currentState ~= "none" and animData.tracks[animData.currentState] then
                        animData.tracks[animData.currentState]:Stop(0.25)
                    end
                    if targetState ~= "none" and animData.tracks[targetState] then
                        animData.tracks[targetState]:Play(0.25)
                    end
                    animData.currentState = targetState
                end
                
                -- Ajuste dinámico de velocidad visual de pierna sin temblar (lerp suave)
                if targetState == "walk" or targetState == "run" then
                    local track = animData.tracks[targetState]
                    if track and track.IsPlaying then
                        local targetSpeed = math.clamp(flatSpeed / 14, 0.6, 1.4)
                        animData.currentSpeed = animData.currentSpeed + (targetSpeed - animData.currentSpeed) * 0.1
                        track:AdjustSpeed(animData.currentSpeed)
                    end
                end
            end
        else
            -- Si este cuerpo es el personaje principal, aseguramos que su script nativo corra
...
local function switchCharacter(newIndex)
    ...
    if oldChar and oldChar:FindFirstChild("Humanoid") then
        -- 1. Detener su movimiento forzosamente para que no siga corriendo solo
        oldChar.Humanoid:MoveTo(oldChar.HumanoidRootPart.Position)
        
        -- 2. Limpiar animaciones que se hayan quedado "pegadas"
    ...
    if targetChar and targetChar:FindFirstChild("Humanoid") then
        ...
        -- 3. Forzar refresco de la cámara para evitar la vista vertical estática
...
RunService.Heartbeat:Connect(function()
    ...
    if cloneCount > 0 then
        ...
        for i, body in ipairs(movingClones) do
            ...
            -- ==========================================
            -- MOVIMIENTO Y ROTACIÓN (SIN EFECTO DESLIZANTE)
            -- ==========================================
            if distToTarget2D > 2.5 then -- Zona muerta ampliada (evita baile al llegar)
                hum.AutoRotate = true
                ...
                -- AL LLEGAR: Detenemos la orden explícitamente para que actúe el Idle y no patinen
                ...
                if not isPlayerMoving then
                    hum.AutoRotate = true
                    -- Orientamos al clon hacia ti sin romper la fricción del suelo (adiós moonwalk)
                    local lookAtPos = Vector3.new(activePos.X, root.Position.Y, activePos.Z)
                    local targetDir = (lookAtPos - root.Position).Unit
                    
                    if root.CFrame.LookVector:Dot(targetDir) < 0.95 then
...
PlayerTab:CreateButton({
   ...
   Callback = function()
       ...
       if originalCharacter and originalCharacter:FindFirstChild("HumanoidRootPart") then
           ...
           setupAnimations(clone) -- Inicializa sus animaciones extraídas de forma limpia
    ...
    Callback = function()
        ...
        for _, clone in ipairs(cloneList) do
            if clone and clone:FindFirstChild("Humanoid") then
                customAnimators[clone] = nil -- Limpiar caché de animaciones
...
Rayfield:Notify({
   Title = "Parche Quirúrgico Aplicado",
   Content = "Físicas y animaciones sincronizadas (AssemblyVelocity + zona muerta ampliada).",
   Duration = 4,
   Image = 4483362458,
})
