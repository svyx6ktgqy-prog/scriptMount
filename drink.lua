...
mainFrame.Size = UDim2.new(0, 260, 0, 50) -- Ancho incrementado para el nuevo botón
...
local btnJump = createBtn("^") -- NUEVO BOTÓN DE SALTO MASIVO

-- Eventos del botón de salto masivo (Mantener presionado)
...
-- ==========================================
-- LÓGICA DEL ENJAMBRE (Formación, Mirada al detenerse y Salto Inteligente)
-- ==========================================
...
RunService.Heartbeat:Connect(function()
    ...
    for i, body in ipairs(movingClones) do
        ...
        -- CÁLCULO 2D (Ignorando Eje Y) PARA SABER SI ESTÁN PARADOS EN UN OBSTÁCULO
        ...
        -- 3. SALTO MASIVO FORZADO (Vía botón flotante)
        if isSwarmJumping then
            hum.Jump = true
        end

        -- 4. MOVIMIENTO, ROTACIÓN Y AUTO-SALTO INTELIGENTE
        if distToTarget2D > 1.5 then
            -- AÚN NO LLEGAN: Se mueven y auto-saltan
            hum.AutoRotate = true
            hum:MoveTo(targetPosition)
            
            -- PROXIMITY JUMP (Raycast individual, un poco más alto para no detectar el piso base)
            ...
            -- YA LLEGARON (incluso si están parados arriba de un obstáculo)
            if not isPlayerMoving then
                -- Clavan la mirada en el líder
...
-- (BLOQUE BORRADO: UserInputService.JumpRequest ha sido eliminado completamente para evitar saltos simultáneos erróneos)
...
Rayfield:Notify({
   ...
   Content = "Salto masivo [^] añadido. Interacciones de obstáculos corregidas.",
   Duration = 3,
   Image = 4483362458,
})
