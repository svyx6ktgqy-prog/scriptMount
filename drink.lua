-- ==========================================
-- TOWER OF CANS: SURGICAL ESP & SMART SWARM AI + FINAL ANIMATION FIX
-- ==========================================
...
-- Motor de Animaciones Customizado (Soluciona el bug de congelamiento y clons parados)
local function updateAnimations()
    local bodies = getAllBodies()
    local activeChar = LocalPlayer.Character
    local activeHum = activeChar and activeChar:FindFirstChild("Humanoid")
    local isPlayerWalking = activeHum and (activeHum.MoveDirection.Magnitude > 0.05)
        ...
        if body ~= originalCharacter or (body == originalCharacter and body ~= LocalPlayer.Character) then
            ...
            if root and hum and animData then
                ...
                elseif humState == Enum.HumanoidStateType.Freefall then
                    ...
                    -- CONDICIÓN CORREGIDA: Si el clon se mueve, si rota, o SI EL JUGADOR ESTÁ CAMINANDO (para seguírteme)
                    if speed > 0.1 or isRotating or isMovingToTarget or (followingEnabled and isPlayerWalking) then
                        ...
                        if animData.lastMoveTime and (tick() - animData.lastMoveTime < 0.35) then
                ...
                if targetState == "walk" or targetState == "run" then
                    ...
                    if track then
                        ...
                        elseif isRotating then
                            ...
                            playbackSpeed = 0.9 -- Velocidad constante fluida al seguir al jugador
                        end
            ...
            -- Si es el jugador actual original controlado por ti, nos aseguramos que su Animate nativo no se quede pegado
            local animScript = body:FindFirstChild("Animate")
            local hum = body:FindFirstChild("Humanoid")
            if animScript and animScript:IsA("LocalScript") then
                if hum and hum.MoveDirection.Magnitude == 0 then
                    animScript.Disabled = true
                    task.delay(0.05, function() if animScript then animScript.Disabled = false end end)
                else
...
RunService.Heartbeat:Connect(function()
    ...
    if cloneCount > 0 then
        ...
        for i, body in ipairs(movingClones) do
            ...
            elseif distToTarget2D < 1.0 then 
   ...
   Callback = function()
       ...
       if originalCharacter and originalCharacter:FindFirstChild("HumanoidRootPart") then
           ...
           Rayfield:Notify({
               ...
               Content = "Añadido a la formación con animaciones sincronizadas.",
               Duration = 3,
           })
       end
   end,
...
Rayfield:Notify({
   Title = "Parche Final Aplicado",
   Content = "Animación de tu personaje y clones sincronizadas perfectamente.",
   Duration = 4,
   Image = 4483362458,
})
