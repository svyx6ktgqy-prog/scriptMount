...
local Window = Rayfield:CreateWindow({
   ...
   LoadingSubtitle = "CoinGiver Suite | Delta iOS Compatible",
...
-- Filtros de Recolección (Monedas puras vs Losas/Checkpoints)
local coinKeywords = {"coin", "moneda", "reward", "giver", "10000", "star", "diamond", "gold", "token"}
local checkpointKeywords = {"stage", "checkpoint", "check", "spawn", "pad", "losa", "plate", "baseplate", "winpad"}
local blacklistedKeywords = {"admin", "fling", "explode", "topmenu", "currency", "shop", "store"}

-- Historial para el Inspector Móvil (Delta)
local logHistory = {}
local logTextBuffer = "Sin logs registrados aún..."
...
-- Portapapeles Universal para Delta / Executores Móviles
...
-- ========================================================
-- SISTEMA DE FILTRO EXCLUSIVO PARA MONEDAS Y RECOMPENSAS
-- ========================================================
local function isOnlyCoinOrReward(obj)
    ...
    -- 1. DESCARTAR si es un checkpoint, losa de parkour o spawn pad
    for _, badKw in ipairs(checkpointKeywords) do
        if name:find(badKw) or parentName:find(badKw) then
            return false
        end
    end

    -- 2. ACEPTAR solo si coincide con palabras clave de Monedas / Premios
...
-- ========================================================
-- RECOLECCIÓN DIRECTA A MONEDAS CON INSPECTOR EN UI
-- ========================================================
local LogParagraph = LogTab:CreateParagraph({
    Title = "📋 Últimos Objetos Detectados",
...
local function collectMapCoinsDirect()
    ...
    for _, obj in ipairs(workspace:GetDescendants()) do
        if isOnlyCoinOrReward(obj) and obj.Position.Y > 20 then
    ...
    for index, obj in ipairs(targetCoins) do
        ...
        if hrp and obj and obj.Parent then
            ...
            local logMsg = "[" .. os.date("%X") .. "] Moneda: " .. obj.Name .. " | Ruta: " .. obj.Parent.Name
            addLogEntry(logMsg)
...
-- Botón para copiar al portapapeles del iPhone (Delta)
LogTab:CreateButton({
   ...
   Callback = function()
      ...
      Rayfield:Notify({
         Title = "Copiado",
         Content = "El historial de monedas se copió al portapapeles de tu teléfono.",
         Duration = 3,
         Image = 4483362458
      })
   end,
...
LogTab:CreateButton({
   ...
   Callback = function()
      ...
      LogParagraph:Set({ Title = "📋 Últimos Objetos Detectados", Content = logTextBuffer })
   end,
})
...
-- Switch Recolectar Solo Monedas Reales
FarmTab:CreateToggle({
   Name = "🪙 Solo Monedas y Premios (Sin Checkpoints)",
   CurrentValue = false,
   Flag = "ToggleMapCoins",
   Callback = function(Value)
