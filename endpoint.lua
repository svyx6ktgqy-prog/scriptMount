...
local Window = Rayfield:CreateWindow({
   ...
   LoadingSubtitle = "CoinGiver Suite | Fixed Multi-Endpoint Engine",
...
-- LISTA NEGRA (Solo para Switch 2)
...
-- RÁFAGA DE INTERACCIÓN HIPER RÁPIDA (RÁFAGA MULTI-PULSO)
local function hyperInteract(hrp, target)
    ...
    -- Ejecutar ráfaga instantánea de 3 pulsos para asegurar registro inmediato en servidor
    for _ = 1, 3 do
...
-- BUSCADOR UNIVERSAL DE ENDPOINTS (500 A 9.999)
local function getSmartEndpoints()
    ...
    local function checkAndAdd(obj)
        ...
        -- Limpiar caracteres no numéricos como puntos o comas (ej. "1.250" -> 1250)
    ...
    local function scanFolder(folder)
        ...
        for _, child in ipairs(folder:GetChildren()) do
            checkAndAdd(child)
            -- Si es un modelo, escanea su contenido interno por si las partes tienen el nombre numérico
    ...
    -- Búsqueda exhaustiva en todas las ubicaciones posibles del mapa
...
-- Switch 3: NUEVO - Multi-Endpoint High Velocity (500 a 9.999 / Sin TP)
FarmTab:CreateToggle({
   Name = "⚡ Extreme Endpoints (500 a 9.999 / Sin TP / Ultra Velocidad)",
   CurrentValue = false,
         ...
         task.spawn(function()
            while highGiversActive do
               ...
               if hrp then
                  ...
                  -- Procesar la totalidad de endpoints de forma masiva e instantánea
               ...
               -- Intervalo mínimo para máxima velocidad de respuesta sin congelar el cliente
               task.wait(0.01)
            end
         end)
      end
