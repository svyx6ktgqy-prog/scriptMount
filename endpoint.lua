-- Cargar Rayfield UI
local Rayfield = loadstring(game:HttpGet('https://sirius.menu/rayfield'))()

local Window = Rayfield:CreateWindow({
   Name = "Ultimate Auto Farm - Endpoint 10000",
   LoadingTitle = "Cargando Script...",
   LoadingSubtitle = "CoinGiver Suite | Claimer & Sweep",
   ConfigurationSaving = { Enabled = false },
   Discord = { Enabled = false },
   KeySystem = false
})

local FarmTab = Window:CreateTab("Auto Farm", 4483362458)
local GuiTab = Window:CreateTab("Auto Recompensas", 4483362458)

-- Configuración y Estados
local targetPos = Vector3.new(-109.99999237060547, 482.2499084472656, 119.49998474121094)
local autoFarmActive = false
local smartFarmActive = false
local ghostFarmActive = false
local guiClaimActive = false

-- Palabras clave para detectar botones de reclamo en la GUI
local claimKeywords = {"claim", "reclamar", "collect", "get", "take", "free", "reward", "recoger", "obtener"}

-- Funciones auxiliares
local function getCoinPart()
    return workspace:FindFirstChild("Map") 
       and workspace.Map:FindFirstChild("CoinGivers") 
       and workspace.Map.CoinGivers:FindFirstChild("10000")
end

local function getHRP()
    local player = game.Players.LocalPlayer
    return player and player.Character and player.Character:FindFirstChild("HumanoidRootPart")
end

local function getHumanoid()
    local player = game.Players.LocalPlayer
    return player and player.Character and player.Character:FindFirstChildOfClass("Humanoid")
end

local function resetMomentum(hrp)
    if hrp then
        hrp.AssemblyLinearVelocity = Vector3.zero
        hrp.AssemblyAngularVelocity = Vector3.zero
    end
end

-- ========================================================
-- NÚCLEO 1: BARRIDO FÍSICO DE MONEDAS (Sweep Teleport)
-- ========================================================
local function collectFloatingCoinsSweep(hrp)
    if not hrp then return end
    local originalCFrame = hrp.CFrame
    
    for _, obj in ipairs(workspace:GetDescendants()) do
        if not smartFarmActive then break end
        
        if obj:IsA("BasePart") and obj.Name ~= "10000" and not obj.Parent:FindFirstChildOfClass("Humanoid") then
            local name = string.lower(obj.Name)
            local isCoin = name:find("coin") or name:find("moneda") or obj:FindFirstChildOfClass("TouchTransmitter")
            
            if isCoin then
                pcall(function()
                    -- Mover temporalmente al personaje a la moneda para engañar la validación de distancia del servidor
                    resetMomentum(hrp)
                    hrp.CFrame = obj.CFrame
                    task.wait(0.03)
                    
                    -- Intento 1: Touch Interest
                    if firetouchinterest then
                        firetouchinterest(hrp, obj, 0)
                        task.wait(0.01)
                        firetouchinterest(hrp, obj, 1)
                    end
                    
                    -- Intento 2: ProximityPrompt (Si la moneda usa interactuador)
                    local prompt = obj:FindFirstChildOfClass("ProximityPrompt") or obj.Parent:FindFirstChildOfClass("ProximityPrompt")
                    if prompt and fireproximityprompt then
                        fireproximityprompt(prompt)
                    end
                end)
            end
        end
    end
    
    -- Retorno seguro a la base
    if hrp then
        resetMomentum(hrp)
        hrp.CFrame = originalCFrame
    end
end

-- ========================================================
-- NÚCLEO 2: ESCÁNER Y RECLAMADOR DE GUI POR PALABRAS CLAVE
-- ========================================================
local function clickGuiElement(btn)
    pcall(function()
        -- Forzar visibilidad jerárquica
        btn.Visible = true
        local p = btn.Parent
        while p and p:IsA("GuiObject") do
            p.Visible = true
            p = p.Parent
        end
        
        -- Ejecución muticapa de eventos de clic
        if getconnections then
            for _, conn in ipairs(getconnections(btn.MouseButton1Click)) do conn:Fire() end
            for _, conn in ipairs(getconnections(btn.MouseButton1Down)) do conn:Fire() end
            for _, conn in ipairs(getconnections(btn.Activated)) do conn:Fire() end
        elseif firesignal then
            firesignal(btn.MouseButton1Click)
            firesignal(btn.Activated)
        end
    end)
end

local function autoClaimAllGuiRewards()
    local player = game.Players.LocalPlayer
    if not player or not player:FindFirstChild("PlayerGui") then return end
    
    for _, guiObject in ipairs(player.PlayerGui:GetDescendants()) do
        if not guiClaimActive then break end
        
        if guiObject:IsA("GuiButton") then
            local btnName = string.lower(guiObject.Name)
            local btnText = guiObject:IsA("TextButton") and string.lower(guiObject.Text) or ""
            local shouldClick = false
            
            -- 1. Coincidencia por palabra clave en Nombre o Texto
            for _, kw in ipairs(claimKeywords) do
                if btnName:find(kw) or btnText:find(kw) then
                    shouldClick = true
                    break
                end
            end
            
            -- 2. Coincidencia por estructura de slots (Ej: Slots.1.Button o Slots.2000.Button)
            if not shouldClick and guiObject.Parent then
                local parentName = string.lower(guiObject.Parent.Parent and guiObject.Parent.Parent.Name or "")
                if parentName == "slots" or parentName == "playrewards" or parentName == "coins" then
                    shouldClick = true
                end
            end
            
            if shouldClick then
                clickGuiElement(guiObject)
                task.wait(0.05) -- Pausa rápida para procesar reclamo
            end
        end
    end
end

-- ========================================================
-- SECCIÓN DE PESTAÑAS Y SWITCHES
-- ========================================================

-- Switch Auto Reclamo de GUI Inteligente
GuiTab:CreateToggle({
   Name = "🎯 Auto Claimer Inteligente (Keywords + Slots)",
   CurrentValue = false,
   Flag = "ToggleSmartClaim",
   Callback = function(Value)
      guiClaimActive = Value
      if guiClaimActive then
         task.spawn(function()
            while guiClaimActive do
               autoClaimAllGuiRewards()
               -- Pausa entre rondas de escaneo de GUI
               for _ = 1, 30 do
                  if not guiClaimActive then break end
                  task.wait(0.1)
               end
            end
         end)
      end
   end,
})

GuiTab:CreateButton({
   Name = "⚡ Reclamar Todo la GUI Ahora (1-Shot)",
   Callback = function()
      guiClaimActive = true
      autoClaimAllGuiRewards()
      guiClaimActive = false
   end,
})

-- Switch Smart Farm con Barrido Físico de Monedas
FarmTab:CreateToggle({
   Name = "⚡ Smart Switch (Terreno + Salto + Barrido Monedas)",
   CurrentValue = false,
   Flag = "ToggleSmart",
   Callback = function(Value)
      smartFarmActive = Value
      if smartFarmActive then
         task.spawn(function()
            while smartFarmActive do
               local hrp = getHRP()
               local humanoid = getHumanoid()
               local coinPart = getCoinPart()
               local basePos = coinPart and coinPart.Position or targetPos
               
               if hrp and humanoid and humanoid.Health > 0 then
                  resetMomentum(hrp)
                  hrp.CFrame = CFrame.new(basePos + Vector3.new(15, 5, 0))
                  task.wait(0.4) 
                  
                  if not smartFarmActive or humanoid.Health <= 0 then break end
                  
                  resetMomentum(hrp)
                  hrp.CFrame = CFrame.new(basePos + Vector3.new(0, 35, 0))
                  
                  local isJumping = false
                  local fallTimeout = tick()
                  
                  repeat
                     task.wait(0.05)
                     hrp = getHRP()
                     humanoid = getHumanoid()
                     
                     if not isJumping and hrp and humanoid and hrp.Position.Y < (basePos.Y + 5) then
                         humanoid.Jump = true
                         isJumping = true
                     end
                     
                  until not smartFarmActive or not hrp or (hrp.Position.Y < (basePos.Y - 50)) or (tick() - fallTimeout > 5)
                  
                  -- Descanso de 12 segundos + Barrido físico
                  if smartFarmActive then
                     local restStartTime = tick()
                     
                     -- Ejecutar barrido teleport a las monedas del mapa
                     collectFloatingCoinsSweep(hrp)
                     
                     while smartFarmActive and (tick() - restStartTime) < 12 do
                        task.wait(0.5)
                     end
                  end
               else
                  task.wait(0.5)
               end
            end
         end)
      end
   end,
})

-- Switch Continuo Masivo (Spam TP)
FarmTab:CreateToggle({
   Name = "🔄 Switch Continuo (Spam TP)",
   CurrentValue = false,
   Flag = "ToggleContinuous",
   Callback = function(Value)
      autoFarmActive = Value
      if autoFarmActive then
         task.spawn(function()
            while autoFarmActive do
               task.wait(0.01)
               local hrp = getHRP()
               local coinPart = getCoinPart()
               if hrp then
                  resetMomentum(hrp)
                  hrp.CFrame = coinPart and coinPart.CFrame or CFrame.new(targetPos)
               end
            end
         end)
      end
   end,
})
