...
local Window = Rayfield:CreateWindow({
   ...
   LoadingSubtitle = "CoinGiver Suite | Magnet Puro Sin Arrastre",
...
-- PALABRAS CLAVE EXCLUSIVAS Y LISTA NEGRA
local allowedKeywords = {"coingivers", "wingivers", "coins", "coin", "moneda", "reward", "giver", "10000", "1000", "1250"}
local blacklistedTargets = {"gold", "golden", "grouprewardwall", "slap", "carpet", "horns", "sign", "board", "cartel", "wall", "banner"}
...
-- ========================================================
-- NÚCLEO MAGNET PURO (DISPARO VIRTUAL - SIN MOVER OBJETOS DEL MAPA)
-- ========================================================
    ...
    for _, obj in ipairs(workspace:GetDescendants()) do
        ...
        if obj:IsA("BasePart") and not obj.Parent:FindFirstChildOfClass("Humanoid") then
            ...
            if not isBlacklisted then
                -- Check de Palabras Clave Permitidas (Rutas exactas de monedas)
                ...
                if isTarget then
                    pcall(function()
                        -- Moneda de 10.000 (Respeta ciclo exacto de 12s)
                        if name == "10000" or parentName == "10000" then
                            ...
                            -- RESTO DE MONEDAS: Disparo virtual de toque instantáneo (sin alteración de CFrame)
                            if firetouchinterest then
                                firetouchinterest(hrp, obj, 0)
                                firetouchinterest(hrp, obj, 1)
                            end
...
-- Switch Magnet Puro Sin Arrastre de Mapa
FarmTab:CreateToggle({
