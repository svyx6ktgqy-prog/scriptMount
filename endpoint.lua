...
-- Interacción física individual dirigida
...
-- Obtener la lista individual de CoinGivers (500 a 999k, excluyendo 10k)
local function getEndpointCoinGivers()
    local endpoints = {}
    ...
    if coinGiversFolder then
        for _, child in ipairs(coinGiversFolder:GetChildren()) do
            if child:IsA("BasePart") then
                ...
                if val and val >= 500 and val <= 999999 and val ~= 10000 then
...
local function isValidMagnetCoin(obj)
    ...
    -- Si es un CoinGiver numérico de 500 o superior, se ignora (lo maneja el Switch 3)
...
-- Switch 1: Smart Switch Original (Moneda de 10.000 con TP + Caída + 12s)
...
-- Switch 3: NUEVO - CoinGivers Individual Sequencer (500 a 999k / Sin TP)
FarmTab:CreateToggle({
   Name = "💎 Individual CoinGivers (500 a 999k / Sin TP)",
   CurrentValue = false,
   Flag = "ToggleHighGiversIndividual",
   Callback = function(Value)
      highGiversActive = Value
      if highGiversActive then
         task.spawn(function()
            while highGiversActive do
               local endpoints = getEndpointCoinGivers()
               
               if #endpoints > 0 then
                  -- Recorrido enfocado uno por uno (Individual)
                  for _, targetPart in ipairs(endpoints) do
                     ...
                     if hrp and targetPart and targetPart.Parent then
                        -- Interactúa directamente con la parte individual sin teletransportar al jugador
                        interactWithPart(hrp, targetPart)
                        
                        -- Tiempo de respuesta enfocado para que el servidor procese cada regalo
                        task.wait(0.15)
                     end
                  end
               else
