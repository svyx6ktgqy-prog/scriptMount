...
local Window = Rayfield:CreateWindow({
   ...
   LoadingSubtitle = "CoinGiver Suite | Módulos Independientes",
...
-- Palabras clave permitidas para recompensas
local claimKeywords = {"claim", "reclamar", "collect", "get", "take", "free", "reward", "recoger", "obtener"}

-- Lista negra para EVITAR abrir menús de Admin, Tienda o Currency
local blacklistedKeywords = {"admin", "fling", "explode", "currency", "shop", "store", "tienda", "topmenu", "buy", "comprar"}
...
-- Desactiva/activa colisiones del cuerpo para no atascarse en plataformas
...
-- ========================================================
-- NÚCLEO 1: RECOLECCIÓN INDEPENDIENTE DE MONEDAS DEL MAPA
-- ========================================================
local function collectMapCoinsClean()
    ...
    -- Desactivar colisiones para atravesar terreno sin empujar el cuerpo
    ...
    -- Restaurar posición original y reacoplar colisiones físicamente
    if hrp then
        resetMomentum(hrp)
...
-- ========================================================
-- NÚCLEO 2: RECLAMO DE GUI FILTRADO (Sin Menús Secundarios)
-- ========================================================
...
local function autoClaimAllGuiRewards()
    ...
    for _, guiObject in ipairs(player.PlayerGui:GetDescendants()) do
        ...
        if guiObject:IsA("GuiButton") then
            ...
            local grandParentName = (guiObject.Parent and guiObject.Parent.Parent) and string.lower(guiObject.Parent.Parent.Name) or ""
            
            -- Verificar Lista Negra
            local isBlacklisted = false
            for _, badKw in ipairs(blacklistedKeywords) do
                if btnName:find(badKw) or btnText:find(badKw) or parentName:find(badKw) or grandParentName:find(badKw) then
                    isBlacklisted = true
                    break
                end
            end
            ...
            if not isBlacklisted then
                ...
                if not shouldClick then
                    if grandParentName == "playrewards" or grandParentName == "coins" or parentName == "slots" then
                        shouldClick = true
                    end
                end
...
-- Switch Nuevo e Independiente: Recolectar Monedas del Mapa
FarmTab:CreateToggle({
   Name = "🪙 Recolectar Monedas del Mapa (Sin Colisión)",
   CurrentValue = false,
   Flag = "ToggleMapCoins",
   Callback = function(Value)
...
-- Switch Auto Reclamar GUI Filtrado
GuiTab:CreateToggle({
   Name = "🎯 Auto Claimer GUI (Sin Admin/Tienda)",
   CurrentValue = false,
