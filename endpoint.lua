...
-- Switch Smart: Ping-Pong Mejorado (Caída alta + TP Automático del juego + Descanso 60s)
FarmTab:CreateToggle({
   Name = "⚡ Smart Switch (Caída Alta + 60s Descanso)",
   CurrentValue = false,
   Flag = "ToggleSmart",
      ...
      if smartFarmActive then
         task.spawn(function()
            while smartFarmActive do
               ...
               if hrp then
                  -- 1. Elevar mucho más por encima del endpoint para que la caída se note claramente
                  hrp.CFrame = CFrame.new(basePos + Vector3.new(0, 35, 0))
                  
                  -- 2. Esperar a que el personaje caiga naturalmente, toque el endpoint, 
                  ...
                  -- 3. Descanso de 60 segundos en el mapa inicial
                  if smartFarmActive then
                     -- Bucle de 60 segundos comprobando cada 1 segundo.
                     -- Esto permite que si apagas el switch durante el descanso, se detenga de inmediato.
                     for i = 1, 60 do
