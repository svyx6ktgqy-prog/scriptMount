...
local function TransformToCustomSkeleton()
    ...
    -- Usamos pcall porque GetObjects es una función de exploit y puede fallar si no hay internet
    ...
    if success and objects and objects[1] then
        ...
        -- Limpiamos scripts o humanoides residuales dentro del modelo descargado para evitar bugs
        ...
        -- Soldamos cada parte del esqueleto descargado al cuerpo real del jugador
        for _, skelPart in ipairs(skeletonModel:GetDescendants()) do
            if skelPart:IsA("BasePart") then
                ...
                -- Destruimos la cabeza del esqueleto para dejar espacio a la Agony Bomb
                ...
                -- Adaptación por si tu personaje es R15 pero el esqueleto es R6
                ...
                -- Si la parte se encuentra, la soldamos
                if realPart then
    ...
    -- 4. CABEZA DE AGONY BOMB (Búsqueda en bombs.lua y Reducción de Tamaño)
    ...
    if agonyBomb then
        ...
        if bombPart then
            ...
            -- Soldamos la Agony Bomb a la cabeza real (invisible)
...
SkeletonTab:CreateButton({
   ...
   Callback = function()
        ...
        Rayfield:Notify({
            ...
            Content = "Modelo descargado e inyectado. Agony Bomb ajustada.", 
            Duration = 4
        })
   end,
})
