...
-- =========================================================
-- MÉTODOS DE INYECCIÓN QUIRÚRGICA DE RECURSOS (DELTA SAFE)
-- =========================================================
local function obtenerAssetRaw(assetId)
...
local function aplicarMorphSurgico(char)
    if not char then return end

    -- 1. Limpieza absoluta: Desnudar personaje (Ropa, Pelo, Accesorios, Mallas)
    ...
    -- 2. Cargar cuerpo base limpio (11058199848) sin añadir ropa ni accesorios
    local baseBody = obtenerAssetRaw(11058199848)
    if baseBody then
        for _, item in ipairs(baseBody:GetDescendants()) do
    ...
    -- 3. Reemplazo quirúrgico de piernas (120778770632792)
    local legsAsset = obtenerAssetRaw(120778770632792)
    if legsAsset then
        -- Remover meshes previas de piernas (R6)
        for _, v in ipairs(char:GetChildren()) do
            if v:IsA("CharacterMesh") and (v.BodyPart == Enum.BodyPart.LeftLeg or v.BodyPart == Enum.BodyPart.RightLeg) then
                v:Destroy()
            end
        end

        -- Inyectar las mallas de las nuevas piernas
        for _, item in ipairs(legsAsset:GetDescendants()) do
            if item:IsA("CharacterMesh") and (item.BodyPart == Enum.BodyPart.LeftLeg or item.BodyPart == Enum.BodyPart.RightLeg) then
                item:Clone().Parent = char
            end
        end
    end

    -- 4. Equipar pantalones de recurso (6196345139)
    local pantsAsset = obtenerAssetRaw(6196345139)
    if pantsAsset then
        if pantsAsset:IsA("Pants") then
            pantsAsset:Clone().Parent = char
        else
            local pantsObj = pantsAsset:FindFirstChildOfClass("Pants") or pantsAsset:FindFirstChild("Pants", true)
            if pantsObj then
...
-- =========================================================
-- LÓGICA PRINCIPAL (EQUIPAMIENTO Y MORPH)
-- =========================================================
aplicarOutfitYBandera = function(Value)
    ...
    if isSystemActive then
        Rayfield:Notify({Title = "Procesando", Content = "Aplicando morph quirúrgico, bandera y gorro...", Duration = 2})
        
        -- INYECCIÓN DEL MORPH DESNUDO Y PIERNAS
        aplicarMorphSurgico(char)
        ...
        -- 1. CAMISETA / GRAFICO SOBRE EL CUERPO MORPHEADO
        if textureId ~= "" then
        ...
        Rayfield:Notify({Title = "¡Éxito!", Content = "Morph quirúrgico y equipamiento cargados.", Duration = 3})
        ...
        -- Restauración de descripción por defecto
        pcall(function()
            local realDesc = game.Players:GetHumanoidDescriptionFromUserId(player.UserId)
