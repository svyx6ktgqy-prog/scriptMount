...
local function ToggleMorphCompuesto(encendido)
    ...
    if encendido then
        pcall(function()
            ...
            -- 2. CARGAR PAQUETE BASE (TORSO Y EXTREMIDADES)
                    ...
                    for _, nombrePart in ipairs(partesBrazos) do
                        ...
                        if part then
                            ...
                            if attTarget and attTorso then
                                ...
                                weld.Part1 = part
                                weld.C0 = upperTorso.CFrame:ToObjectSpace(attTorso.WorldCFrame)
                                weld.C1 = part.CFrame:ToObjectSpace(attTarget.WorldCFrame)
                                weld.Parent = part
                            end
                        end
                    end
    ...
    pcall(function()
        ...
        if isSystemActive then
            ...
            local function applyPerfectTexture(part, index, isBack)
                ...
                img.Image = textureId
                img.ScaleType = Enum.ScaleType.Crop -- [NUEVO] Evita el estiramiento de la imagen
            ...
            -- 5. ANIMACIÓN (Física orgánica de serpiente integrada)
            ...
            animConnection = RunService.Heartbeat:Connect(function(dt)
                pcall(function()
                    ...
                    for _, activeHandle in ipairs(banderasActivas) do
                        ...
                        local waveFrequency = 1.1 
                        
                        for i = 1, numSegments do
                            local motor = activeHandle:FindFirstChild("Joint_" .. i)
                            if motor then
                                -- Ondulación orgánica compuesta
                                local baseWave = math.sin((timePassed * windSpeed) - (i * waveFrequency))
                                local secondaryWave = math.cos((timePassed * (windSpeed * 0.7)) - (i * (waveFrequency * 0.6))) * 0.3
                                local angleY = baseWave + secondaryWave
                                local amplitude = 0.05 + (i * 0.07) 
                                
                                if i == 1 then
                                    motor.C0 = CFrame.new(0, 2, 0) * CFrame.Angles(swayZ, swayX + (angleY * 0.05), swayY)
                                else
