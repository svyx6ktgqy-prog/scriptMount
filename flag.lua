...
-- =========================================================
-- INYECCIÓN: EFECTOS DE FLAMA (REESCALADO MATEMÁTICO AL 5%)
-- =========================================================
local function aplicarLlamasManos(char)
    ...
    if success and assets then
        -- Soporte para R15 (Hand) y R6 (Arm)
                ...
                for _, rootAsset in ipairs(assets) do
                    ...
                    for _, desc in ipairs(allDescendants) do
                        if desc:IsA("ParticleEmitter") then
                            -- 1. Reducir el tamaño interno (NumberSequence) a escala miniatura
                            local nuevosKeypoints = {}
                            for _, kp in ipairs(desc.Size.Keypoints) do
                                local nuevoValor = kp.Value * 0.05 -- Escala del 5%
                                local nuevoEnvelope = kp.Envelope * 0.05
                                
                                -- Prevención de error por coma flotante en Roblox Engine
                            ...
                            -- 2. Reducir la velocidad y esparcimiento (para que no salga volando)
                        ...
                        elseif desc:IsA("Fire") then
                            desc.Size = math.clamp(desc.Size * 0.1, 1, 30)
                            desc.Parent = manoPart
                            
                        elseif desc:IsA("PointLight") then 
                            desc.Range = desc.Range * 0.1 
                            desc.Parent = manoPart
                            
                        elseif desc:IsA("BasePart") then
                            -- Ocultamos el bloque físico original pero lo pegamos a la mano
...
aplicarOutfitYBandera = function(Value)
    ...
    if isSystemActive then
        ...
        -- Cargar llamas miniatura en las manos
