...
local Window = Rayfield:CreateWindow({
   ...
   LoadingTitle = "Cargando Morph R15/R6...",
...
-- =========================================================
-- INYECCIÓN: MORPH COMPUESTO CORREGIDO (SOPORTE R15 + R6)
-- =========================================================
local function ToggleMorphCompuesto(encendido)
    ...
    if encendido then
        -- 0. GUARDAR PROPIEDADES ORIGINALES DE CADA PARTE
        ...
        pcall(function()
            -- 1. LIMPIEZA DE ROPA Y ACCESORIOS QUE INTERFIERAN
            for _, v in ipairs(char:GetChildren()) do
            ...
            -- Función auxiliar para inyectar paquetes R15/R6
            local function aplicarPaquete(assetId, soloPiernas)
                ...
                for _, rootObj in ipairs(objects) do
                    for _, item in ipairs(rootObj:GetDescendants()) do
                        ...
                        -- R6: Si el juego usa CharacterMesh clásico
                        ...
                        -- Detalle de cabeza / cara
                        if item.Parent and item.Parent.Name == "Head" and myHead then
                            if item:IsA("Decal") or item:IsA("SpecialMesh") or item:IsA("Mesh") then
...
Tab:CreateToggle({
   ...
   Callback = function(Value)
        ...
        if Value then
            Rayfield:Notify({Title = "Morph Aplicado", Content = "El morph R15/R6 se ha inyectado correctamente.", Duration = 3})
        else
