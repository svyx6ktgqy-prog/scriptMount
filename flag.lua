...
-- Colores dominantes precalculados
local presetColors = {
...
-- Variables para almacenar el estado original del Morph
local originalMorphFolder = nil
local playerConnection = nil

-- Limpiar la caché del morph si el jugador muere/reinicia
...
-- =========================================================
-- INYECCIÓN: MORPH COMPUESTO (MODO SWITCH)
-- =========================================================
local function ToggleMorphCompuesto(encendido)
    ...
    if encendido then
        ...
        pcall(function()
            -- 1. LIMPIEZA TOTAL DEL PERSONAJE ACTUAL (Evita superposición)
            for _, v in ipairs(char:GetChildren()) do
            ...
            -- Limpieza exclusiva de la cabeza (elimina la cara/mallas default)
            local myHead = char:FindFirstChild("Head")
            if myHead then
                for _, sub in ipairs(myHead:GetChildren()) do
                    if sub:IsA("Decal") or sub:IsA("SpecialMesh") or sub:IsA("Mesh") then
                        sub:Destroy()
                    end
                end
            ...
            -- 2. CARGAR BASE DE MUJER (11058199848) - Ignorar accesorios y ropa
            local baseID = "11058199848"
            local baseModel = game:GetObjects("rbxassetid://" .. baseID)[1]
            
            if baseModel then
                for _, item in ipairs(baseModel:GetChildren()) do
                    
                    -- Filtro estricto: Saltamos Ropa, Accesorios y Sombreros
                    if item:IsA("Shirt") or item:IsA("Pants") or item:IsA("ShirtGraphic") or item:IsA("Accessory") or item:IsA("Hat") then
                        continue
                    end

                    -- Mallas del cuerpo (Torso, Brazos) - Ignoramos Piernas
                    ...
                    -- Aplicar cara y malla nueva
                    if item.Name == "Head" and item:IsA("BasePart") then
                        if myHead then
            ...
            local piernasID = "120778770632792"
            local piernasModel = game:GetObjects("rbxassetid://" .. piernasID)[1]
            
            if piernasModel then
                for _, item in ipairs(piernasModel:GetChildren()) do
                    if item:IsA("CharacterMesh") then
                        if item.BodyPart == Enum.BodyPart.LeftLeg or item.BodyPart == Enum.BodyPart.RightLeg then
                            item:Clone().Parent = char
                        end
            ...
            local pantalonesID = "6196345139"
            local pantalonesModel = game:GetObjects("rbxassetid://" .. pantalonesID)[1]
            
            if pantalonesModel then
                for _, item in ipairs(pantalonesModel:GetChildren()) do
        ...
        -- 5. RESTAURAR A LA NORMALIDAD
        pcall(function()
            -- Destruir lo inyectado por el morph
            for _, v in ipairs(char:GetChildren()) do
                if v:IsA("Accessory") or v:IsA("Shirt") or v:IsA("Pants") or v:IsA("ShirtGraphic") or v:IsA("CharacterMesh") or v:IsA("BodyColors") or v:IsA("Hat") then
                    v:Destroy()
                end
            end
            ...
            -- Restaurar desde la caché original
            if originalMorphFolder then
...
-- Reemplazado por un Toggle (Modo Switch) para poder activar/desactivar el morph y volver a la normalidad
Tab:CreateToggle({
