                ...
                for _, rootAsset in ipairs(assets) do
                    ...
                    for _, desc in ipairs(allDescendants) do
                        ...
                        elseif desc:IsA("BasePart") then
                            -- [FIX APLICADO]: BreakJoints elimina los anclajes fantasma del asset que dislocaban las manos.
                            desc:BreakJoints()
...
local function ToggleMorphCompuesto(encendido)
    ...
    if encendido then
        pcall(function()
            ...
            -- [FIX APLICADO]: Aplicar la remera negra y el logo también en este switch
        ...
        pcall(function()
            ...
            -- [FIX APLICADO]: Limpiar aura oscura, ropa y accesorios al desactivar el switch
            for _, v in pairs(char:GetChildren()) do
                if v.Name == "EfectoFlamaManos" then v:Destroy() end
...
aplicarOutfitYBandera = function(Value)
    ...
    if isSystemActive then
        ...
        -- [FIX APLICADO]: Limpieza integral al desactivar el switch 1
        for _, v in pairs(char:GetChildren()) do 
            if v.Name == "EfectoFlamaManos" then v:Destroy() end
            if v:IsA("ShirtGraphic") or v:IsA("Shirt") or v:IsA("Pants") then v:Destroy() end
        end
