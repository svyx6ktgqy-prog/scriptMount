...
-- =========================================================
-- INYECCIÓN DE OUTFIT (FUERZA BRUTA)
-- =========================================================
local function equiparOutfitBase(char, humanoid)
    -- 1. Purgar Pelo y Sombreros
    task.spawn(function()
        pcall(function()
            for _, acc in pairs(char:GetDescendants()) do
                if acc:IsA("Accessory") then
                    local handle = acc:FindFirstChild("Handle")
                    if acc.AccessoryType == Enum.AccessoryType.Hair or 
                       acc.Name:lower():match("hair") or 
                       acc.Name:lower():match("pelo") or 
                       (handle and handle:FindFirstChild("HairAttachment")) then
                        acc:Destroy()
                    end
                end
            end
...
-- =========================================================
-- BLINDAJE EXTRA: AUTORESPAWN RECOVERY
-- =========================================================
game.Players.LocalPlayer.CharacterAdded:Connect(function(newChar)
    if isSystemActive then
        task.wait(1) -- Tiempo de gracia para cargar el avatar
        aplicarOutfitYBandera(true)
    end
end)
