...
local Window = Rayfield:CreateWindow({
   ...
   LoadingTitle = "Ensamblando Rig R15 Fix...",
...
-- =========================================================
-- INYECCIÓN: MORPH CON RECONSTRUCCIÓN RIGÍDA Y LIMPIEZA DE CARA
-- =========================================================
local function ToggleMorphCompuesto(encendido)
    ...
    if encendido then
        ...
        pcall(function()
            -- 1. LIMPIEZA ABSOLUTA DE ACCESORIOS / CARAS DINÁMICAS / DECAL VIEJOS
            for _, v in ipairs(char:GetChildren()) do
            ...
            local targetFaceTexture = nil

            if myHead then
                -- Eliminar cualquier elemento en la cabeza que cree la cara antigua
                for _, sub in ipairs(myHead:GetDescendants()) do
                    if sub:IsA("Decal") or sub:IsA("Texture") or sub:IsA("FaceControls") or sub:IsA("SurfaceGui") or sub.Name:lower():find("face") then
                        sub:Destroy()
                    end
                end
            ...
            -- 2. FUNCIÓN DE EXTRACCIÓN Y ALINEACIÓN DE PUNTOS
            local function aplicarPaquete(assetId, soloPiernas)
                ...
                for _, rootObj in ipairs(objects) do
                    -- Buscar y capturar el Decal de la cara del paquete 1
                    for _, d in ipairs(rootObj:GetDescendants()) do
                        if d:IsA("Decal") and not targetFaceTexture then
                            targetFaceTexture = d.Texture
                        end
                    end
                        ...
                        if item:IsA("MeshPart") then
                            ...
                            if (soloPiernas and esPierna) or (not soloPiernas and not esPierna) then
                                ...
                                if targetPart and targetPart:IsA("MeshPart") then
                                    -- Transferencia de mallas y tamaños
                                    targetPart.MeshId = item.MeshId
                                    targetPart.TextureID = item.TextureID
                                    targetPart.Size = item.Size
                                    
                                    -- RE-ALINEACIÓN DE MOTORES6D
                                    -- Mover articulaciones según los Attachments del nuevo paquete
            ...
            -- 3. APLICAR CARA ÚNICA LIMPIA
            if myHead and targetFaceTexture then
                local newDecal = Instance.new("Decal")
                newDecal.Name = "face"
                newDecal.Texture = targetFaceTexture
                newDecal.Face = Enum.NormalId.Front
                newDecal.Parent = myHead
            end

            -- 4. FORCE-REBUILDRIG & RE-ALIGN MOTOR6D
            task.wait(0.05)
            humanoid:BuildRigFromAttachments()

            -- Ajuste manual de Offset para evitar extremidades separadas en R15
            for _, motor in ipairs(char:GetDescendants()) do
                if motor:IsA("Motor6D") and motor.Part0 and motor.Part1 then
                    local p0Att = motor.Part0:FindFirstChild(motor.Name .. "Attachment")
                    local p1Att = motor.Part1:FindFirstChild(motor.Name .. "Attachment")
                    if p0Att and p1Att then
                        motor.C0 = p0Att.CFrame
                        motor.C1 = p1Att.CFrame
                    end
                end
            end
