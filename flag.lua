...
-- =========================================================
-- INTERFAZ RAYFIELD Y VARIABLES GLOBALES
-- =========================================================
...
local desiredOutfit = false -- NUEVO: Guarda tu decisión sobre el Outfit
local desiredMorph = false  -- NUEVO: Guarda tu decisión sobre el Morph
...
-- =========================================================
-- EVENTO REAPARICIÓN (RESPAWN) FIX
-- =========================================================
game.Players.LocalPlayer.CharacterAdded:Connect(function(char)
    ...
    -- NUEVO: Hilo que espera y reactiva automáticamente si los switches están en ON
    task.spawn(function()
        local humanoid = char:WaitForChild("Humanoid", 5)
        local head = char:WaitForChild("Head", 5)
        if not humanoid or not head then return end
        
        task.wait(0.5) -- Pausa necesaria para que Roblox acabe de cargar el personaje base
        
        if desiredMorph then
            ToggleMorphCompuesto(true)
            task.wait(0.5) -- Evita conflictos con 'isProcessing'
        end
...
-- =========================================================
-- LÓGICA PRINCIPAL (EQUIPAMIENTO Y OUTFIT)
-- =========================================================
            ...
            for _, rootObj in ipairs(morphObjects) do
                ...
                for _, item in ipairs(allItems) do
                    ...
                    elseif item:IsA("MeshPart") then
                        local targetPart = char:FindFirstChild(item.Name)
                        -- FIX: Ignoramos la cabeza (Head) para no destruir la cara original
                        if targetPart and targetPart:IsA("MeshPart") and targetPart.Name ~= "Head" then
                            pcall(function()
                                targetPart.MeshId = item.MeshId
...
-- NUEVO: Actualizar la variable deseada en los callbacks de la interfaz
Tab:CreateToggle({
