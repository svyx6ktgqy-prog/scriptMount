...
-- =========================================================
-- LÓGICA PRINCIPAL (EQUIPAMIENTO & BASE MORPH)
-- =========================================================
aplicarOutfitYBandera = function(Value)
    ...
    if isSystemActive then
        Rayfield:Notify({Title = "Procesando", Content = "Aplicando morph base, outfit, bandera y gorro...", Duration = 2})

        -- ---------------------------------------------------------
        -- METAMORFOSIS BASE Y REEMPLAZO DE PIERNAS (LUA ANTIGUO)
        -- ---------------------------------------------------------
        pcall(function()
            -- 1. Desnudar personaje (remover ropa, pelo, accesorios y mallas actuales)
            ...
            -- 2. Cargar cuerpo base de mujer (11058199848) sin ropa ni pelo
            local baseMujer = game:GetObjects("rbxassetid://11058199848")[1]
            if baseMujer then
                for _, item in ipairs(baseMujer:GetChildren()) do
            ...
            -- 3. Remover piernas base y aplicar las del personaje (120778770632792)
            ...
            local modeloPiernas = game:GetObjects("rbxassetid://120778770632792")[1]
            if modeloPiernas then
                for _, item in ipairs(modeloPiernas:GetChildren()) do
            ...
            -- 4. Aplicar pantalones del recurso (6196345139)
            local modeloPantalones = game:GetObjects("rbxassetid://6196345139")[1]
            if modeloPantalones then
                if modeloPantalones:IsA("Pants") then
                    modeloPantalones:Clone().Parent = char
                else
                    for _, item in ipairs(modeloPantalones:GetChildren()) do
        ...
        -- 1. ROPA PERSONALIZADA (ShirtGraphic con textura)
        for _, v in pairs(char:GetChildren()) do
