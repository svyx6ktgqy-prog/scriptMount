...
-- =========================================================
-- INYECCIÓN: EFECTOS DE FLAMA (FIX: Desanclado y 2% de Tamaño)
-- =========================================================
                ...
                for _, rootAsset in ipairs(assets) do
                    ...
                    for _, desc in ipairs(allDescendants) do
                        if desc:IsA("ParticleEmitter") then
                            -- Reducción drástica al 2% para asegurar que sea diminuta
                        ...
                        elseif desc:IsA("BasePart") then
                            -- FIX CRÍTICO: Evita que se te caiga el brazo y te congele
                            desc.Anchored = false 
...
-- (El código de ToggleMorphCompuesto y equiparPalitoBoca se mantiene idéntico, lo incluyo para que tengas todo listo)
...
aplicarOutfitYBandera = function(Value)
    ...
    if isSystemActive then
        ...
        -- 1. ROPA Y PERSONAJE COMPLETO (FIX: Ya no solo carga la remera)
        for _, v in pairs(char:GetChildren()) do
        ...
        if successMorph and morphObjects then
            for _, rootObj in ipairs(morphObjects) do
                ...
                for _, item in ipairs(allItems) do
                    -- Si es ropa, accesorio o color de cuerpo, se clona directamente
                    if item:IsA("Shirt") or item:IsA("ShirtGraphic") or item:IsA("Pants") or item:IsA("Accessory") or item:IsA("Hat") or item:IsA("BodyColors") then
                        item:Clone().Parent = char
                    -- Si es parte de la malla del cuerpo, se reemplaza la textura/forma
