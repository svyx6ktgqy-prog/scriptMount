...
-- =========================================================
-- VARIABLES DE CONTROL (Para poder borrar todo al apagar)
-- =========================================================
...
-- Usamos CreateToggle en lugar de CreateButton
Tab:CreateToggle({
   ...
   Callback = function(Value)
        ...
        if Value == true then
            ...
            animConnection = RunService.Heartbeat:Connect(function(dt)
                ...
                if isEquipped and flagTool.Parent == char then
                    activeHandle = handle
                elseif not isEquipped and backFlag and backFlag.Parent == char then
                    activeHandle = backFlag
                end
            ...
            Rayfield:Notify({Title = "¡Todo Listo!", Content = "Outfit equipado.", Duration = 3})
            ...
            -- 1. Detener el viento (muy importante para evitar lag)
            ...
            -- 4. Eliminar Bandera (Inventario y Mano)
            if flagTool then flagTool:Destroy() end
            if player.Backpack:FindFirstChild("Bandera Realista") then player.Backpack:FindFirstChild("Bandera Realista"):Destroy() end
            if char:FindFirstChild("Bandera Realista") then char:FindFirstChild("Bandera Realista"):Destroy() end

            -- 5. Eliminar Bandera de la Espalda
            if backFlag then backFlag:Destroy() end
            if char:FindFirstChild("BackFlagHandle") then char:FindFirstChild("BackFlagHandle"):Destroy() end
        end
   end
