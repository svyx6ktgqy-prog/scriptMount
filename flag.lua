...
-- =========================================================
-- INYECCIÓN: MORPH COMPUESTO
-- =========================================================
local function AplicarMorphCompuesto()
    local char = game:GetService("Players").LocalPlayer.Character
    local humanoid = char and char:FindFirstChildOfClass("Humanoid")
    if not char or not humanoid then return end

    pcall(function()
        -- 1. LIMPIEZA DEL PERSONAJE ACTUAL
        ...
        -- 2. CARGAR BASE DE MUJER (11058199848) - Sin pelo, sin ropa, sin piernas
        ...
        if baseModel then
            for _, item in ipairs(baseModel:GetChildren()) do
                ...
                if item:IsA("Accessory") or item:IsA("Hat") then
                    ...
                    local accHandle = acc:FindFirstChild("Handle")
                    if accHandle then
                        local att = accHandle:FindFirstChildOfClass("Attachment")
                        ...
                        if targetBodyPart then
                            if att and targetAtt then
                                accHandle.CFrame = targetBodyPart.CFrame * targetAtt.CFrame * att.CFrame:Inverse()
                            else
                                accHandle.CFrame = targetBodyPart.CFrame
                            end
...
Tab:CreateButton({
   Name = "Aplicar Morph Compuesto (Mujer + Ropa)",
   Callback = function()
        AplicarMorphCompuesto()
        Rayfield:Notify({Title = "Morph Aplicado", Content = "El morph se ha aplicado correctamente.", Duration = 3})
   end,
})
