-- =========================================================
-- CONFIGURACIÓN DE IMÁGENES Y SEGURIDAD PARA iOS (DELTA)
-- =========================================================
...
Tab:CreateDropdown({
   ...
   Callback = function(Option)
        ...
        if texturePresets[selected] then
            ...
            customAssetId = "" -- Resetear ID para forzar recarga
...
Tab:CreateButton({
   ...
   Callback = function()
        if customInputUrl ~= "" and (customInputUrl:sub(1,4) == "http") then
            ...
            customAssetId = "" -- Resetear ID para forzar descarga
...
aplicarOutfitYBandera = function(Value)
    ...
    if isSystemActive then
        Rayfield:Notify({Title = "Procesando", Content = "Aplicando outfit y física...", Duration = 2})
