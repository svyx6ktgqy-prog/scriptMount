...
-- Colores dominantes precalculados para detección automática (Últimos 3 recalibrados)
local presetColors = {
...
aplicarOutfitYBandera = function(Value)
    ...
    if isSystemActive then
        Rayfield:Notify({Title = "Procesando", Content = "Aplicando outfit base, bandera, gorro y palito...", Duration = 2})
        
        -- =========================================================
        -- INYECCIÓN: PRE-EQUIPADO DEL AVATAR DEFAULT Y EXTRACCIÓN DE PELO
        -- =========================================================
        local function equiparBaseDefault()
            local desc = humanoid:GetAppliedDescription() or Instance.new("HumanoidDescription")
            
            -- 1. Sin pelo y Cara Sonriente Default
            desc.HairAccessory = ""
            desc.Face = 0 
            
            -- 2. Ropa Default (Usando IDs reales del catálogo de Roblox)
            desc.Shirt = 144071012 -- Camiseta de guitarra con chamarra negra
            desc.Pants = 144076358 -- Pantalones vaqueros negros con zapatos blancos
            
            -- 3. Partes del cuerpo
            -- Cuerpo de Mujer (Roblox Girl Torso y Brazos)
            desc.Torso = 86500008 
            desc.LeftArm = 86500036
            desc.RightArm = 86500054
            -- Piernas de Chico (Roblox Boy)
            desc.LeftLeg = 81069695
            desc.RightLeg = 81069704

            -- Intentamos aplicar mediante la API oficial (Soporta R15/R6 perfecto si el exploit lo permite)
            local success = pcall(function()
                humanoid:ApplyDescription(desc)
            end)
            
            -- Fallback manual implacable si ApplyDescription está bloqueado
            if not success then
                -- Arrancar el pelo de raíz
                for _, v in pairs(char:GetChildren()) do
                    if v:IsA("Accessory") and (v.AccessoryType == Enum.AccessoryType.Hair or v.Name:match("Hair") or v.Name:match("Pelo")) then
                        v:Destroy()
                    end
                end
                
                -- Configurar cara feliz
                if head and head:FindFirstChildOfClass("Decal") then
                    head:FindFirstChildOfClass("Decal").Texture = "rbxasset://textures/face.png"
                end
                
                -- Imponer ropa forzada
        ...
        -- Pausa quirúrgica para garantizar que la ropa y el cuerpo carguen visualmente 
        -- ANTES de que se monte la bandera T-Shirt y los accesorios.
        task.wait(0.6)
        -- =========================================================
        ...
        -- 1. ROPA (T-Shirt/Bandera sobre la chaqueta)
        for _, v in pairs(char:GetChildren()) do
