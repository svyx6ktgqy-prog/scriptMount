...
-- =========================================================
-- INYECCIÓN QUIRÚRGICA: AVATAR DEFAULT (ROPA, CUERPO, CARA)
-- =========================================================
local function equiparAvatarDefault(char, hum)
    -- 1. Remover Pelo (y sombreros extraños si interfieren)
    for _, obj in pairs(char:GetChildren()) do
        if obj:IsA("Accessory") then
            local handle = obj:FindFirstChild("Handle")
            if handle and (handle:FindFirstChildOfClass("HairAttachment") or obj.Name:lower():match("hair") or obj.Name:lower():match("pelo")) then
                obj:Destroy()
            end
        end
    end

    -- 2. Cara Sonriente Default
    ...
    -- 3. Ropa Manual (Aseguramos las texturas correctas 100% cliente)
    for _, obj in pairs(char:GetChildren()) do
    ...
    local ropaCamiseta = Instance.new("Shirt")
    ropaCamiseta.Name = "ChaquetaGuitarra"
    ropaCamiseta.ShirtTemplate = "rbxassetid://144076358" -- Template visual exacto
    ropaCamiseta.Parent = char

    local ropaPantalones = Instance.new("Pants")
    ropaPantalones.Name = "JeansZapatosBlancos"
    ropaPantalones.PantsTemplate = "rbxassetid://129458514" -- Template visual exacto
    ropaPantalones.Parent = char

    -- 4. Modificar complexión (HumanoidDescription) para el cuerpo Mujer y piernas Chico
    pcall(function()
        local desc = hum:GetAppliedDescription()
        if not desc then desc = Instance.new("HumanoidDescription") end
        
        -- IDs del catálogo para asegurar la ropa de forma nativa
        desc.Shirt = 144071013   -- Camiseta de guitarra con chamarra negra
        desc.Pants = 129458426   -- Pantalones vaqueros negros con zapatos blancos
        desc.Face = 0            -- Default Smile
        
        -- IDs típicos de Mujer (Torso/Brazos) y Chico (Piernas)
        desc.Head = 0
        desc.Torso = 27112052
        desc.LeftArm = 27112056
        desc.RightArm = 27112029
        desc.LeftLeg = 86500008  -- Pierna Izquierda Chico
        desc.RightLeg = 86500036 -- Pierna Derecha Chico
...
local function equiparPalitoBoca(char)
    ...
    -- Limpieza previa estricta
    for _, v in pairs(char:GetChildren()) do
        if v.Name == "PalitoBoca" then v:Destroy() end
    end
    
    -- Creación del mesh puro (emulando cartón blanco)
    ...
    -- Unión a la cabeza con calibración diagonal
...
aplicarOutfitYBandera = function(Value)
    ...
    if isSystemActive then
        Rayfield:Notify({Title = "Procesando", Content = "Aplicando avatar default, bandera, gorro y palito...", Duration = 2})
        
        -- APLICAR AVATAR DEFAULT PRIMERO
        equiparAvatarDefault(char, humanoid)
        
        -- Refrescar variables en caso de que ApplyDescription reconstruyera el personaje localmente
        char = player.Character or player.CharacterAdded:Wait()
        humanoid = char:WaitForChild("Humanoid")
        head = char:WaitForChild("Head", 5)
        
        task.wait(0.2) -- Pausa breve para asegurar que la ropa se asiente
        ...
        -- 1. ROPA (T-Shirt de la Bandera, no borra la Shirt base)
        for _, v in pairs(char:GetChildren()) do
        ...
        Rayfield:Notify({Title = "¡Éxito!", Content = "Equipamiento y Avatar cargados.", Duration = 3})
        ...
        -- Destrucción unificada del gorro y el palito
        if customHatModel then customHatModel:Destroy() end
        for _, v in pairs(char:GetChildren()) do
