...
-- =========================================================
-- EXTRACCIÓN Y TRANSFERENCIA ANATÓMICA
-- =========================================================
local function loadSurgicalAsset(assetId)
...
local function aplicarModificacionesCuerpo(char)
    -- 1. Limpieza total de Ropa, Accesorios y Pelo
    ...
    -- 2. Aplicar Personaje Base + Cara (ID: 11058199848)
    local baseBody = loadSurgicalAsset(11058199848)
    if baseBody then
        -- Desnudar la base eliminando ropa que traiga integrada
        for _, child in pairs(baseBody:GetDescendants()) do
            if child:IsA("Clothing") or child:IsA("ShirtGraphic") then
                child:Destroy()
            end
        end

        -- Transferir cara al personaje del jugador
        local head = char:FindFirstChild("Head")
        if head then
            local oldFace = head:FindFirstChildOfClass("Decal")
            if oldFace then oldFace:Destroy() end
            
            local newFace = baseBody:FindFirstChild("face", true) or baseBody:FindFirstChildOfClass("Decal")
            if newFace then
                newFace:Clone().Parent = head
            end
        end

        -- Transferir anatomía base (Mallas, Escalas y Colores)
        for _, child in pairs(baseBody:GetChildren()) do
            if child:IsA("CharacterMesh") or child:IsA("SpecialMesh") then
                child:Clone().Parent = char
            elseif child:IsA("BodyColors") then
                local oldCol = char:FindFirstChildOfClass("BodyColors")
                if oldCol then oldCol:Destroy() end
                child:Clone().Parent = char
            end
        end
    end

    -- 3. Inyección Quirúrgica de Piernas (ID: 120778770632792)
    local legsAsset = loadSurgicalAsset(120778770632792)
    if legsAsset then
        -- Eliminar mallas de piernas actuales
        for _, m in pairs(char:GetChildren()) do
            if m:IsA("CharacterMesh") and (m.BodyPart == Enum.BodyPart.LeftLeg or m.BodyPart == Enum.BodyPart.RightLeg) then
                m:Destroy()
            end
        end

        -- Inyectar mallas R6
        for _, child in pairs(legsAsset:GetDescendants()) do
            if child:IsA("CharacterMesh") and (child.BodyPart == Enum.BodyPart.LeftLeg or child.BodyPart == Enum.BodyPart.RightLeg) then
                child:Clone().Parent = char
            end
        end

        -- Compatibilidad R15: Transferir SpecialMesh/WrapTarget de piernas si el avatar es R15
        local piernasR15 = {"LeftUpperLeg", "LeftLowerLeg", "LeftFoot", "RightUpperLeg", "RightLowerLeg", "RightFoot"}
        for _, limbName in ipairs(piernasR15) do
            local targetLimb = char:FindFirstChild(limbName)
            local sourceLimb = legsAsset:FindFirstChild(limbName, true)
            if targetLimb and sourceLimb then
                for _, obj in pairs(targetLimb:GetChildren()) do
                    if obj:IsA("SpecialMesh") or obj:IsA("WrapTarget") then obj:Destroy() end
                end
                for _, obj in pairs(sourceLimb:GetChildren()) do
                    if obj:IsA("SpecialMesh") or obj:IsA("WrapTarget") then
                        obj:Clone().Parent = targetLimb
                    end
                end
            end
        end
    ...
    -- 4. Inyección de Pantalones (ID: 6196345139)
    local pantsAsset = loadSurgicalAsset(6196345139)
    if pantsAsset then
        local pantsObj = pantsAsset:IsA("Pants") and pantsAsset or pantsAsset:FindFirstChildOfClass("Pants")
        if pantsObj then
...
local Window = Rayfield:CreateWindow({
   ...
   LoadingTitle = "Inyectando geometría y físicas...",
...
Tab:CreateButton({
   ...
   Callback = function()
        if customInputUrl ~= "" and (customInputUrl:sub(1,4) == "http") then
            ...
            currentPreset = nil 
            customAssetId = "" 
            Rayfield:Notify({Title = "Procesando", Content = "Descargando textura...", Duration = 2})
            if isSystemActive then aplicarOutfitYBandera(true) end
        end
   end,
})
...
aplicarOutfitYBandera = function(Value)
    ...
    if isSystemActive then
        Rayfield:Notify({Title = "Procesando", Content = "Inyectando cuerpo, piernas y cara...", Duration = 2})
        
        -- Inyección quirúrgica del cuerpo, piernas, cara y pantalones
        aplicarModificacionesCuerpo(char)
        ...
        local function crearLoseta(nombre, tamano, alturaY)
            ...
            if textureId ~= "" then
                ...
                for _, cara in ipairs(Enum.NormalId:GetEnumItems()) do
        ...
        -- Animación Orgánica Serpiente 3D
        ...
        Rayfield:Notify({Title = "¡Éxito!", Content = "Anatomía y outfit inyectados.", Duration = 3})
    else
        ...
        for _, v in pairs(char:GetChildren()) do
            if v:IsA("ShirtGraphic") or v.Name == "GorroCustomSoldado" or v.Name == "PalitoBoca" then v:Destroy() end
        end
