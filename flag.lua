...
local function guardarObjetosOriginales(char)
    -- Evita duplicados, si ya guardamos algo, mantenemos el original seguro.
...
local function restaurarObjetosOriginales(char)
    ...
    -- 1. Destruimos toda la ropa, accesorios y meshes r6 (cuerpo moldeado) actuales
    for _, v in ipairs(char:GetChildren()) do
        if v:IsA("Accessory") or v:IsA("Hat") or v:IsA("Shirt") or v:IsA("Pants") or v:IsA("ShirtGraphic") or v:IsA("CharacterMesh") or v:IsA("BodyColors") then
            v:Destroy()
        end
    ...
    -- 2. Restauramos ropa y accesorios desde el backup original
    ...
    -- 3. Restauramos las proporciones y físicas del cuerpo (MeshParts R15)
    ...
    -- 4. Vaciamos las memorias para permitir nuevos guardados perfectos
    originalItemsBackup = {}
    originalMorphCache = {}
end
...
local function ToggleMorphCompuesto(encendido)
    ...
    if encendido then
        ...
        isProcessing = false
        -- Si el outfit y bandera sigue activo, se reconecta automáticamente limpio
...
aplicarOutfitYBandera = function(Value)
    ...
    if Value then
        ...
        isProcessing = false
        -- Si el morph (cuerpo) sigue activo, se reconecta automáticamente limpio
...
Tab:CreateToggle({
   ...
   CurrentValue = false,
   Flag = "ToggleOutfit",
   Callback = function(Value) aplicarOutfitYBandera(Value) end
})
...
Tab:CreateToggle({
   ...
   CurrentValue = false,
   Flag = "ToggleMorph",
   Callback = function(Value) ToggleMorphCompuesto(Value) end
})
