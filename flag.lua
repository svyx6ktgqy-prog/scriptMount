-- =========================================================
-- CONFIGURACIÓN DE IMÁGENES, COLORES Y SEGURIDAD (DELTA iOS)
-- =========================================================
local texturePresets = {
    ["NEGRON"] = "https://raw.githubusercontent.com/svyx6ktgqy-prog/scriptMount/refs/heads/main/IMG_8066.jpeg",
    ["Spyder550"] = "https://raw.githubusercontent.com/svyx6ktgqy-prog/scriptMount/refs/heads/main/flags/IMG_8088.jpeg",
    ["Glade"] = "https://raw.githubusercontent.com/svyx6ktgqy-prog/scriptMount/refs/heads/main/flags/IMG_8089.jpeg",
    ["Darkwole"] = "https://raw.githubusercontent.com/svyx6ktgqy-prog/scriptMount/refs/heads/main/flags/IMG_8090.jpeg",
    ["Camufla"] = "https://raw.githubusercontent.com/svyx6ktgqy-prog/scriptMount/refs/heads/main/flags/IMG_8091.jpeg",
    ["CatOnge"] = "https://raw.githubusercontent.com/svyx6ktgqy-prog/scriptMount/refs/heads/main/flags/IMG_8094.jpeg",
    ["Panic-Attack"] = "https://raw.githubusercontent.com/svyx6ktgqy-prog/scriptMount/refs/heads/main/flags/IMG_8071.jpeg",
    ["3-Ladys"] = "https://raw.githubusercontent.com/svyx6ktgqy-prog/scriptMount/refs/heads/main/flags/IMG_8101.jpeg",
    ["GirlGENTAI"] = "https://raw.githubusercontent.com/svyx6ktgqy-prog/scriptMount/refs/heads/main/flags/IMG_8104.jpeg",
    ["BobJHOD"] = "https://raw.githubusercontent.com/svyx6ktgqy-prog/scriptMount/refs/heads/main/flags/IMG_8105.jpeg"
}

local presetColors = {
    ["NEGRON"] = Color3.fromRGB(25, 25, 25),          
    ["Spyder550"] = Color3.fromRGB(180, 180, 180),    
    ["Glade"] = Color3.fromRGB(34, 139, 34),          
    ["Darkwole"] = Color3.fromRGB(30, 30, 45),        
    ["Camufla"] = Color3.fromRGB(85, 107, 47),        
    ["CatOnge"] = Color3.fromRGB(255, 140, 0),        
    ["Panic-Attack"] = Color3.fromRGB(178, 34, 34),
    ["3-Ladys"] = Color3.fromRGB(255, 105, 180),      
    ["GirlGENTAI"] = Color3.fromRGB(138, 43, 226),    
    ["BobJHOD"] = Color3.fromRGB(70, 130, 180)        
}

local currentTextureUrl = texturePresets["NEGRON"]
local currentPreset = "NEGRON" 
local customInputUrl = ""
local customAssetId = ""
local cachedTextures = {} 
local customPoleColor = Color3.fromRGB(150, 150, 150) 
local fileCounter = 0

local isOutfitActive = false
local isMorphActive = false

-- =========================================================
-- SISTEMA DE DESCARGA DE IMÁGENES
-- =========================================================
local function getTextureId()
    if customAssetId ~= "" then return customAssetId end
    if cachedTextures[currentTextureUrl] then return cachedTextures[currentTextureUrl] end
    
    local getAsset = getcustomasset or getsynasset
    if not getAsset then return "" end
    
    local fetchSuccess, imageData = pcall(function() return game:HttpGet(currentTextureUrl) end)
    if not fetchSuccess or not imageData then return "" end

    fileCounter = fileCounter + 1
    local fileName = "delta_tex_" .. tostring(fileCounter) .. ".jpeg"

    local writeSuccess = pcall(function() writefile(fileName, imageData) end)
    if not writeSuccess then return "" end
    
    local assetSuccess, id = pcall(function() return getAsset(fileName) end)
    if assetSuccess and id then
        customAssetId = id
        cachedTextures[currentTextureUrl] = id 
        return customAssetId
    end
    return ""
end

-- =========================================================
-- BACKUP Y RESTAURACIÓN DEL PERSONAJE (LA "MÁQUINA DEL TIEMPO")
-- =========================================================
local originalCache = { Items = {}, Meshes = {}, HeadTexture = "", IsCached = false }

local function CaptureOriginal(char)
    if originalCache.IsCached then return end
    originalCache.IsCached = true
    originalCache.Items = {}
    originalCache.Meshes = {}

    -- Guardar ropa, pelo, y accesorios originales
    for _, v in ipairs(char:GetChildren()) do
        if v:IsA("Accessory") or v:IsA("Shirt") or v:IsA("Pants") or v:IsA("ShirtGraphic") or v:IsA("CharacterMesh") or v:IsA("Hat") or v:IsA("BodyColors") then
            table.insert(originalCache.Items, v:Clone())
        end
        if v:IsA("MeshPart") then
            originalCache.Meshes[v.Name] = { MeshId = v.MeshId, TextureID = v.TextureID, Size = v.Size, Color = v.Color, UsePartColor = v.UsePartColor }
        end
    end
    local head = char:FindFirstChild("Head")
    if head and head:IsA("MeshPart") then originalCache.HeadTexture = head.TextureID end
end

local function RestoreOriginal(char)
    -- 1. Destruir TODO lo custom que haya agregado el script
    for _, v in ipairs(char:GetChildren()) do
        if v:IsA("Accessory") or v:IsA("Shirt") or v:IsA("Pants") or v:IsA("ShirtGraphic") or v:IsA("CharacterMesh") or v:IsA("Hat") or v:IsA("BodyColors") then
            v:Destroy()
        end
        if v.Name == "GorroCustomSoldado" or v.Name == "PalitoBoca" or v.Name == "BackFlagHandle" or v.Name == "Bandera Realista" then
            v:Destroy()
        end
    end
    
    local bp = game.Players.LocalPlayer:FindFirstChild("Backpack")
    if bp and bp:FindFirstChild("Bandera Realista") then bp:FindFirstChild("Bandera Realista"):Destroy() end

    -- Limpiar llamas (Busca las partículas directamente guardadas)
    local flamaFolder = char:FindFirstChild("FlamaTracker")
    if flamaFolder then
        for _, tag in ipairs(flamaFolder:GetChildren()) do
            if tag:IsA("ObjectValue") and tag.Value then pcall(function() tag.Value:Destroy() end) end
        end
        flamaFolder:Destroy()
    end

    -- 2. Restaurar el cuerpo original
    for _, v in ipairs(char:GetChildren()) do
        if v:IsA("MeshPart") and originalCache.Meshes[v.Name] then
            pcall(function()
                v.MeshId = originalCache.Meshes[v.Name].MeshId
                v.TextureID = originalCache.Meshes[v.Name].TextureID
                v.Size = originalCache.Meshes[v.Name].Size
                if originalCache.Meshes[v.Name].Color then v.Color = originalCache.Meshes[v.Name].Color end
                if originalCache.Meshes[v.Name].UsePartColor ~= nil then v.UsePartColor = originalCache.Meshes[v.Name].UsePartColor end
            end)
        end
    end
    local head = char:FindFirstChild("Head")
    if head and head:IsA("MeshPart") then head.TextureID = originalCache.HeadTexture end

    -- 3. Devolver la ropa y pelo original
    for _, item in ipairs(originalCache.Items) do item:Clone().Parent = char end
    
    local hum = char:FindFirstChildOfClass("Humanoid")
    if hum then task.spawn(function() task.wait(0.1); pcall(function() hum:BuildRigFromAttachments() end) end) end
end

-- =========================================================
-- FIX 100% SEGURO: FLAMAS SIN PARTES NI WELDS (NO DISLOCA)
-- =========================================================
local function aplicarLlamasManos(char)
    -- Tracker para saber qué partículas borrar después
    local flamaFolder = Instance.new("Folder")
    flamaFolder.Name = "FlamaTracker"
    flamaFolder.Parent = char

    local success, assets = pcall(function() return game:GetObjects("rbxassetid://89194108265492") end)
    
    if success and assets then
        local manos = {"LeftHand", "RightHand", "Left Arm", "Right Arm"}
        for _, rootAsset in ipairs(assets) do
            local allDescendants = rootAsset:GetDescendants()
            for _, nombreMano in ipairs(manos) do
                local manoPart = char:FindFirstChild(nombreMano)
                if manoPart then
                    for _, desc in ipairs(allDescendants) do
                        -- Extraemos SOLO los efectos visuales. Nada de físicas.
                        if desc:IsA("ParticleEmitter") or desc:IsA("Fire") or desc:IsA("PointLight") then
                            local clone = desc:Clone()
                            
                            if clone:IsA("ParticleEmitter") then
                                local nuevosKeypoints = {}
                                for _, kp in ipairs(clone.Size.Keypoints) do
                                    local nuevoValor = kp.Value * 0.02 
                                    local nuevoEnvelope = kp.Envelope * 0.02
                                    if nuevoEnvelope > nuevoValor then nuevoEnvelope = nuevoValor end 
                                    table.insert(nuevosKeypoints, NumberSequenceKeypoint.new(kp.Time, nuevoValor, nuevoEnvelope))
                                end
                                clone.Size = NumberSequence.new(nuevosKeypoints)
                                clone.Speed = NumberRange.new(clone.Speed.Min * 0.1, clone.Speed.Max * 0.1)
                                clone.SpreadAngle = Vector2.new(clone.SpreadAngle.X * 0.5, clone.SpreadAngle.Y * 0.5)
                            elseif clone:IsA("Fire") then
                                clone.Size = math.clamp(clone.Size * 0.05, 1, 10)
                            elseif clone:IsA("PointLight") then
                                clone.Range = clone.Range * 0.05
                            end
                            
                            -- Se inyectan directo en tu propia mano.
                            clone.Parent = manoPart
                            
                            local tag = Instance.new("ObjectValue")
                            tag.Name = "Ref"
                            tag.Value = clone
                            tag.Parent = flamaFolder
                        end
                    end
                end
            end
        end
    end
end

-- =========================================================
-- FUNCIONES DE CONSTRUCCIÓN (Gorro, Palito, Bandera)
-- =========================================================
local function equiparPalitoBoca(char)
    local head = char:WaitForChild("Head")
    local palito = Instance.new("Part")
    palito.Name = "PalitoBoca"
    palito.Size = Vector3.new(0.05, 0.7, 0.05) 
    palito.Color = Color3.fromRGB(240, 240, 240) 
    palito.Material = Enum.Material.Wood 
    palito.Massless = true; palito.CanCollide = false
    Instance.new("CylinderMesh", palito)
    palito.Parent = char
    local weld = Instance.new("Weld")
    weld.Part0 = head; weld.Part1 = palito
    weld.C0 = CFrame.new(0.12, -0.26, -0.55) * CFrame.Angles(math.rad(-110), math.rad(25), math.rad(10))
    weld.Parent = palito
end

local function crearGorro(char, textureId)
    local head = char:WaitForChild("Head")
    local customHatModel = Instance.new("Model")
    customHatModel.Name = "GorroCustomSoldado"
    local elevacionGorro = 0.08 
    
    local function crearLoseta(nombre, tamano, alturaY)
        local bloque = Instance.new("Part")
        bloque.Name = nombre; bloque.Shape = Enum.PartType.Block; bloque.Size = tamano
        bloque.Material = Enum.Material.Fabric; bloque.Massless = true; bloque.CanCollide = false
        if textureId ~= "" then
            bloque.Color = Color3.fromRGB(255, 255, 255)
            for _, cara in ipairs({Enum.NormalId.Front, Enum.NormalId.Back, Enum.NormalId.Top, Enum.NormalId.Bottom, Enum.NormalId.Left, Enum.NormalId.Right}) do
                local texturaCara = Instance.new("Decal")
                texturaCara.Texture = textureId; texturaCara.Face = cara; texturaCara.Parent = bloque
            end
        else
            bloque.Color = Color3.fromRGB(18, 18, 18)
        end
        bloque.CFrame = head.CFrame * CFrame.new(0, alturaY + elevacionGorro, 0)
        bloque.Parent = customHatModel
        local weld = Instance.new("WeldConstraint")
        weld.Part0 = head; weld.Part1 = bloque; weld.Parent = bloque
    end

    crearLoseta("Copa1", Vector3.new(1.0, 0.25, 1.0), 0.75) 
    crearLoseta("Copa2", Vector3.new(1.1, 0.25, 1.1), 0.50)
    crearLoseta("Copa3", Vector3.new(1.2, 0.25, 1.2), 0.25)
    crearLoseta("Caida1", Vector3.new(1.5, 0.1, 1.5), 0.15)
    crearLoseta("Caida2", Vector3.new(1.8, 0.1, 1.8), 0.05)
    crearLoseta("Caida3", Vector3.new(2.1, 0.1, 2.1), -0.05)
    customHatModel.Parent = char
end

local function crearBandera(char, textureId)
    local flagPoleColor = currentPreset and presetColors[currentPreset] or customPoleColor
    local player = game.Players.LocalPlayer

    local flagTool = Instance.new("Tool")
    flagTool.Name = "Bandera Realista"
    flagTool.RequiresHandle = true
    flagTool.Grip = CFrame.new(0, -2, 0) 

    local handleFlag = Instance.new("Part")
    handleFlag.Name = "Handle"
    handleFlag.Size = Vector3.new(0.15, 6, 0.15)
    handleFlag.Color = flagPoleColor 
    handleFlag.Material = Enum.Material.Metal
    handleFlag.Parent = flagTool

    local numSegments = 6; local segWidth = 3 / numSegments
    local function applyTex(part, index, isBack)
        local surface = Instance.new("SurfaceGui")
        surface.Face = isBack and Enum.NormalId.Back or Enum.NormalId.Front
        surface.SizingMode = Enum.SurfaceGuiSizingMode.PixelsPerStud; surface.PixelsPerStud = 50
        surface.ClipsDescendants = true; surface.LightInfluence = 1; surface.Parent = part
        local img = Instance.new("ImageLabel")
        img.BackgroundTransparency = 1; img.Image = textureId; img.Size = UDim2.new(numSegments, 0, 1, 0)
        img.Position = isBack and UDim2.new(-(index - 1), 0, 0, 0) or UDim2.new(-(numSegments - index), 0, 0, 0)
        img.Parent = surface
    end

    for i = 1, numSegments do
        local seg = Instance.new("Part")
        seg.Size = Vector3.new(segWidth, 2, 0.05)
        seg.Massless = true; seg.CanCollide = false; seg.Transparency = 1 
        if textureId ~= "" then applyTex(seg, i, false); applyTex(seg, i, true) end

        local motor = Instance.new("Motor6D", handleFlag)
        motor.Name = "Joint_" .. i
        if i == 1 then
            motor.Part0 = handleFlag; motor.Part1 = seg
            motor.C0 = CFrame.new(0, 2, 0); motor.C1 = CFrame.new(-segWidth/2, 0, 0)
        else
            motor.Part0 = handleFlag:FindFirstChild("Seg_" .. (i-1)); motor.Part1 = seg
            motor.C0 = CFrame.new(segWidth/2, 0, 0); motor.C1 = CFrame.new(-segWidth/2, 0, 0)
        end
        seg.Name = "Seg_" .. i; seg.Parent = handleFlag 
    end

    local function createBackFlag()
        local backFlag = handleFlag:Clone()
        backFlag.Name = "BackFlagHandle"
        backFlag.Massless = true; backFlag.CanCollide = false
        backFlag.Parent = char
        for i = 1, numSegments do
            local joint = backFlag:FindFirstChild("Joint_" .. i)
            if joint then joint.Part0 = (i == 1) and backFlag or backFlag:FindFirstChild("Seg_" .. (i-1)) end
        end
        local torso = char:FindFirstChild("UpperTorso") or char:FindFirstChild("Torso")
        if torso then
            backFlag.CFrame = torso.CFrame * CFrame.new(0, 0, 0.65) * CFrame.Angles(math.rad(15), 0, math.rad(-45))
            local weldFlag = Instance.new("WeldConstraint")
            weldFlag.Part0 = torso; weldFlag.Part1 = backFlag; weldFlag.Parent = backFlag
        end
    end

    flagTool.Equipped:Connect(function() if char:FindFirstChild("BackFlagHandle") then char:FindFirstChild("BackFlagHandle"):Destroy() end end)
    flagTool.Unequipped:Connect(function() if isOutfitActive or isMorphActive then createBackFlag() end end)
    flagTool.Parent = player.Backpack
    createBackFlag()
end

-- =========================================================
-- EL CEREBRO CENTRAL: ADMINISTRA LOS ESTADOS SIN ROMPER NADA
-- =========================================================
local function UpdateCharacterState()
    local char = game.Players.LocalPlayer.Character
    if not char then return end

    -- Siempre capturar el estado original si es la primera vez
    CaptureOriginal(char)
    
    -- Antes de aplicar algo, restauramos al estado 100% limpio (Así no se rompe al alternar random)
    RestoreOriginal(char)

    if not isOutfitActive and not isMorphActive then return end -- Si ambos están apagados, terminamos aquí.

    local textureId = getTextureId()
    
    -- 1. LÓGICA DEL MORPH COMPUESTO (Cuerpo Femenino)
    if isMorphActive then
        local function aplicarPaquete(assetId, soloPiernas)
            local success, objects = pcall(function() return game:GetObjects("rbxassetid://" .. tostring(assetId)) end)
            if success and objects then
                for _, rootObj in ipairs(objects) do
                    for _, item in ipairs(rootObj:GetDescendants()) do
                        if item:IsA("MeshPart") then
                            local lowerName = item.Name:lower()
                            local esPierna = lowerName:find("leg") or lowerName:find("foot")
                            if (soloPiernas and esPierna) or (not soloPiernas and not esPierna) then
                                local targetPart = char:FindFirstChild(item.Name)
                                if targetPart and targetPart:IsA("MeshPart") then
                                    pcall(function()
                                        targetPart.MeshId = item.MeshId; targetPart.Size = item.Size
                                        if lowerName == "head" then targetPart.TextureID = ""
                                        elseif lowerName == "uppertorso" or lowerName == "lowertorso" then targetPart.TextureID = ""; targetPart.Color = Color3.fromRGB(20, 20, 20); targetPart.UsePartColor = true 
                                        else targetPart.TextureID = item.TextureID end
                                    end)
                                end
                            end
                        end
                        if item:IsA("CharacterMesh") then
                            local esPiernaR6 = (item.BodyPart == Enum.BodyPart.LeftLeg or item.BodyPart == Enum.BodyPart.RightLeg)
                            if (soloPiernas and esPiernaR6) or (not soloPiernas and not esPiernaR6) then item:Clone().Parent = char end
                        end
                    end
                end
            end
        end
        aplicarPaquete(11058199848, false)
        aplicarPaquete(120778770632792, true)
    end

    -- 2. ROPA Y LOGO (Aplica a ambos modos, manteniendo la estética)
    local successRopa, ropaObjects = pcall(function() return game:GetObjects("rbxassetid://11716577088") end)
    if successRopa and ropaObjects then
        for _, rootObj in ipairs(ropaObjects) do
            for _, item in ipairs(rootObj:GetDescendants()) do
                if item:IsA("Shirt") then item:Clone().Parent = char end
                if not isMorphActive and item:IsA("Pants") then item:Clone().Parent = char end
            end
        end
    end

    if isMorphActive then
        local successPants, pantalonesObjects = pcall(function() return game:GetObjects("rbxassetid://6196345139") end)
        if successPants and pantalonesObjects then
            for _, rootObj in ipairs(pantalonesObjects) do
                for _, item in ipairs(rootObj:GetDescendants()) do
                    if item:IsA("Pants") then item:Clone().Parent = char end
                end
            end
        end
    end

    if textureId ~= "" then
        local ts = Instance.new("ShirtGraphic", char); ts.Graphic = textureId
    end

    -- 3. FLAMAS OSCURAS (Aplica a ambos modos)
    aplicarLlamasManos(char)

    -- 4. ACCESORIOS DEL OUTFIT (Gorro, Palo, Bandera)
    if isOutfitActive then
        crearGorro(char, textureId)
        equiparPalitoBoca(char)
        crearBandera(char, textureId)
    end

    -- Reajuste final para asegurar que nada se rompa
    local hum = char:FindFirstChildOfClass("Humanoid")
    if hum then task.spawn(function() task.wait(0.1); pcall(function() hum:BuildRigFromAttachments() end) end) end
end

-- Evento para mantener tu backup seguro si te matan o reseteas el personaje
game.Players.LocalPlayer.CharacterAdded:Connect(function()
    originalCache.IsCached = false
    task.wait(1)
    if isOutfitActive or isMorphActive then UpdateCharacterState() end
end)

-- =========================================================
-- BUCLE DE ANIMACIÓN DE BANDERA (CORRE SIEMPRE EN SEGUNDO PLANO)
-- =========================================================
local RunService = game:GetService("RunService")
local timePassed = 0

RunService.Heartbeat:Connect(function(dt)
    timePassed = timePassed + dt
    local char = game.Players.LocalPlayer.Character
    if not char then return end
    
    local banderasActivas = {}
    local tool = char:FindFirstChild("Bandera Realista")
    if tool and tool:FindFirstChild("Handle") then table.insert(banderasActivas, tool.Handle) end
    local backFlag = char:FindFirstChild("BackFlagHandle")
    if backFlag then table.insert(banderasActivas, backFlag) end
    
    if #banderasActivas == 0 then return end
    
    local humanoid = char:FindFirstChildOfClass("Humanoid")
    local speed = humanoid and humanoid.MoveDirection.Magnitude or 0

    local swayX = math.sin(timePassed * 3) * 0.15 
    local swayY = math.sin(timePassed * 5) * 0.08 
    local swayZ = math.sin(timePassed * 2) * 0.05 
    if speed > 0 then swayX = swayX + (math.sin(timePassed * 8) * 0.15) end
    
    local windSpeed = 6 + (speed * 0.5) 
    local waveFrequency = 1.2 

    for _, activeHandle in ipairs(banderasActivas) do
        for i = 1, 6 do
            local motor = activeHandle:FindFirstChild("Joint_" .. i)
            if motor then
                local angleY = math.sin((timePassed * windSpeed) - (i * waveFrequency)) 
                local amplitude = 0.1 + (i * 0.05) 
                if i == 1 then motor.C0 = CFrame.new(0, 2, 0) * CFrame.Angles(swayZ, swayX + (angleY * 0.1), swayY)
                else motor.C0 = CFrame.new(3/6/2, 0, 0) * CFrame.Angles(0, angleY * amplitude, 0) end
            end
        end
    end
end)

-- =========================================================
-- INTERFAZ RAYFIELD
-- =========================================================
local Rayfield = loadstring(game:HttpGet('https://sirius.menu/rayfield'))()
local Window = Rayfield:CreateWindow({
   Name = "Bandera y Outfit Definitivo",
   LoadingTitle = "Procesando físicas seguras...",
   LoadingSubtitle = "Delta iOS",
   ConfigurationSaving = { Enabled = false }
})

local Tab = Window:CreateTab("Equipamiento", 4483362458) 

Tab:CreateDropdown({
   Name = "Texturas Predeterminadas",
   Options = {"NEGRON", "Spyder550", "Glade", "Darkwole", "Camufla", "CatOnge", "Panic-Attack", "3-Ladys", "GirlGENTAI", "BobJHOD"},
   CurrentOption = {"NEGRON"},
   MultipleOptions = false,
   Flag = "DropdownTexturas",
   Callback = function(Option)
        local selected = type(Option) == "table" and Option[1] or Option
        if texturePresets[selected] then
            currentTextureUrl = texturePresets[selected]
            currentPreset = selected
            customAssetId = "" 
            UpdateCharacterState()
        end
   end,
})

Tab:CreateInput({
   Name = "URL Custom (Raw Link)",
   PlaceholderText = "Pegar link raw de imagen aquí...",
   RemoveTextAfterFocusLost = false,
   Callback = function(Text) customInputUrl = Text end,
})

Tab:CreateButton({
   Name = "Cargar URL Custom",
   Callback = function()
        if customInputUrl ~= "" and (customInputUrl:sub(1,4) == "http") then
            currentTextureUrl = customInputUrl
            currentPreset = nil 
            customAssetId = "" 
            UpdateCharacterState()
        end
   end,
})

Tab:CreateColorPicker({
    Name = "Color del Palo (Solo para Custom URL)",
    Color = Color3.fromRGB(150, 150, 150),
    Flag = "ColorPaloPicker",
    Callback = function(Value)
        customPoleColor = Value
        if isOutfitActive and not currentPreset then UpdateCharacterState() end
    end
})

Tab:CreateToggle({
   Name = "Activar Outfit y Bandera",
   CurrentValue = false,
   Flag = "ToggleOutfit",
   Callback = function(Value) 
       isOutfitActive = Value 
       UpdateCharacterState() 
   end
})

Tab:CreateToggle({
   Name = "Activar Morph Compuesto (Mujer)",
   CurrentValue = false,
   Flag = "ToggleMorph",
   Callback = function(Value) 
       isMorphActive = Value 
       UpdateCharacterState() 
   end
})
