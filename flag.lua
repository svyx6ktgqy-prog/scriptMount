...
Tab:CreateButton({
   ...
   Callback = function()
        ...
        -- Función para aplicar la imagen perfectamente cortada (CON EL FIX FRONTAL)
        local function applyPerfectTexture(part, index, isBack)
            ...
            -- FIX NATURAL APLICADO AQUÍ
            if isBack then
                -- La cara de atrás mantiene tu método original que ya funcionaba
                img.Position = UDim2.new(-(index - 1), 0, 0, 0)
            else
                -- La cara de adelante invierte la posición matemática para que coincida
        ...
        Rayfield:Notify({
            Title = "¡Éxito!",
            Content = "Ambas caras unidas perfectamente.",
            Duration = 5
        })
   end
})
