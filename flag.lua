...
-- =========================================================
-- LÓGICA DE INYECCIÓN ADAPTADA DEL LUA PROFESIONAL
-- =========================================================
local function InyectarAnatomiaYOutfit(char)
    if not char then return end

    pcall(function()
        -- 1. LIMPIEZA ABSOLUTA DE ROPA, PELO Y ACCESORIOS (DESNUDAR)
        for _, v in ipairs(char:GetDescendants()) do
            if v:IsA("Accessory") or v:IsA("Clothing") or v:IsA("ShirtGraphic") or v:IsA("Hat") or v:IsA("CharacterMesh") then
                v:Destroy()
            end
        end

        -- 2. CARGAR CUERPO BASE Y CARA (ID: 11058199848)
        local baseModel = game:GetObjects("rbxassetid://11058199848")[1]
        if baseModel then
            for _, item in ipairs(baseModel:GetChildren()) do
                -- Inyectar mallas y colores del cuerpo base
                if item:IsA("CharacterMesh") or item:IsA("BodyColors") then
                    item:Clone().Parent = char
                -- Inyectar la cara directamente inspeccionando la cabeza del modelo descargado
                        ...
                        for _, sub in ipairs(item:GetChildren()) do
                            if sub:IsA("Decal") then
                                for _, mySub in ipairs(myHead:GetChildren()) do
                                    if mySub:IsA("Decal") then mySub:Destroy() end
                                end
        ...
        -- 3. REMPLAZAR PIERNAS (ID: 120778770632792)
        local legsModel = game:GetObjects("rbxassetid://120778770632792")[1]
        if legsModel then
            for _, item in ipairs(legsModel:GetChildren()) do
                -- Extraer únicamente CharacterMesh asignados a piernas (LeftLeg / RightLeg)
        ...
        -- 4. INYECTAR PANTALONES (ID: 6196345139)
        local pantsModel = game:GetObjects("rbxassetid://6196345139")[1]
        if pantsModel then
            local pantsObj = pantsModel:IsA("Pants") and pantsModel or pantsModel:FindFirstChildOfClass("Pants")
            if pantsObj then
...
local Window = Rayfield:CreateWindow({
   ...
   LoadingTitle = "Cargando lógica profesional...",
...
aplicarOutfitYBandera = function(Value)
    ...
    if isSystemActive then
        Rayfield:Notify({Title = "Procesando", Content = "Inyectando anatomía quirúrgica...", Duration = 2})
        
        -- Ejecución de la lógica extraída del script profesional
        InyectarAnatomiaYOutfit(char)
        ...
        Rayfield:Notify({Title = "¡Éxito!", Content = "Anatomía e inyección completadas.", Duration = 3})
    else
