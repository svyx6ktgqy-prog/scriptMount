...
-- =========================================================
-- INTERFAZ RAYFIELD Y ENVOLTORIO SILENCIOSO
-- =========================================================
...
-- Envoltorio totalmente silencioso que ignora cualquier tipo de error visual
local function safeCallback(callbackFunc)
    return function(...)
        local args = {...}
        pcall(function()
...
Tab:CreateDropdown({
   ...
   Callback = safeCallback(function(Option)
        ...
        if selected and texturePresets[selected] then
...
Tab:CreateInput({
   ...
   Callback = safeCallback(function(Text)
        if type(Text) == "string" then customInputUrl = Text end
   end),
})
