...
-- =========================================================
-- INYECCIÓN QUIRÚRGICA: OUTFIT BASE (ROBUSTO)
-- =========================================================
local function equiparOutfitBase(char, humanoid)
    -- 1. Método Oficial (HumanoidDescription API)
    local success = pcall(function()
        local desc = humanoid:GetAppliedDescription()
        if not desc then desc = Instance.new("HumanoidDescription") end
        
        desc.HairAccessories = "" -- Remueve el cabello
        
        -- IDs del Catálogo
        desc.Shirt = 382538295 -- Camiseta de guitarra con chamarra negra
        desc.Pants = 398635338 -- Pantalones vaqueros negros con zapatos blancos
        desc.Face = 0 -- Cara default (Smile)
        
        -- Configuración Física (Torso/Brazos Mujer, Piernas Chico)
        desc.Torso = 27112056
        desc.LeftArm = 27112052
        desc.RightArm = 27112025
        desc.LeftLeg = 86500185
        desc.RightLeg = 86499905
        
        humanoid:ApplyDescription(desc)
    end)

    -- 2. Fallback Directo (Por si el Executor bloquea ApplyDescription local)
    -- Destruir cabello manualmente
    for _, acc in ipairs(humanoid:GetAccessories()) do
        if acc.AccessoryType == Enum.AccessoryType.Hair or acc.Name:lower():match("hair") then
            acc:Destroy()
        end
    end

    -- Inyección estricta de ropa 2D usando Templates IDs (IDs de textura cruda)
    local shirt = char:FindFirstChildOfClass("Shirt")
    if not shirt then
        shirt = Instance.new("Shirt", char)
    end
    shirt.ShirtTemplate = "rbxassetid://382538284" 

    local pants = char:FindFirstChildOfClass("Pants")
    if not pants then
        pants = Instance.new("Pants", char)
    end
    pants.PantsTemplate = "rbxassetid://398633812"

    -- Forzar cara sonriente (Decal Override)
...
-- =========================================================
-- LÓGICA PRINCIPAL (EQUIPAMIENTO COMPLETO)
-- =========================================================
aplicarOutfitYBandera = function(Value)
    ...
    if isSystemActive then
        -- 1. APLICAR EL OUTFIT BASE PRIMERO (Antes de accesorios custom)
        Rayfield:Notify({Title = "Procesando", Content = "Configurando cuerpo, rostro y ropa base...", Duration = 1.5})
        equiparOutfitBase(char, humanoid)
        
        -- Pausa técnica robusta (Evita que el recálculo de físicas rompa los Welds del gorro/bandera)
        task.wait(0.15)
        
        Rayfield:Notify({Title = "Procesando", Content = "Aplicando gorro, palito y bandera...", Duration = 2})
        ...
        -- 2. ROPA EXTRA (Estampado T-Shirt de la Bandera)
        for _, v in pairs(char:GetChildren()) do
        ...
        -- 3. GORRO CUBETA CON DECAL Y ELEVACIÓN
        ...
        -- 6. ANIMACIÓN (Respiración + Serpiente 3D)
