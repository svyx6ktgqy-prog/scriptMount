...
-- ==========================================
-- SISTEMA DE INTERACCIÓN FORZADA (NUEVO)
-- ==========================================
-- Simula apretar teclas físicas para juegos de RP que bloquean el fireproximityprompt normal
local function ForceInteract(promptObj)
    if promptObj and promptObj:IsA("ProximityPrompt") then
        promptObj.HoldDuration = 0
        pcall(function() fireproximityprompt(promptObj) end)
    end
    
    -- Simular teclas E, F, G (Típicas en Hood Argentino)
    local keys = {Enum.KeyCode.E, Enum.KeyCode.F, Enum.KeyCode.G}
...
-- ==========================================
-- TOGGLE 1: AUTO REPARTOSYA (Reparado)
-- ==========================================
    ...
    task.spawn(function()
        while getgenv().a do
            ...
            pcall(function()
                ...
                -- FASE 2: Teletransporte y simulación de tecla
                for _, obj in ipairs(Workspace:GetDescendants()) do
                    if obj:IsA("ProximityPrompt") and obj.Enabled then
                        ...
                        if string.match(parentName, "pedido") or string.match(parentName, "delivery") or string.match(parentName, "restaurant") or string.match(actionText, "recoger") or string.match(actionText, "entregar") or string.match(actionText, "agarrar") then
                            ...
                            -- Nos teletransportamos directamente encima del objeto
                            hrp.CFrame = obj.Parent.CFrame * CFrame.new(0, 1, 0)
                            task.wait(0.3)
                            
                            -- Forzamos la interacción (Simulación de Teclas)
                            ForceInteract(obj)
...
-- ==========================================
-- TOGGLE 2: AUTO AGARRAR DINERO/OBJETOS (Reparado)
-- ==========================================
            ...
            pcall(function()
                ...
                for _, obj in ipairs(Workspace:GetDescendants()) do
                    if obj:IsA("ProximityPrompt") and obj.Enabled then
                        ...
                        if string.match(parentName, "cash") or string.match(parentName, "dinero") or string.match(parentName, "plata") or string.match(parentName, "drop") or string.match(actionText, "claim") or string.match(actionText, "agarrar") or string.match(actionText, "recoger") then
                            
                            local oldPos = hrp.CFrame
                            -- Teletransporte justo encima
                            hrp.CFrame = obj.Parent.CFrame * CFrame.new(0, 1, 0)
                            task.wait(0.2)
                            
                            -- Forzamos interacción con teclas simuladas G/F/E
                            ForceInteract(obj)
...
Credits.Text = "ID: Gattoi | VirtualInput Integrado"
