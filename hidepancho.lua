...
-- ==========================================
-- ACTUALIZACIÓN: Toggle 1 Adaptado a 'RepartosYaApp' (Y: 60)
-- ==========================================
CreateToggle("Auto RepartosYa (Celular)", 60, function(Value)
    ...
    if Value then
        ...
        -- Si existe el punto de trabajo físico, vamos a buscarlo
        if p then
        ...
        task.spawn(function()
            ...
            while getgenv().a do
                ...
                pcall(function()
                    ...
                    -- 1. Buscamos la nueva App basada en tus logs
                    local repartosApp = nil
                    for _, desc in ipairs(playerGui:GetDescendants()) do
                        if desc.Name == "RepartosYaApp" and desc:IsA("Frame") then
                            repartosApp = desc
                            break
                        end
                    end
                    
                    -- Lógica Nueva (RepartosYaApp)
                    if repartosApp then
                        local ordersFrame = repartosApp:FindFirstChild("OrdersFrame", true)
                        if ordersFrame then
                            -- Buscamos cualquier frame de orden (Ej: Order_3690 o OrderFrame)
                            for _, frame in ipairs(ordersFrame:GetChildren()) do
                                if string.find(frame.Name, "Order_") or frame.Name == "OrderFrame" then
                                    -- Disparamos cualquier botón de aceptar que esté dentro de esa orden
                                    local btn = frame:FindFirstChild("acceptButton", true) or frame:FindFirstChildWhichIsA("TextButton", true)
                    ...
                    -- Lógica Vieja de Fallback (pedidosYaGUI) - Por si algún servidor viejo la sigue usando
                    local oldGui = playerGui:FindFirstChild("pedidosYaGUI")
                    if oldGui then
                        local rLoc, dLoc = oldGui:FindFirstChild("restaurantLocation"), oldGui:FindFirstChild("deliveryLocation")
                        local acceptBtn = oldGui:FindFirstChild("screenFrame") and oldGui.screenFrame:FindFirstChild("acceptButton")
...
CreateToggle("Auto-Agarrar Dinero", 115, function(Value) getgenv().autoGrab = Value end)
...
-- Toggle 4: Rastreador de GUI e Inventario (Y: 225)
