-- Load the Rayfield library from the provided URL[span_4](start_span)[span_4](end_span)[span_5](start_span)[span_5](end_span)
local Rayfield = loadstring(game:HttpGet('https://raw.githubusercontent.com/svyx6ktgqy-prog/rayfield/refs/heads/main/source.lua'))()

-- Create the main window[span_6](start_span)[span_6](end_span)[span_7](start_span)[span_7](end_span)
...
-- Roblox Services[span_8](start_span)[span_8](end_span)[span_9](start_span)[span_9](end_span)
...
-- State variables[span_10](start_span)[span_10](end_span)[span_11](start_span)[span_11](end_span)
...
-- History logs[span_12](start_span)[span_12](end_span)[span_13](start_span)[span_13](end_span)
local ActionLogs = {}
local ShopLogs = {}
local InvLogs = {} 
local MaxLogs = 50

-- Floating "CLEAR" Button Setup[span_14](start_span)[span_14](end_span)
local ClearGui = nil
...
local function CreateClearButton()
    ...
    -- Click event logic to safely destroy the highlighted target[span_15](start_span)[span_15](end_span)
...
-- Create the Highlight to see what we are touching[span_16](start_span)[span_16](end_span)[span_17](start_span)[span_17](end_span)
...
-- ==========================================
-- MENU TABS[span_18](start_span)[span_18](end_span)[span_19](start_span)[span_19](end_span)
-- ==========================================
...
local ShopTab = Window:CreateTab("Shops & NPCs ðª", 4483362458)
local InvTab = Window:CreateTab("Inventory & GUI ð", 4483362458) 
local NPCTab = Window:CreateTab("NPC Cloner ð¥", 4483362458)

-- ==========================================
-- STRICT NPC FILTERING AND EXTRACTION[span_20](start_span)[span_20](end_span)
-- ==========================================
...
local function AnalyzeNPCDetails(root)
    ...
    for _, child in ipairs(root:GetChildren()) do
        if child:IsA("BasePart") then
            table.insert(partsList, string.format("  â¢ %s [Material: %s]", child.Name, child.Material.Name))
        elseif child:IsA("Accessory") then
            table.insert(accessoriesList, string.format("  â¢ %s (Accessory)", child.Name))
        elseif child:IsA("Shirt") or child:IsA("Pants") or child:IsA("ShirtGraphic") then
            table.insert(clothesList, string.format("  â¢ %s (%s)", child.Name, child.ClassName))
        elseif child:IsA("ProximityPrompt") or child:IsA("ClickDetector") or child:IsA("Dialog") then
            table.insert(interactiveList, string.format("  â¢ %s (%s)", child.Name, child.ClassName))
        end
    end
    
    return string.format(
        "ð¤ NPC NAME: %s\nð Root Path: %s\n\nâï¸ HUMANOID DATA:\n  - DisplayName: %s\n  - Current Health: %s / %s\n  - WalkSpeed: %s\n\nð§± BODY PARTS:\n%s\n\nð CLOTHING & APPEARANCE:\n%s\n\nð ACCESSORIES:\n%s\n\nâ¡ INTERACTIVITY:\n%s",
...
local function GenerateFullNPCDump(root)
    ...
    local function recurse(parent, depth)
        ...
        for _, child in ipairs(parent:GetChildren()) do
            table.insert(dump, string.format("%sâ¢ %s (%s)", indent, child.Name, child.ClassName))
...
-- ==========================================
-- NPC CLONER INTERFACE[span_21](start_span)[span_21](end_span)
-- ==========================================
...
NPCTab:CreateButton({
   Name = "ð Copy Data Report to Clipboard",
   Callback = function()
...
-- ==========================================
-- SECTION 1: MAIN INSPECTOR (DOM)[span_22](start_span)[span_22](end_span)
-- ==========================================
...
-- ==========================================
-- SECTION 2: DEVELOPER ACTIONS[span_23](start_span)[span_23](end_span)
-- ==========================================
...
-- ==========================================
-- SECTION 3: NETWORK MONITOR (General)[span_24](start_span)[span_24](end_span)
-- ==========================================
...
-- ==========================================
-- SECTION 4: SHOPS AND NPCS[span_25](start_span)[span_25](end_span)
-- ==========================================
...
InvTab:CreateButton({
   ...
   Callback = function()
        ...
        local clipboardText = "ð === PLAYER INVENTORY AND GUI LOG === ð\n\n"
...
-- ==========================================
-- GENERAL NETWORK HOOKING (__namecall)[span_26](start_span)[span_26](end_span)
-- ==========================================
...
-- Physical Interaction Events[span_27](start_span)[span_27](end_span)
