...
local Window = Rayfield:CreateWindow({
   Name = "ALB8RAAQ | Inspect 🔎 NINJA 2.0",
...
local InvSpyEnabled = false -- New variable for the inventory GUI

-- History logs
local ActionLogs = {}
local ShopLogs = {}
local InvLogs = {} -- New log
local MaxLogs = 50
...
local InvTab = Window:CreateTab("Inventory & GUI 🎒", 4483362458) -- NEW TAB
...
local InfoParagraph = MainTab:CreateParagraph({Title = "Element Data", Content = "Click on an object to analyze it."})
...
-- ==========================================
-- SECTION 5: INVENTORY AND GUI (NEW)
-- ==========================================
InvTab:CreateToggle({
...
oldNamecall = hookmetamethod(game, "__namecall", function(self, ...)
    ...
    if not checkcaller() and (method == "FireServer" or method == "InvokeServer") then
        ...
        -- 3. Inventory/GUI Filter (NEW)
        if InvSpyEnabled then
            local strData = string.lower(path .. formattedArgs)
            -- Keywords related to selectors, backpacks, equipment, and tools
