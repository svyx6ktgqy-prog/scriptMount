-- =====================================================================
-- 💎 MINE DIAMOND MASTER - PRO EDITION V2
-- UI: Rayfield | Físicas por BodyGyro | Auto-Equipado de Herramientas
-- =====================================================================
...
local Window = Rayfield:CreateWindow({
    Name = "💎 Mine Diamond Master | Pro V2",
    LoadingTitle = "Cargando Herramientas...",
    LoadingSubtitle = "Físicas y Movimiento Corregidos",
...
-- =====================================================================
-- PESTAÑA 4: 🚀 MOVIMIENTO Y #TROLL (ACTUALIZADA Y CORREGIDA)
-- =====================================================================
...
-- 🎭 APARTADO TROLL (REDISEÑO FÍSICO COMPLETO)
...
MoveTab:CreateToggle({
    ...
    Callback = function(Value)
        ...
        if Value then
            ...
            -- Teletransporte instantáneo inicial
            pcall(function()
            ...
            Rayfield:Notify({Title = "Modo Sombra", Content = "Físicas reales enlazadas con: " .. selectedTrollPlayer, Duration = 3})
            
            task.spawn(function()
                while trollFollowEnabled do
                    task.wait(0.01) -- Bucle ultra rápido para evitar retrasos de desincronización
                    if selectedTrollPlayer and selectedTrollPlayer ~= "Ninguno" then
                        ...
                        if targetPlayer and targetPlayer.Character and LocalPlayer.Character then
                            ...
                            if targetRoot and targetHum and localRoot and localHum then
                                ...
                                -- 🌟 CORRECCIÓN DE PIES Y MOVIMIENTO: Uso de BodyGyro nativo de físicas
                                local bg = localRoot:FindFirstChild("TrollGyro")
                                if not bg then
                                    bg = Instance.new("BodyGyro")
                                    bg.Name = "TrollGyro"
                                    bg.maxTorque = Vector3.new(0, 999999, 0) -- Rotación forzada horizontal en el eje Y
                                    bg.P = 50000 -- Fuerza instantánea
                                    bg.D = 500
                                    bg.Parent = localRoot
                                end
                                -- Mantiene el cuerpo rotado mirando exactamente hacia donde mira la víctima
                                bg.CFrame = CFrame.lookAt(Vector3.zero, targetRoot.CFrame.LookVector)

                                -- Cálculo de posición simulando joystick virtual hacia su espalda
                                local behindPosition = (targetRoot.CFrame * CFrame.new(0, 0, 3)).Position
                                local distanceToShadow = (localRoot.Position - behindPosition).Magnitude
                                localHum.WalkSpeed = targetHum.WalkSpeed -- Sincroniza velocidad de carrera
                                
                                if distanceToShadow > 1.2 then
                                    -- ¡AVANZA DE VERDAD! Envía la dirección de joystick real a tope y activa animaciones de pies
                                    local moveDirection = (behindPosition - localRoot.Position).Unit
                                    localHum:Move(moveDirection, false)
                                else
                                    -- Freno total del joystick virtual cuando está acoplado perfectamente
                                    localHum:Move(Vector3.new(0, 0, 0), false)
                                end
                                
                                -- Sincronización nativa de saltos
                                ...
                                -- 🌟 CORRECCIÓN DEL GOLPE DE PICO + AUTO-EQUIPADO DESDE MOCHILA
                                local isAttacking = false
                                pcall(function()
                                    for _, anim in ipairs(targetHum:GetPlayingAnimationTracks()) do
                                        if anim.IsPlaying then
                                            local animName = anim.Name:lower()
                                            -- Si la animación activa no es de movimiento base, es un toque de pantalla/golpe
                                            if not animName:find("idle") and not animName:find("walk") 
                                               and not animName:find("run") and not animName:find("jump") 
                                               and not animName:find("fall") and not animName:find("strafe") 
                                               and not animName:find("sit") and not animName:find("climb") then
                                                isAttacking = true 
                                                break
                                            end
                                        end
                                ...
                                if isAttacking then
                                    ...
                                    if not localTool then
                                        -- Si el pico está guardado abajo en el inventario, lo saca automáticamente
                                    ...
                                    -- Ejecuta el golpe nativo del pico de forma sincronizada
                                    if localTool then 
                                        localTool:Activate() 
                                    end
                                end
                                
                                -- Absorción magnética automática de gemas y minerales
                                pcall(function()
            ...
            -- Limpieza física al apagar el script para recuperar control del personaje
            local root = getRoot()
            if root and root:FindFirstChild("TrollGyro") then
                root.TrollGyro:Destroy()
            end
            local hum = getHumanoid()
            if hum then hum:Move(Vector3.new(0,0,0), false) end
            Rayfield:Notify({Title = "Modo Sombra", Content = "Desactivado. Control restaurado.", Duration = 2})
        end
    end,
})
