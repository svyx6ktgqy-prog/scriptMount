...
-- =====================================================================
task.spawn(function()
    task.wait(2) -- Esperar a que la interfaz cargue en el CoreGui
    for _, gui in pairs(game:GetService("CoreGui"):GetDescendants()) do
        -- Busca el frame principal del menú Rayfield
        if gui:IsA("Frame") and gui.Name == "Main" and gui.Parent and gui.Parent.Name == "Rayfield" then
            ...
            -- URL/ID de paisaje de Roblox (puedes cambiar el ID numérico por otro rbxassetid si prefieres)
            wallpaper.Image = "rbxassetid://1378768007" 
            wallpaper.ImageTransparency = 0.65 -- Ajustado para no opacar el texto del menú
    ...
    Callback = function()
        ...
        for _, item in ipairs(workspace:GetDescendants()) do
            if item:IsA("Part") or item:IsA("MeshPart") then
                ...
                -- Busca variantes de minerales grandes o caros
                if string.match(name, "huge") or string.match(name, "big") or string.match(name, "giant") or string.match(name, "master") then
                    ...
                    -- Simula el toque físico como si le dieras con el pico
...
-- Bucle rápido (RenderStepped) para Animación Rainbow y Tracers
RunService.RenderStepped:Connect(function()
    -- Generador de color Rainbow animado basado en el tiempo
    local hue = tick() % 3 / 3
        ...
        if player ~= LocalPlayer and player.Character and player.Character:FindFirstChild("HumanoidRootPart") then
            ...
            if espEnabled then
                ...
                if onScreen then
                    ...
                    -- Dibuja desde abajo-centro hasta el jugador
            ...
            -- Limpieza si el jugador muere o sale
            if Tracers[player] then
                Tracers[player]:Remove()
                Tracers[player] = nil
            end
...
VisualsTab:CreateButton({
    ...
    Callback = function()
        -- 1. Intentar disparar remotos para obtener skins
        ...
        -- 2. Ocultar candados en la interfaz de la tienda (Visual)
