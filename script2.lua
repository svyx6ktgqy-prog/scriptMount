...
-- =====================================================================
-- 🎭 APARTADO: #Troll (Versión Definitiva - Sincronización Total)
-- =====================================================================
-- NOTA: Cambia 'MoveTab' por la variable de la pestaña donde quieras colocarlo.
local TrollSection = MoveTab:CreateSection("#Troll") 

local selectedTrollPlayer = "Ninguno"
local trollFollowEnabled = false

-- Dropdown para seleccionar a la víctima
...
-- Toggle para activar/desactivar la persecución sigilosa y copia de acciones
    ...
    Callback = function(Value)
        ...
        if Value then
            if selectedTrollPlayer == "Ninguno" or selectedTrollPlayer == "" then
                Rayfield:Notify({Title = "Aviso #Troll", Content = "Selecciona un jugador válido primero.", Duration = 3})
                return
            end
            
            Rayfield:Notify({Title = "Modo Troll Activado", Content = "Sincronización total con: " .. selectedTrollPlayer, Duration = 3})
            
            -- Bucle principal de imitación física y recolección
            task.spawn(function()
                while trollFollowEnabled do
                    task.wait(0.02) -- Frecuencia fluida (casi 60 ejecuciones por segundo)
                    ...
                    if selectedTrollPlayer and selectedTrollPlayer ~= "Ninguno" then
                        ...
                        if targetPlayer and targetPlayer.Character and LocalPlayer.Character then
                            ...
                            if targetRoot and targetHum and localRoot and localHum then
                                ...
                                -- 1. MOVIMIENTO Y FRENADO NATURAL
                                ...
                                if targetHum.MoveDirection.Magnitude == 0 then
                                    -- 🔥 FRENAR AL INSTANTE: Copia el estado quieto del jugador sin retrasos
                                    localHum:Move(Vector3.new(0,0,0))
                                else
                                    if distanceToShadow > 1.2 then
                                        localHum:MoveTo(behindPosition)
                                    end
                                end
                                
                                -- Siempre mirar hacia donde mira la víctima para actuar en sincronía
                                localRoot.CFrame = CFrame.lookAt(localRoot.Position, localRoot.Position + targetRoot.CFrame.LookVector)
                                
                                -- 2. REPLICAR SALTO EXACTO
                                ...
                                -- 3. REPLICAR GOLPES DE PICO / ARMA
                                local isAttacking = false
                                pcall(function()
                                    for _, anim in ipairs(targetHum:GetPlayingAnimationTracks()) do
                                        local animName = anim.Name:lower()
                                        if animName:find("attack") or animName:find("swing") or animName:find("punch") or animName:find("slash") or animName:find("hit") or animName:find("tool") or animName:find("mine") or animName:find("pico") then
                                            isAttacking = true
                                            break
                                        end
                                    end
                                ...
                                -- 4. 🧲 ROBO / RECOLECCIÓN AUTOMÁTICA DE ITEMS AMBIENTALES
                                -- Escanea de forma masiva y ligera el entorno del objetivo (Radio de 8 studs)
                                pcall(function()
                                    if firetouchinterest then
                                        local regionParts = workspace:GetPartBoundsInRadius(targetRoot.Position, 8)
                                        for _, part in ipairs(regionParts) do
                                            -- Verifica si es un mineral tirado, moneda, gema o tiene un TouchInterest nativo
                                            ...
                                            if part:FindFirstChild("TouchInterest") or nameLower:find("gem") or nameLower:find("diamond") or nameLower:find("coin") or nameLower:find("ore") or nameLower:find("item") then
                                                -- Simula de forma invisible que tus pies están tocando ese item
                                                firetouchinterest(localRoot, part, 0) -- Toca
                                                task.wait()
                                                firetouchinterest(localRoot, part, 1) -- Suelta
                                            end
                                        end
                                    end
                                end)
            ...
            Rayfield:Notify({Title = "Modo Troll Desactivado", Content = "Sincronización apagada.", Duration = 2})
        end
    end,
})

-- Función interna para mantener la lista de jugadores actualizada
