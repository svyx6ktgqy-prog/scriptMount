...
-- ==========================================
-- SWITCH MÁSCARA BALACLAVA (AHORA CON FIX DE JOYTICK Y WELD MANUAL)
-- ==========================================
                ...
                if maskModel then
                    ...
                    if accessory then
                        ...
                        if head then
                            ...
                            -- SOLDADURA MANUAL (WELD) PARA FORZAR QUE APAREZCA EN LA CABEZA
                            local handle = accClone:FindFirstChild("Handle")
                            if handle then
                                local att = handle:FindFirstChildOfClass("Attachment")
                                local targetAtt = att and head:FindFirstChild(att.Name)
                                
                                -- Posicionarlo correctamente con o sin attachment
                                ...
                                -- Alternativa si el accesorio no tiene un "Handle" estándar
...
-- + BALACLAVA MASK & RAINBOW RADIO
-- + FIX ANTI-CONGELAMIENTO DE JOYSTICK
-- + VISUALIZADOR AUDIO COLA RADIACTIVA (NUEVO)
-- ==========================================
...
-- ==========================================
-- 6. NEW TAB: BOOMBOX ITEM E INTEGRACIÓN VISUALIZADOR
-- ==========================================
...
local currentRadioSound = nil -- Variable global para el sonido que leerá el visualizador
   ...
   Callback = function(Value)
       ...
       if Value then
           local success, errorMessage = pcall(function()
               ...
               currentRadioSound = radioSound -- <-- VINCLAMOS PARA EL VISUALIZADOR DE AUDIO
...
-- ==========================================
-- INTEGRACIÓN: VISUALIZADOR DE AUDIO (COLA RADIACTIVA) 
-- Funciona leyendo la Boombox equipada
-- ==========================================
local trailCubes = {}
local spinePoints = {}
local numCubes = 35 
...
local function updateTrailVisualizer()
    ...
    -- Obtenemos el volumen de nuestra Radio
    local snd = currentRadioSound
    local loudness = (snd and snd.IsPlaying) and snd.PlaybackLoudness or 0
...
BoomboxTab:CreateSection("Efectos Visuales de Audio (Cola)")
...
BoomboxTab:CreateToggle({
   Name = "Activar Cola Radiactiva (Al piso)",
   CurrentValue = false,
   ...
   Callback = function(Value)
       ...
       if Value then
           createTrailCubes()
           if not renderConnectionVis then
               renderConnectionVis = RunService.Heartbeat:Connect(updateTrailVisualizer)
           end
       else
           if renderConnectionVis then
               renderConnectionVis:Disconnect()
               renderConnectionVis = nil
           end
           cleanupTrail()
       end
   end,
...
BoomboxTab:CreateSection("Otros Efectos Boombox")
