...
-- + COLA RADIACTIVA (VISUALIZADOR FÍSICO ADAPTADO AL HANDLE)
-- ==========================================
...
-- ==========================================
-- LÓGICA DE COLA RADIACTIVA (INTEGRADA A LA RADIO)
-- ==========================================
local trailCubes = {}
local spinePoints = {} 
local numCubes = 35 
...
local boomboxClonedTool = nil -- Referencia global a la radio
...
local function createTrailCubes()
    ...
    -- Intentar iniciar desde la radio si existe
    if boomboxClonedTool and boomboxClonedTool:FindFirstChild("Handle") then
        startCFrame = boomboxClonedTool.Handle.CFrame
    elseif LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then
        startCFrame = LocalPlayer.Character.HumanoidRootPart.CFrame
    end
...
local function updateTrailVisualizer()
    ...
    -- Conectar lógicamente al Handle y al Audio de la Radio Equipada
    if boomboxClonedTool and boomboxClonedTool:FindFirstChild("Handle") then
        targetPart = boomboxClonedTool.Handle
        activeSound = targetPart:FindFirstChild("CustomMusicPlayer")
    else
        local char = LocalPlayer.Character
    ...
    local loudness = (activeSound and activeSound.IsPlaying) and activeSound.PlaybackLoudness or 0
    ...
    for i = 1, numCubes do
        ...
        if i == 1 then
            -- El trail persigue al Handle de la radio hacia el suelo
            targetSpineCFrame = targetPart.CFrame * CFrame.new(0, -1.5, 1.2)
...
-- ==========================================
-- 6. TAB: BOOMBOX ITEM + COLA RADIACTIVA
-- ==========================================
...
BoomboxTab:CreateSection("Efectos Visuales (Cola Radiactiva)")

BoomboxTab:CreateSlider({
   Name = "Sensibilidad de Graves (Bass Trail)",
   Range = {2, 30},
   Increment = 1,
   CurrentValue = 8,
   Flag = "BassSlider",
...
BoomboxTab:CreateToggle({
   Name = "Activar Cola Radiactiva al ritmo de la Radio",
   CurrentValue = false,
