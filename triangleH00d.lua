...
local stealthRunTrack = nil
local stealthIdleTrack = nil
...
local StealthToggle = StealthTab:CreateToggle({
    ...
    Callback = function(Value)
        ...
        if Value then
            -- 1. CAMBIAR APARIENCIA COMPLETA Y SOLDADURA
            if char and humanoid then
                ...
                -- 2. CONFIGURAR ANIMADOR PERSONALIZADO
                originalAnimateScript = char:FindFirstChild("Animate")
                if originalAnimateScript then
                    originalAnimateScript.Disabled = true -- Apagar sistema base de Roblox
                end
                ...
                -- Cargar Animación Base para correr (Id universal de correr en R15/R6)
                local runAnim = Instance.new("Animation")
                runAnim.AnimationId = (humanoid.RigType == Enum.HumanoidRigType.R15) and "rbxassetid://507767714" or "rbxassetid://180426354"
                stealthRunTrack = (animator or humanoid):LoadAnimation(runAnim)
                
                -- Cargar Animación de estar Quieto (La del Ninja si está instalada en el Animate script)
                local idleAnim = Instance.new("Animation")
                idleAnim.AnimationId = (humanoid.RigType == Enum.HumanoidRigType.R15) and "rbxassetid://507766388" or "rbxassetid://180435571"
                if originalAnimateScript and originalAnimateScript:FindFirstChild("idle") then
                    local customIdle = originalAnimateScript.idle:FindFirstChildOfClass("Animation")
                    if customIdle then idleAnim.AnimationId = customIdle.AnimationId end
                end
                stealthIdleTrack = (animator or humanoid):LoadAnimation(idleAnim)
            end
            
            -- 3. FLY + NOCLIP CON ORIENTACIÓN SUAVE
            flyConn = RunService.RenderStepped:Connect(function()
                ...
                if hrp and hum and hum.Health > 0 then
                    ...
                    if moveDir.Magnitude > 0 then
                        ...
                        local newPos = hrp.Position + (moveDir * 0.35)
                        
                        -- CFrame con rotación hacia donde apuntas (Previene giros a lo loco)
                        local flatDir = Vector3.new(moveDir.X, 0, moveDir.Z)
                        if flatDir.Magnitude > 0.001 then
                            hrp.CFrame = CFrame.lookAt(newPos, newPos + flatDir)
                        else
                            hrp.CFrame = CFrame.new(newPos) * hrp.CFrame.Rotation
                        end
                        
                        -- Controles de Animación: Si te mueves, corre
                        if stealthIdleTrack and stealthIdleTrack.IsPlaying then stealthIdleTrack:Stop() end
                        if stealthRunTrack and not stealthRunTrack.IsPlaying then stealthRunTrack:Play() end
                    else
                        -- Controles de Animación: Si estás quieto, pose ninja
                        if stealthRunTrack and stealthRunTrack.IsPlaying then stealthRunTrack:Stop() end
                        if stealthIdleTrack and not stealthIdleTrack.IsPlaying then stealthIdleTrack:Play() end
                    end
                end
            end)
            
            -- 4. ACTIVAR SISTEMA VISUAL ESP ROJO SANGRE
            ...
            -- 5. GUI DE ALERTA SANGRE LATENTE
            ...
            if stealthRunTrack then stealthRunTrack:Stop() stealthRunTrack:Destroy() stealthRunTrack = nil end
            if stealthIdleTrack then stealthIdleTrack:Stop() stealthIdleTrack:Destroy() stealthIdleTrack = nil end
            if originalAnimateScript then 
