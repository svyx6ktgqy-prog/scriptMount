...
local StealthToggle = StealthTab:CreateToggle({
    ...
    Callback = function(Value)
        ...
        if Value then
            -- 1. CAMBIAR APARIENCIA COMPLETA (Mantiene animaciones/cámara originales y arregla la máscara)
            if char and humanoid then
                pcall(function()
                    -- Limpiamos la ropa y accesorios actuales
                    ...
                    if model then
                        for _, item in ipairs(model:GetChildren()) do
                            ...
                            -- IGNORAMOS animaciones o scripts del nuevo personaje para usar los tuyos por defecto
                            if item:IsA("Script") or item:IsA("LocalScript") or item:IsA("Animation") or item:IsA("Animator") then
                                continue
                            end
                            ...
                            if item:IsA("Accessory") or item:IsA("Hat") then
                                ...
                                if handle and head then
                                    -- Vital para no afectar tu cámara o movimiento original
                                    ...
                                    -- Usamos un Weld clásico (C0/C1) para encaje milimétrico de la máscara
                                    ...
                                    if att and headAtt then
                                        ...
                                        -- Si no tiene attachments, lo centra
