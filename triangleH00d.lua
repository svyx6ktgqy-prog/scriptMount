...
-- ==========================================
-- SWITCH MÁSCARA BALACLAVA (AHORA CON FIX DE JOYTICK)
-- ==========================================
StealthTab:CreateToggle({
    ...
    Callback = function(Value)
        ...
        if Value then
            pcall(function()
                ...
                if maskModel then
                    ...
                    if accessory then
                        ...
                        -- FIX ANTI-CONGELAMIENTO PARA LA MÁSCARA
                        ...
                        -- Ocultar la cara original para el efecto de reemplazo
            ...
            Rayfield:Notify({Title = "Balaclava", Content = "Máscara equipada sin afectar movimiento.", Duration = 3})
        else
            local mask = char:FindFirstChild("BalaclavaMask")
            if mask then mask:Destroy() end
            
            -- Restaurar la cara original
            local head = char:FindFirstChild("Head")
            if head then
                for _, v in ipairs(head:GetChildren()) do
                    ...
                    if model then
                        for _, item in ipairs(model:GetChildren()) do
                            if item:IsA("Accessory") or item:IsA("Hat") then
                                ...
                                -- FIX ANTI-CONGELAMIENTO DE JOYSTICK PARA EL MODELO ENTERO
...
-- ==========================================
-- SWITCH RADIO RAINBOW + PARTÍCULAS
-- ==========================================
local rainbowRadioLoop = nil
local radioEmitter = nil
local originalRadioColor = nil
local originalRadioMaterial = nil

BoomboxTab:CreateToggle({
    Name = "Efecto Radio RAINBOW + Partículas",
    CurrentValue = false,
    Flag = "RainbowRadioToggle",
    Callback = function(Value)
        if not boomboxClonedTool or not boomboxClonedTool:FindFirstChild("Handle") then
            Rayfield:Notify({Title = "Aviso", Content = "Primero equipa la radio Alienware para aplicar el efecto.", Duration = 4})
            return
        end
        
        local handle = boomboxClonedTool.Handle
        
        if Value then
            originalRadioColor = handle.Color
            originalRadioMaterial = handle.Material
            
            -- Crear emisor de partículas
            radioEmitter = Instance.new("ParticleEmitter")
            radioEmitter.Name = "RainbowAura"
            radioEmitter.Texture = "rbxassetid://243660364" -- Textura estándar de destellos de Roblox
            radioEmitter.Rate = 50
            radioEmitter.Speed = NumberRange.new(1.5, 3)
            radioEmitter.Lifetime = NumberRange.new(0.5, 1)
            radioEmitter.SpreadAngle = Vector2.new(360, 360)
            radioEmitter.Size = NumberSequence.new({NumberSequenceKeypoint.new(0, 0.4), NumberSequenceKeypoint.new(1, 0)})
            radioEmitter.Parent = handle
            
            -- Bucle de colores RGB
            local hue = 0
            rainbowRadioLoop = RunService.RenderStepped:Connect(function(dt)
                if boomboxClonedTool and handle and handle.Parent then
                    hue = (hue + dt * 0.4) % 1
                    local currentRGB = Color3.fromHSV(hue, 1, 1)
                    handle.Color = currentRGB
                    handle.Material = Enum.Material.Neon
                    if radioEmitter then
                        radioEmitter.Color = ColorSequence.new(currentRGB)
                    end
                else
                    if rainbowRadioLoop then rainbowRadioLoop:Disconnect() end
                end
            end)
            Rayfield:Notify({Title = "Rainbow Activado", Content = "Efecto visual aplicado a la radio.", Duration = 3})
        else
            -- Limpiar efecto
            ...
            if radioEmitter then
                radioEmitter:Destroy()
                radioEmitter = nil
            end
            if handle and originalRadioColor then
                handle.Color = originalRadioColor
                handle.Material = originalRadioMaterial or Enum.Material.Plastic
            end
            Rayfield:Notify({Title = "Rainbow Desactivado", Content = "Apariencia de radio restaurada.", Duration = 3})
        end
    end,
})
