...
local StealthToggle = StealthTab:CreateToggle({
    ...
    Callback = function(Value)
        ...
        if Value then
            -- 1. CAMBIAR APARIENCIA COMPLETA (Arreglo de Casco y Máscara)
                    ...
                    if model then
                        for _, item in ipairs(model:GetChildren()) do
                            if item:IsA("Accessory") then
                                -- AddAccessory fuerza a Roblox a ensamblar el casco/máscara correctamente en la cabeza
                            ...
                            elseif item.Name == "Head" and item:IsA("BasePart") then
                                -- Copiar cara (decal) si el modelo trae una personalizada
            ...
            -- 2. FLY + NOCLIP CAUTELOSO + ANTI-SUELO
            flyConn = RunService.RenderStepped:Connect(function()
                ...
                if hrp and hum and hum.Health > 0 then
                    -- Noclip: Desactivar colisiones (para atravesar paredes)
                    ...
                    -- SISTEMA ANTI-SUELO (Raycast)
                    local rayParams = RaycastParams.new()
                    rayParams.FilterDescendantsInstances = {currentChar, espFolder}
                    rayParams.FilterType = Enum.RaycastFilterType.Exclude
                    
                    -- Lanza un rayo 3.5 studs hacia abajo (apenas por debajo de los pies)
                    local floorResult = workspace:Raycast(hrp.Position, Vector3.new(0, -3.5, 0), rayParams)
                    
                    if floorResult then
                        -- Si detectamos suelo y la cámara/movimiento apunta hacia abajo, anulamos el eje Y
                        if moveDir.Y < 0 then
                            moveDir = Vector3.new(moveDir.X, 0, moveDir.Z)
                        end
                        
                        -- Evita que la gravedad te hunda corrigiendo la posición mínima
                        local minHeight = floorResult.Position.Y + 3.0
                        if hrp.Position.Y < minHeight then
                            hrp.CFrame = CFrame.new(hrp.Position.X, minHeight, hrp.Position.Z) * hrp.CFrame.Rotation
                        end
                    end
