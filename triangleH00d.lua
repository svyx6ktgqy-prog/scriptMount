...
local texturePresets = {
    ...
    ["Drak"] = { -- NUEVA BANDERA ANIMADA DRAK
        "https://raw.githubusercontent.com/svyx6ktgqy-prog/scriptMount/refs/heads/main/flags/roblox1.jpeg",
        "https://raw.githubusercontent.com/svyx6ktgqy-prog/scriptMount/refs/heads/main/flags/roblox2.jpeg",
        "https://raw.githubusercontent.com/svyx6ktgqy-prog/scriptMount/refs/heads/main/flags/roblox3.jpeg",
        "https://raw.githubusercontent.com/svyx6ktgqy-prog/scriptMount/refs/heads/main/flags/roblox4.jpeg",
        "https://raw.githubusercontent.com/svyx6ktgqy-prog/scriptMount/refs/heads/main/flags/roblox5.jpeg",
        "https://raw.githubusercontent.com/svyx6ktgqy-prog/scriptMount/refs/heads/main/flags/roblox6.jpeg"
    }
}
                ...
                for _, rootAsset in ipairs(assets) do
                    ...
                    for _, desc in ipairs(allDescendants) do
                        if desc:IsA("ParticleEmitter") then
                            ...
                            desc.Size = NumberSequence.new(nuevosKeypoints)
                            desc.Speed = NumberRange.new(desc.Speed.Min * 0.1, desc.Speed.Max * 0.1)
                            desc.SpreadAngle = Vector2.new(desc.SpreadAngle.X * 0.5, desc.SpreadAngle.Y * 0.5)
                            desc.Parent = manoPart
                            
                        elseif desc:IsA("Fire") then
                            desc.Size = math.clamp(desc.Size * 0.05, 1, 10)
                            desc.Parent = manoPart
                            
                        elseif desc:IsA("PointLight") then 
                            desc.Range = desc.Range * 0.05 
                            desc.Parent = manoPart
                        ...
                        elseif desc:IsA("BasePart") then
                            ...
                            desc.Massless = true
                            desc.Transparency = 1 
                            desc.Size = Vector3.new(0.05, 0.05, 0.05)
...
local NombresPersonajes = {
    "Tr$xsh", "Alien", "Max", "Madrid", "Catting", "slen", "nekoWhite", "supreme", "purpose", "kukumer", "☪️", "NPcEm0", "NPcEm1", "satan", "entityD", "joker", "BruuhMus", "IndigestGreen", "stilect", "halloV", "hacklord", "admin", "b3hemoop", "monsterV9", "shouldermane", "legomane", "necromancer", "girlGotic", "miniGirl", "4ktre", "gucciCat", "batman", "evil6", "%1", "freefire", "santa", "pim", "Everlasting", "TrashGang", "Goat", "Monster", "Reaper", "OnlyJdottt888", "SpyEffect", "DemonSuite",
...
local boomboxPlaylist = {
    "139393062477955", "139371029481481", "102864915432195", "1847733588", "9042281328", "7215629038596", "135992805356761",
...
local desiredOutfit = false 
local desiredMorph = false  
local originalMorphCache = {}
local originalItemsBackup = {}
local aplicarOutfitYBandera
local ToggleMorphCompuesto

-- =========================================================
-- SISTEMA UNIFICADO DE RESPALDO Y RESTAURACIÓN
-- =========================================================
local function guardarObjetosOriginales(char)
    if #originalItemsBackup > 0 then return end
    ...
    for _, v in ipairs(char:GetChildren()) do
        if v:IsA("Accessory") or v:IsA("Hat") or v:IsA("Shirt") or v:IsA("Pants") or v:IsA("ShirtGraphic") or v:IsA("CharacterMesh") or v:IsA("BodyColors") then
            table.insert(originalItemsBackup, v:Clone())
        end
        if v:IsA("MeshPart") then
            local _, props = pcall(function() return { Part = v, MeshId = v.MeshId, TextureID = v.TextureID, Size = v.Size, Color = v.Color, UsePartColor = v.UsePartColor } end)
    ...
    if head then
        for _, sub in ipairs(head:GetChildren()) do
            if sub:IsA("Decal") or sub:IsA("Texture") or sub:IsA("FaceControls") or sub:IsA("SurfaceAppearance") or sub:IsA("SpecialMesh") then
                ...
                clone.Name = "RESTORE_HEAD_" .. sub.Name
                table.insert(originalItemsBackup, clone)
            end
        end
    end
end

local function restaurarObjetosOriginales(char)
    if #originalItemsBackup == 0 then return end
    ...
    for _, item in ipairs(originalItemsBackup) do
        if item.Name:sub(1, 13) == "RESTORE_HEAD_" then
            if head then
                local realItem = item:Clone()
                realItem.Name = item.Name:sub(14)
                realItem.Parent = head
            end
        else
            item:Clone().Parent = char
        end
...
local function limpiarEfectosYAccesorios(char)
    if animConnection then animConnection:Disconnect(); animConnection = nil end
    for _, v in pairs(char:GetChildren()) do
        if v.Name == "EfectoFlamaManos" or v.Name == "GorroCustomSoldado" or v.Name == "PalitoBoca" or v.Name == "BackFlagHandle" or v.Name == "Bandera Realista" then
            v:Destroy()
        end
    end
...
-- =========================================================
-- EVENTO REAPARICIÓN (RESPAWN CORREGIDO)
-- =========================================================
game.Players.LocalPlayer.CharacterAdded:Connect(function(char)
    ...
    task.spawn(function()
        ...
        if desiredOutfit then
            aplicarOutfitYBandera(true)
            task.wait(0.3)
        end
        
        if desiredMorph then
            ToggleMorphCompuesto(true)
        end
    end)
end)

-- =========================================================
-- MORPH COMPUESTO (SWITCH 2)
-- =========================================================
ToggleMorphCompuesto = function(encendido)
    ...
    if encendido then
        isProcessing = true
        isMorphActive = true
        guardarObjetosOriginales(char)
        
        pcall(function()
            for _, v in ipairs(char:GetChildren()) do
                if v:IsA("Accessory") or v:IsA("Hat") or v:IsA("Shirt") or v:IsA("Pants") or v:IsA("ShirtGraphic") or v:IsA("CharacterMesh") then v:Destroy() end
            end

            local myHead = char:FindFirstChild("Head")
            if myHead then
                for _, sub in ipairs(myHead:GetDescendants()) do
                    if sub:IsA("Decal") or sub:IsA("Texture") or sub:IsA("FaceControls") or sub:IsA("SurfaceAppearance") or sub:IsA("SpecialMesh") then sub:Destroy() end
                end
            ...
            local function aplicarPaquete(assetId, soloPiernas)
                ...
                for _, rootObj in ipairs(objects) do
                    for _, item in ipairs(rootObj:GetDescendants()) do
                        if item:IsA("Shirt") or item:IsA("Pants") or item:IsA("ShirtGraphic") or item:IsA("Accessory") or item:IsA("Hat") or item:IsA("Decal") or item:IsA("SurfaceAppearance") or item:IsA("FaceControls") then continue end
            ...
            if successRopa and ropaObjects then
                for _, rootObj in ipairs(ropaObjects) do
                    local allItems = rootObj:GetDescendants()
                    table.insert(allItems, rootObj)
            ...
            if textureId ~= "" then
                local ts = Instance.new("ShirtGraphic", char)
                ts.Graphic = textureId
            end
            ...
            task.spawn(function() task.wait(0.1); pcall(function() humanoid:BuildRigFromAttachments() end) end)
        end)
        isProcessing = false
    else
        isProcessing = true
        ...
        limpiarEfectosYAccesorios(char)
        restaurarObjetosOriginales(char)
...
local function equiparPalitoBoca(char)
    ...
    for _, v in pairs(char:GetChildren()) do if v.Name == "PalitoBoca" then v:Destroy() end end
...
-- =========================================================
-- LÓGICA PRINCIPAL (SWITCH 1: EQUIPAMIENTO Y OUTFIT)
-- =========================================================
aplicarOutfitYBandera = function(Value)
    ...
    if Value then
        ...
        -- BORRADO DE CARA PARA EL PRIMER SWITCH (Misma lógica del segundo)
        if head then
            for _, sub in ipairs(head:GetChildren()) do
                if sub:IsA("Decal") or sub:IsA("Texture") or sub:IsA("FaceControls") or sub:IsA("SurfaceAppearance") or sub:IsA("SpecialMesh") then
                    sub:Destroy()
        ...
        for _, v in pairs(char:GetChildren()) do
            if v:IsA("ShirtGraphic") or v:IsA("Shirt") or v:IsA("Pants") or v:IsA("Accessory") then v:Destroy() end
        end
        
        local successMorph, morphObjects = pcall(function()
            return game:GetObjects("rbxassetid://11716577088")
        end)
        
        if successMorph and morphObjects then
            for _, rootObj in ipairs(morphObjects) do
                local allItems = rootObj:GetDescendants()
                table.insert(allItems, rootObj)
                
                for _, item in ipairs(allItems) do
                    if item:IsA("Shirt") or item:IsA("ShirtGraphic") or item:IsA("Pants") or item:IsA("Accessory") or item:IsA("Hat") or item:IsA("BodyColors") then
                        item:Clone().Parent = char
        ...
        local function applyPerfectTexture(part, index, isBack)
            ...
            if isBack then img.Position = UDim2.new(-(index - 1), 0, 0, 0)
            else img.Position = UDim2.new(-(numSegments - index), 0, 0, 0) end
            img.Parent = surface
        end
        ...
        if wasEquipped then humanoid:EquipTool(flagTool)
        else createBackFlag() end
            ...
            for _, activeHandle in ipairs(banderasActivas) do
                ...
                for i = 1, numSegments do
                    ...
                    if motor then
                        ...
                        else motor.C0 = CFrame.new(segWidth/2, 0, 0) * CFrame.Angles(0, angleY * amplitude, 0) end
                    end
                end
            end
        end)
