-- ==========================================
-- Mini chit Hood Argentino V8 (RAYFIELD EDITION) - FIXED
-- ==========================================
...
local StealthToggle = StealthTab:CreateToggle({
    ...
    Callback = function(Value)
        ...
        if Value then
                        ...
                        for _, item in ipairs(model:GetChildren()) do
                            if item:IsA("Accessory") or item:IsA("Hat") then
                                ...
                                if handle and head then
                                    ...
                                    -- SOLUCIÓN: Calcular el CFrame PRIMERO y luego aplicar el WeldConstraint
            ...
            -- 2. FLY SEGURO (Mantiene colisiones para no caer bajo mapa y evita BAN)
            if hrp and humanoid then
                ...
                flyConn = RunService.RenderStepped:Connect(function()
                    ...
                    if currentHrp and hum and hum.Health > 0 then
                        ...
                        -- Girar la vista con la camara (solo eje Y para evitar temblar y bugs de camara)
                        bg.cframe = CFrame.new(currentHrp.Position, currentHrp.Position + camera.CFrame.LookVector * Vector3.new(1, 0, 1))
                        
                        if moveDir.Magnitude > 0 then
                            -- Velocidad segura para que el anti-cheat no detecte aceleraciones instantáneas
                            bv.velocity = moveDir.Unit * 30 
                        else
                            bv.velocity = Vector3.new(0, 0, 0)
                        end
                    end
            ...
            alertLabel.Text = "¡Modo Seguro Activado!"
            ...
            if hrp then
                local bg = hrp:FindFirstChild("StealthGyro")
                local bv = hrp:FindFirstChild("StealthVelocity")
