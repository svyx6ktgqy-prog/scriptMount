...
local StealthToggle = StealthTab:CreateToggle({
    ...
    Callback = function(Value)
        ...
        if Value then
            ...
            -- 2. FLY + NOCLIP CAUTELOSO PURO (Sin Suelo + Control Total de Cámara)
            flyConn = RunService.RenderStepped:Connect(function()
                ...
                if hrp and hum and hum.Health > 0 then
                    ...
                    -- Forzamos a que el frente del personaje se alinee con la cámara
                    local lookDirection = camera.CFrame.LookVector
                    
                    if moveDir.Magnitude > 0 then
                        -- Calcula la nueva posición
                        local newPos = hrp.Position + (moveDir.Unit * 0.35)
                        
                        -- CFrame.lookAt(posición_actual, punto_al_que_mirar)
                        hrp.CFrame = CFrame.lookAt(newPos, newPos + lookDirection)
                    else
                        -- Aunque el jugador no presione teclas, mantenemos la rotación fluida con la cámara
