...
local Window = Rayfield:CreateWindow({
   ...
   LoadingSubtitle = "Sistema Multiclón y Animación",
...
local function FindNPC(name)
    if name == "ProClimber" then
        -- Buscar primero en la ruta detallada del reporte
    ...
    elseif name == "Seller" then
        -- Buscar primero con el nombre de "Seller", fallback a "SellWorker"
            ...
            if npcStateFolder then
                for _, myAnim in ipairs(stateFolder:GetChildren()) do
                    if myAnim:IsA("Animation") then
                        ...
                        -- Si tiene la animación original, la mapeamos; si no, la esquiva automáticamente
            ...
            pcall(function()
                ...
                for _, track in pairs(playingMyTracks) do
                    if track.Animation and track.Animation.AnimationId then
                        ...
                        -- Usa la animación del NPC si existe, si no "esquiva" usando la del jugador
...
-- =========================================================
-- 🖱️ MENÚ DE BOTONES DE INTERFAZ (RAYFIELD)
-- =========================================================

MorphTab:CreateButton({
   Name = "Clonar Pro Climber 🧗‍♂️",
   Callback = function()
        StopCloning() -- Detener el clon activo antes de cambiar
