...
local function StartCloning(TargetNPC)
    ...
    -- =========================================================
    -- 🛠️ GUARDAR POSTURA ORIGINAL (Evita que se deforme el brazo al desequipar)
    -- =========================================================
    local DummyOriginalJoints = {}
    for _, obj in pairs(Dummy:GetDescendants()) do
        if obj:IsA("Motor6D") then
            -- Guardamos los valores base de las articulaciones del clon intacto
    ...
    if MyHum and DummyHum then
        AnimConnection = RunService.Stepped:Connect(function()
            ...
            pcall(function()
                ...
                -- =========================================================
                -- 🛠️ FIX DE RETARDO DE ANIMACIÓN (Sincronización 100% exacta)
                -- =========================================================
                for _, track in pairs(playingMyTracks) do
                    if track.Animation and track.Animation.AnimationId then
                        ...
                        if dummyTrack then
                            ...
                            -- Forzamos a la animación del clon a igualar tiempo, peso y velocidad del jugador
                            pcall(function()
                                dummyTrack:AdjustWeight(track.Weight)
                                dummyTrack:AdjustSpeed(track.Speed)
                                
                                -- Si hay desincronización visible (más de 0.05 seg), igualamos la línea de tiempo.
                ...
                -- =========================================================
                -- 🛠️ FIX DE DEFORMACIÓN DEL BRAZO (Solo aplicar si hay pico)
                -- =========================================================
                local MiChar = LocalPlayer.Character
                if not MiChar then return end
                
                -- Comprobamos si tienes una herramienta en la mano
                        ...
                        if MiJoint and ClonJoint then
                            pcall(function()
                                if TieneHerramienta then
                                    -- Si está equipado, forzamos su mano para agarrar la herramienta
                                    ...
                                    -- Si NO está equipado, restauramos las articulaciones originales para que no se deforme
                                    ...
                                    -- No copiamos Transform aquí; dejamos que la animación sincronizada lo mueva naturalmente.
                                end
                            end)
                        end
                    end
