...
local function StartCloning(TargetNPC)
    ...
    -- HACER INVISIBLE AL JUGADOR (Mantiene herramientas visibles)
    ...
    -- =========================================================
    -- 🛠️ SINCRONIZACIÓN VISUAL Y DE POSICIÓN (RenderStepped)
    -- Aquí forzamos el brazo para capturar los fotogramas en movimiento
    -- =========================================================
    Connection = RunService.RenderStepped:Connect(function()
        if not IsActive or not MyRoot or not DummyRoot then return end
        
        -- Sincronizar posición exacta del cuerpo
        DummyRoot.CFrame = MyRoot.CFrame
        
        -- CALCAR EL BRAZO EXACTO CON EL PICO (Movimiento natural)
        ...
        local function CalcarArticulacionVisual(NombreParte, NombreArticulacion)
                ...
                if MiJoint and ClonJoint then
                    pcall(function()
                        if TieneHerramienta then
                            -- Copiamos la rotación (Transform) después de que el motor de Roblox la animara,
                            -- esto captura el balanceo del brazo corriendo con la herramienta.
                            ...
                            -- Si se desequipa, soltamos el brazo para que la animación normal actúe
        ...
        if MiChar:FindFirstChild("Torso") and Dummy:FindFirstChild("Torso") then
            CalcarArticulacionVisual("Torso", "Right Shoulder")
        elseif MiChar:FindFirstChild("RightUpperArm") and Dummy:FindFirstChild("RightUpperArm") then
            CalcarArticulacionVisual("RightUpperArm", "RightShoulder")
            CalcarArticulacionVisual("RightLowerArm", "RightElbow")
            CalcarArticulacionVisual("RightHand", "RightWrist")
        end
    end)
    
    -- =========================================================
    -- 🛠️ SINCRONIZACIÓN DE ANIMACIONES BASE (Stepped)
    -- =========================================================
