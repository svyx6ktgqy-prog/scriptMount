...
local function StartCloning(TargetNPC)
    ...
    -- =========================================================
    -- 🛠️ GUARDAR POSTURA ORIGINAL Y CREAR CACHÉ DE CONGELAMIENTO
    -- =========================================================
    local DummyOriginalJoints = {}
    local FrozenJoints = {} -- Memoria para congelar el brazo
    ...
    if MyHum and DummyHum then
        AnimConnection = RunService.Stepped:Connect(function()
            ...
            pcall(function()
                ...
                -- Si se desequipa el pico, limpiamos la memoria para liberar el brazo
                if not TieneHerramienta then
                    FrozenJoints = {} 
                end
                    ...
                    if track.Animation and track.Animation.AnimationId then
                        ...
                        if dummyTrack then
                            ...
                            pcall(function()
                                ...
                                -- Forzar velocidad 0 en las animaciones de acción si tienes el pico
                ...
                -- =========================================================
                -- ❄️ BLOQUEO ABSOLUTO DE ARTICULACIÓN (FREEZE)
                -- =========================================================
                        ...
                        if MiJoint and ClonJoint then
                            pcall(function()
                                if TieneHerramienta then
                                    -- Capturar la postura inicial y no cambiarla jamás
                                    if not FrozenJoints[ClonJoint] then
                                        FrozenJoints[ClonJoint] = {
                                            Transform = MiJoint.Transform,
                                            CurrentAngle = MiJoint.CurrentAngle,
                                            DesiredAngle = MiJoint.DesiredAngle,
                                            C0 = MiJoint.C0,
                                            C1 = MiJoint.C1
                                        }
                                    end
                                    
                                    -- Aplicar la postura congelada sobreescribiendo todo
                                    if MiJoint:IsA("Motor6D") and ClonJoint:IsA("Motor6D") then
                                        ClonJoint.Transform = FrozenJoints[ClonJoint].Transform
                                        ClonJoint.CurrentAngle = FrozenJoints[ClonJoint].CurrentAngle
                                        ClonJoint.DesiredAngle = FrozenJoints[ClonJoint].DesiredAngle
                                    end
                                    ClonJoint.C0 = FrozenJoints[ClonJoint].C0
                                    ClonJoint.C1 = FrozenJoints[ClonJoint].C1
                                else
                                    -- Restaurar comportamiento inactivo cuando no hay pico
